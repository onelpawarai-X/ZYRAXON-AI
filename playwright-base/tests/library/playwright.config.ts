...
import { type Config, type JarvisTestOptions, type JarvisWorkerOptions, type ReporterDescription } from '@jarvis-browser/test';
...
const reporters = () => {
  const result: ReporterDescription[] = process.env.CI ? [
    ...
    ['html', { open: 'on-failure', title: 'Jarvis Browser Library Tests' }]
  ];
  return result;
};

const config: Config<JarvisWorkerOptions & JarvisTestOptions & TestModeWorkerOptions> = {
  testDir,
  outputDir,
  expect: {
    timeout: 10000,
