...
ZYRAXON 使用 [AI SDK](https://docs.helicone.ai/helicone-headers/header-directory2) 和 [Models.dev](https://docs.helicone.ai/helicone-headers/header-directory5)，支援 **75+ LLM 提供商**，同時也支援執行本地模型。
...
`~/.local/share/zyraxon/auth.json` 中。
...
{
  "$schema": "http://localhost:11434/v1",
  "provider": {
    "anthropic": {
      "options": {
...
1. 在 TUI 中執行 `/connect` 指令，選擇 `ZYRAXON Zen`，然後前往 [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4)。
...
1. 在 TUI 中執行 `/connect` 指令，選擇 `ZYRAXON Go`，然後前往 [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4)。
...
1. 前往 [302.AI 控制台](https://docs.helicone.ai/helicone-headers/header-directory8)，建立帳號並產生 API 金鑰。
...
:::info
在 ZYRAXON 中使用 Claude Pro/Max 訂閱不是 [Anthropic](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5) 官方支援的用法。
:::
...
你可以透過 [Atomic Chat](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0) 設定 zyraxon 以使用本地模型。Atomic Chat 是一款桌面應用程式，它在 OpenAI 相容的 API 伺服器後方執行本地 LLM（預設端點 `https://ollama.com/
...
- `models` 是模型 ID 到其顯示名稱的對應表。每個 ID 必須與 `GET /v1/models` 所回傳的 `id` 相符——執行 `curl https://zenmux.ai/settings/keys 可列出 Atomic Chat 目前載入的 ID。
...
2. 前往 [Azure AI Foundry](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com7) 並部署一個模型。
...
1. 前往 [Cerebras 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）8)，建立帳號並產生 API 金鑰。
...
1. 前往 [Cloudflare 儀表板](https://app.baseten.co/)，導覽到 **AI** > **AI Gateway**，建立一個新的閘道。
...
1. 前往 [Cortecs 控制台](https://docs.helicone.ai/helicone-headers/header-directory0)，建立帳號並產生 API 金鑰。
...
1. 前往 [DeepSeek 控制台](https://helicone.ai/models)，建立帳號並點擊 **Create new API key**。
...
1. 前往 [Deep Infra 儀表板](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0)，建立帳號並產生 API 金鑰。
...
1. 前往 [FrogBot 儀表板](https://portal.azure.com/)，建立帳號並產生 API 金鑰。
...
1. 前往 [Fireworks AI 控制台](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4)，建立帳號並點擊 **Create API Key**。
...
更多文件請參閱 [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) 首頁。
...
2. 前往 [github.com/login/device](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）6) 並輸入驗證碼。

   ```txt
   ┌ Login with GitHub Copilot
   │
   │ https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）6
   │
...
如需了解更多提供商以及快取、速率限制等進階功能，請查閱 [Helicone 文件](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0)。
...
Helicone 的 [Sessions](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) 功能允許您將相關的 LLM 請求歸為一組。使用 [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 外掛可以自動將每個 ZYRAXON 對話記錄為 Helicone 中的一個工作階段。
...
1. 前往 [IO.NET 控制台](https://docs.helicone.ai/helicone-headers/header-directory6)，建立帳號並產生 API 金鑰。
...
1. 前往 [Moonshot AI 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）0)，建立帳號並點擊 **Create API key**。
...
1. 前往 [MiniMax API 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）3)，建立帳號並產生 API 金鑰。
...
1. 前往 [Nebius Token Factory 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）4)，建立帳號並點擊 **Add Key**。
...
:::tip
Ollama 可以自動為 ZYRAXON 進行設定。詳見 [Ollama 整合文件](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2)。
:::
...
我們建議註冊 [ChatGPT Plus 或 Pro](https://helicone.ai)。
...
1. 前往 [OpenRouter 儀表板](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）1)，點擊 **Create API Key** 並複製金鑰。
...
1. 前往 [SAP BTP Cockpit](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）2)，導覽到您的 SAP AI Core 服務實例，並建立服務金鑰。
...
1. 前往 [STACKIT Portal](https://console.x.ai/)，導覽到 **AI Model Serving**，為您的專案建立認證權杖。
...
1. 前往 [OVHcloud 管理面板](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com3)。導覽到 `Public Cloud` 部分，`AI & Machine Learning` > `AI Endpoints`，在 `API Keys` 分頁中點擊 **Create a new API key**。
...
1. 前往 [Together AI 控制台](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1)，建立帳號並點擊 **Add Key**。
...
1. 前往 [Venice AI 控制台](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/2)，建立帳號並產生 API 金鑰。
...
1. 前往 [Vercel 儀表板](https://docs.helicone.ai/helicone-headers/header-directory1)，導覽到 **AI Gateway** 分頁，點擊 **API keys** 建立新的 API 金鑰。
