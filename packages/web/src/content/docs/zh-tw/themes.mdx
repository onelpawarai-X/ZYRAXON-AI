...
預設情況下，ZYRAXON 使用我們自己的 `zyraxon` 主題。
...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/zyraxon/themes/*.json` 或 `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.zyraxon/themes/*.json`
4. **當前工作目錄** — 定義在 `./.zyraxon/themes/*.json`
