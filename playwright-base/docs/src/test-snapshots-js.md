...
Jarvis Browser Test includes the ability to produce and visually compare screenshots using `await expect(page).toHaveScreenshot()`. On first execution, Jarvis Browser Test will generate reference screenshots. Subsequent runs will compare against the reference.
...
```bash
npx Jarvis Browser Test --update-snapshots
```
...
Jarvis Browser Test uses the [pixelmatch](https://github.com/mapbox/pixelmatch) library. You can [pass various options](./api/class-pageassertions.md#page-assertions-to-have-screenshot-1) to modify its behavior:
...
Apart from screenshots, you can use `expect(value).toMatchSnapshot(snapshotName)` to compare text or arbitrary binary data. Jarvis Browser Test auto-detects the content type and uses the appropriate comparison algorithm.
