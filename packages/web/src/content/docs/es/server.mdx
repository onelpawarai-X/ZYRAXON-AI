---
title: Servidor
description: Interactuar con el servidor zyraxon a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `zyraxon serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente zyraxon puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `zyraxon.local` |
...
Configure `ZYRAXON_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `zyraxon`, o configure `ZYRAXON_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `zyraxon serve` como a `zyraxon web`.
...
Cuando ejecuta `zyraxon`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor zyraxon para interactuar con zyraxon mediante programación.
:::

Esta arquitectura permite que zyraxon admita múltiples clientes y le permite interactuar con zyraxon mediante programación.

Puede ejecutar `zyraxon serve` para iniciar un servidor independiente. Si tienes el
zyraxon TUI ejecutándose, `zyraxon serve` iniciará un nuevo servidor.
...
El servidor zyraxon expone las siguientes API.
