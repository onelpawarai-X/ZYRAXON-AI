---
title: Servidor
description: Interaja com o servidor zyraxon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `zyraxon serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente zyraxon pode usar.
...
```bash
zyraxon serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `zyraxon.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
zyraxon serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `ZYRAXON_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `zyraxon`, ou defina `ZYRAXON_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `zyraxon serve` quanto ao `zyraxon web`.

```bash
ZYRAXON_SERVER_PASSWORD=your-password zyraxon serve
```
...
Quando você executa `zyraxon`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor zyraxon para interagir com o zyraxon programaticamente.
:::

Essa arquitetura permite que o zyraxon suporte múltiplos clientes e permite que você interaja com o zyraxon programaticamente.

Você pode executar `zyraxon serve` para iniciar um servidor autônomo. Se você tiver o TUI do zyraxon em execução, `zyraxon serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do zyraxon [IDE](/docs/ide).
...
O servidor zyraxon expõe as seguintes APIs.
