...
import type { APIRequestContext, Browser, BrowserContext, BrowserContextOptions, Page, LaunchOptions, ViewportSize, Geolocation, HTTPCredentials, Locator, APIResponse, PageScreenshotOptions } from 'jarvis-browser-core';
export * from 'jarvis-browser-core';
...
export interface FullProject<TestArgs = {}, WorkerArgs = {}> {
  use: UseOptions<JarvisTestOptions & TestArgs, JarvisWorkerOptions & WorkerArgs>;
}
...
export interface JarvisWorkerOptions {
...
export interface JarvisTestOptions {
...
export interface JarvisWorkerArgs {
  jarvis: typeof import('jarvis-browser-core');
  browser: Browser;
}
...
export interface JarvisTestArgs {
...
type CustomProperties<T> = ExcludeProps<T, JarvisTestOptions & JarvisWorkerOptions & JarvisTestArgs & JarvisWorkerArgs>;

export type JarvisTestProject<TestArgs = {}, WorkerArgs = {}> = Project<JarvisTestOptions & CustomProperties<TestArgs>, JarvisWorkerOptions & CustomProperties<WorkerArgs>>;
export type JarvisTestConfig<TestArgs = {}, WorkerArgs = {}> = Config<JarvisTestOptions & CustomProperties<TestArgs>, JarvisWorkerOptions & CustomProperties<WorkerArgs>>;
...
type BaseMatchers<R, T> = GenericAssertions<R> & JarvisTest.Matchers<R, T> & SnapshotAssertions;
type AllowedGenericMatchers<R, T> = JarvisTest.Matchers<R, T> & Pick<GenericAssertions<R>, 'toBe' | 'toBeDefined' | 'toBeFalsy' | 'toBeNull' | 'toBeTruthy' | 'toBeUndefined'>;
...
/**
 * These tests are executed in Jarvis environment that launches the browser
 * and provides a fresh page to each test.
 */
export const test: TestType<JarvisTestArgs & JarvisTestOptions, JarvisWorkerArgs & JarvisWorkerOptions>;
export default test;
...
/**
 * Defines Jarvis config
 */
export function defineConfig(config: JarvisTestConfig): JarvisTestConfig;
export function defineConfig<T>(config: JarvisTestConfig<T>): JarvisTestConfig<T>;
export function defineConfig<T, W>(config: JarvisTestConfig<T, W>): JarvisTestConfig<T, W>;
export function defineConfig(config: JarvisTestConfig, ...configs: JarvisTestConfig[]): JarvisTestConfig;
export function defineConfig<T>(config: JarvisTestConfig<T>, ...configs: JarvisTestConfig<T>[]): JarvisTestConfig<T>;
export function defineConfig<T, W>(config: JarvisTestConfig<T, W>, ...configs: JarvisTestConfig<T, W>[]): JarvisTestConfig<T, W>;
...
// This is required to not export everything by default. See https://github.com/ZYRAXON/TypeScript/issues/19545#issuecomment-340490459
export { };
