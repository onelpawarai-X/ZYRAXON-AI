...
import type { LLMClientShape } from "@zyraxon-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("zyraxon"))
    return { type: "unsupported", reason: "provider is not openai, zyraxon, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @zyraxon-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains zyraxon-owned. The native runtime only adapts
      // the @zyraxon-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
