---
title: SDK
description: Client JS de type sécurisé pour le serveur zyraxon.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK zyraxon JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler zyraxon par programme.
...
Créez une instance de zyraxon :

```javascript
import { createZyraxon } from "@zyraxon-ai/sdk"

const { client } = await createZyraxon()
```
...
console.log(`Server running at ${zyraxon.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de zyraxon, vous pouvez créer une instance client pour vous y connecter :
...
const textResults = await client.find.text({
  query: { pattern: "function.*zyraxon" },
})
