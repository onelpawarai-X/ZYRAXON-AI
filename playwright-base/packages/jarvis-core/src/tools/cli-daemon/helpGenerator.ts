...
function generateCommandHelp(command: AnyCommandSchema) {
  ...
  const lines: string[] = [
    `jarvis-cli ${command.name} ${commandArgsText(args)}`,
    '',
    command.description,
    '',
  ];
...
export function generateHelp() {
  const lines: string[] = [];
  lines.push('Usage: jarvis-cli <command> [args] [options]');
  lines.push('Usage: jarvis-cli -s=<session> <command> [args] [options]');
...
function generateReadmeEntry(command: AnyCommandSchema): string {
  const args = commandArgs(command);
  const prefix = `jarvis-cli ${command.name} ${commandArgsText(args)}`;
  const suffix = '# ' + command.description.toLowerCase();
