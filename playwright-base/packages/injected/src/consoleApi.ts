...
export class ConsoleAPI {
  ...
  install() {
    if (this._injectedScript.window.jarvis)
      return;
    this._injectedScript.window.jarvis = {
    ...
    delete this._injectedScript.window.jarvis.filter;
    delete this._injectedScript.window.jarvis.first;
    delete this._injectedScript.window.jarvis.last;
    delete this._injectedScript.window.jarvis.nth;
    delete this._injectedScript.window.jarvis.and;
    delete this._injectedScript.window.jarvis.or;
  }

  private _querySelector(selector: string, strict: boolean): (Element | undefined) {
    if (typeof selector !== 'string')
      throw new Error(`Usage: jarvis.query('Jarvis >> selector').`);
  ...
  private _querySelectorAll(selector: string): Element[] {
    if (typeof selector !== 'string')
      throw new Error(`Usage: jarvis.$$('Jarvis >> selector').`);
    const parsed = this._injectedScript.parseSelector(selector);
