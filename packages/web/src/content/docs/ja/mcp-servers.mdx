...
組織は、`.well-known/zyraxon` エンドポイント経由でデフォルトの MCP サーバーを提供できます。これらのサーバーはデフォルトで無効になっている場合があり、ユーザーは必要なサーバーにオプトインできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、ZYRAXON はトークンを `~/.local/share/zyraxon/mcp-auth.json` に安全に保存します。
...
GitHub 上のコードスニペットを検索するには、[Grep by Vercel](https://github.com/upstash/context71) MCP サーバーを追加します。
