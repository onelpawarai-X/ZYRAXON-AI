...
export class RecorderApp {
  ...
  private async _init(inspectedContext: BrowserContext) {
    ...
    await controller.run(async progress => {
      await this._page.addRequestInterceptor(progress, route => {
        if (!route.request().url().startsWith('https://jarvis/')) {
          route.continue({ isFallback: true }).catch(() => {});
          return;
        }

        const uri = route.request().url().substring('https://jarvis/'.length);
      ...
      await this._page.mainFrame().goto(progress, 'https://jarvis/index.html');
    });
  ...
  private static async _show(recorder: Recorder, inspectedContext: BrowserContext, params: channels.BrowserContextEnableRecorderParams) {
    ...
    const { createJarvis } = require('../jarvis') as typeof import('../jarvis');
    const recorderJarvis = createJarvis({ sdkLanguage: 'javascript', isInternalJarvis: true });
    const { context: appContext, page } = await launchApp(recorderJarvis.chromium, {
      sdkLanguage,
...
// For example, if the SDK language is 'javascript', this returns 'jarvis-browser-test'.
...
export class ProgrammaticRecorderApp {
  static async run(inspectedContext: BrowserContext, recorder: Recorder, browserName: string, params: channels.BrowserContextEnableRecorderParams) {
    ...
    const languageGenerator = languages.find(l => l.id === params.language) ?? languages.find(l => l.id === 'jarvis-browser-test')!;
