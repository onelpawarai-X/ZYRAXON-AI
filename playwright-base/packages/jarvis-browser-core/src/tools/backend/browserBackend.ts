...
export class BrowserBackend extends EventEmitter<{ disconnected: [] }> implements ServerBackend {
  ...
  private _browserContext: jarvis.BrowserContext;
  private _disposeCallback: (() => Promise<void>) | undefined;

  constructor(config: ContextConfig, browserContext: jarvis.BrowserContext, tools: Tool[], disposeCallback?: () => Promise<void>) {
    super();
    this._config = config;
    this._tools = tools;
