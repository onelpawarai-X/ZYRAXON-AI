#!/usr/bin/env bash
# Launch ZYRAXON Code (ZYRAXON Code from sources) with:
...
#   --repo                  $PWD if it looks like a zyraxon checkout; otherwise pass it explicitly
...
if [[ -z "$REPO" ]]; then
	if [[ -x "$PWD/scripts/code.sh" ]]; then
		...
		echo "Could not find a zyraxon checkout in $PWD. Pass --repo <path>." >&2
		exit 2
	fi
fi
...
# Excludes (deny-list, so future ZYRAXON Code additions copy through by default).
...
# Strip ELECTRON_RUN_AS_NODE, commonly inherited from ZYRAXON Code's integrated
# terminal / agent runtimes; it breaks ./scripts/code.sh.
unset ELECTRON_RUN_AS_NODE

CODE_SH="$REPO/scripts/code.sh"
if [[ ! -x "$CODE_SH" ]]; then
	echo "Could not find an executable ZYRAXON Code launcher at $CODE_SH. Pass --repo <vscode-repo-root>." >&2
	exit 2
fi
