...
Par défaut, ZYRAXON utilise notre propre thème `zyraxon`.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/zyraxon/themes/*.json` ou `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.zyraxon/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.zyraxon/themes/*.json`
