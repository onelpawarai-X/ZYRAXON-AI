...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\zyraxon\log`
...
zyraxon speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/zyraxon/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\zyraxon`
...
ZYRAXON Desktop startet im Hintergrund einen lokalen ZYRAXON-Server (`zyraxon-cli`-Sidecar).
...
- **macOS/Linux**: `~/.config/zyraxon/zyraxon.jsonc` (oder `~/.config/zyraxon/zyraxon.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
- **Globale Plugins**
  ...
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\zyraxon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/zyraxon`
- **Linux**: loesche `~/.cache/zyraxon` (oder fuehre aus: `rm -rf ~/.cache/zyraxon`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\zyraxon`
...
Wenn du `ZYRAXON_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `ZYRAXON_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
- `zyraxon.global.dat` und `zyraxon.workspace.*.dat` (UI state like recent servers/projects)
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\zyraxon`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. zyraxon installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\zyraxon`

2. Starte zyraxon neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt zyraxon, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
zyraxon erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
