...
import { JarvisPipeServer } from '../remote/jarvisPipeServer';
import { JarvisWebSocketServer } from '../remote/jarvisWebSocketServer';
...
export abstract class Browser extends SdkObject {
  ...
  sdkLanguage() {
    return this.options.sdkLanguage || this.attribution.jarvisBrowser.options.sdkLanguage;
  }
...
export class BrowserServer {
  ...
  private _pipeServer?: JarvisPipeServer;
  private _wsServer?: JarvisWebSocketServer;
  ...
  async start(title: string, options: channels.BrowserStartServerOptions): Promise<{ endpoint: string }> {
    ...
    if (options.host !== undefined || options.port !== undefined) {
      this._wsServer = new JarvisWebSocketServer(this._browser, '/' + createGuid());
      endpoint = await this._wsServer.listen(options.port ?? 0, options.host);
    } else {
      this._pipeServer = new JarvisPipeServer(this._browser);
...
function asClientLaunchOptions(serverOptions: types.LaunchOptions): jarvis.LaunchOptions {
  return {
    ...serverOptions,
