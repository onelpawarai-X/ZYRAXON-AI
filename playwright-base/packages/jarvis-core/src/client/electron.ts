...
export class Electron extends ChannelOwner<channels.ElectronChannel> implements api.Electron {
  ...
  async launch(options: ElectronOptions = {}): Promise<ElectronApplication> {
    options = this._jarvis.selectors._withSelectorOptions(options);
    ...
    this._jarvis.selectors._contextsForSelectors.add(app._context);
    app.once(Events.ElectronApplication.Close, () => this._jarvis.selectors._contextsForSelectors.delete(app._context));
