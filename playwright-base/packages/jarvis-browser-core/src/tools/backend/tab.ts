...
export class Tab extends EventEmitter<TabEventsInterface> {
  ...
  private _requests: jarvis.Request[] = [];
  ...
  constructor(context: Context, page: jarvis.Page, onPageClose: (tab: Tab) => void) {
    super();
  ...
  static forPage(page: jarvis.Page): Tab | undefined {
    // eslint-disable-next-line no-restricted-syntax
    return (page as any)[tabSymbol];
  }

  static async collectConsoleMessages(page: jarvis.Page): Promise<ConsoleMessage[]> {
  ...
  private _dialogShown(dialog: jarvis.Dialog) {
    this.setModalState({
      type: 'dialog',
  ...
  private _handleRequest(request: jarvis.Request) {
    this._requests.push(request);
  ...
  private _handleResponse(response: jarvis.Response) {
  ...
  private _handleRequestFailed(request: jarvis.Request) {
    const timing = request.timing();
  ...
  async navigate(url: string) {
    ...
    const { promise: downloadEvent, abort: abortDownloadEvent } = eventWaiter<jarvis.Download>(this.page, 'download', 3000);
    try {
  ...
  async requests(): Promise<jarvis.Request[]> {
    await this._initializedPromise;
  ...
  async captureSnapshot(root: jarvis.Locator | undefined, depth: number | undefined, boxes: boolean | undefined, relativeTo: string | undefined, ariaFormat: 'none' | 'text' | 'json' = 'text'): Promise<TabSnapshot> {
    await this._initializedPromise;
  ...
  async targetLocator(params: { element?: string, target: string }): Promise<{ locator: jarvis.Locator, resolved: string, selector: string }> {
    await this._initializedPromise;
    return (await this.targetLocators([params]))[0];
  }

  async targetLocators(params: { element?: string, target: string }[]): Promise<{ locator: jarvis.Locator, resolved: string, selector: string }[]> {
    await this._initializedPromise;
...
export type ConsoleMessage = {
  type: ReturnType<jarvis.ConsoleMessage['type']>;
  timestamp: number;
  text: string;
  toString(): string;
};

export function locatorSelector(locator: jarvis.Locator): string {
  return (locator as unknown as { _selector: string })._selector;
}

function messageToConsoleMessage(message: jarvis.ConsoleMessage): ConsoleMessage {
  return {
    type: message.type(),
...
type ConsoleMessageType = ReturnType<jarvis.ConsoleMessage['type']>;
