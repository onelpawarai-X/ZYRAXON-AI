...
type JarvisOptions = {
  ...
  isInternalJarvis?: boolean;
  isServer?: boolean;
  isClientCollocatedWithServer?: boolean;
};

export class Jarvis extends SdkObject {
  readonly chromium: BrowserType;
  readonly android: Android;
  readonly electron: Electron;
  readonly firefox: BrowserType;
  readonly webkit: BrowserType;
  readonly options: JarvisOptions;
  readonly debugController: DebugController;
  private _allPages = new Set<Page>();
  private _allBrowsers = new Set<Browser>();

  constructor(options: JarvisOptions) {
    super(createRootSdkObject(), undefined, 'Jarvis');
    this.options = options;
    this.attribution.jarvisBrowser = this;
...
export function createJarvis(options: JarvisOptions) {
