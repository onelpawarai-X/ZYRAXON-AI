...
Você pode adicionar ferramentas externas ao zyraxon usando o _Model Context Protocol_, ou MCP. O zyraxon suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do zyraxon](https://zyraxon.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/zyraxon`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O zyraxon lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o zyraxon irá:
...
Se o servidor requer autenticação, o zyraxon solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `zyraxon mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o zyraxon armazenará os tokens de forma segura em `~/.local/share/zyraxon/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no zyraxon, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do zyraxon como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o zyraxon à sua conta do Sentry.
...
Adicione o [Grep by Vercel](https://github.com/upstash/context71) servidor MCP para pesquisar através de trechos de código no GitHub.
