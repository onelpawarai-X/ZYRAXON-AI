...
export class TerminalReporter implements ReporterV2 {
  ...
  private _printSlowTests() {
    ...
    if (slowTests.length)
      this.writeLine(this.screen.colors.yellow('  Consider running tests from slow files in parallel. See: https://jarvisbrowser.dev/docs/test-parallel'));
  }
    ...
    for (let i = 0; i < attachmentGroups.length; ++i) {
      ...
      if (attachment.actual?.path) {
        ...
        if (attachment.name === 'trace') {
          ...
          resultLines.push('');
          resultLines.push(screen.colors.dim(`        ${packageManagerCommand} jarvis show-trace ${quotePathIfNeeded(relativePath)}`));
          resultLines.push('');
        }
      } else {
