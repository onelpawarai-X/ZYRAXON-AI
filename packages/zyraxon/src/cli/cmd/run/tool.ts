...
import type { ToolPart } from "@zyraxon-ai/sdk/v2"
...
export function toolEntryBody(commit: StreamCommit, raw: string): RunEntryBody | undefined {
  ...
  // Detect media generation tools in progress — show generating animation
  if (commit.phase === "progress" && commit.kind === "tool") {
    const toolName = (commit.tool || commit.part?.tool || "").toLowerCase()
    const input = commit.part?.input || {}
    const prompt = (input as any).prompt || (input as any).text || (input as any).description || ""

    if (toolName.includes("image") || toolName.includes("generate_image") || toolName.includes("flux")) {
      return { type: "generating", category: "image", prompt: String(prompt) }
    }
    if (toolName.includes("video") || toolName.includes("generate_video") || toolName.includes("wan")) {
      return { type: "generating", category: "video", prompt: String(prompt) }
    }
    if (toolName.includes("audio") || toolName.includes("music") || toolName.includes("generate_audio") || toolName.includes("musicgen")) {
      return { type: "generating", category: "audio", prompt: String(prompt) }
    }
    if (toolName.includes("tts") || toolName.includes("bark") || toolName.includes("speak") || toolName.includes("speech")) {
      return { type: "generating", category: "tts", prompt: String(prompt) }
    }
  }
