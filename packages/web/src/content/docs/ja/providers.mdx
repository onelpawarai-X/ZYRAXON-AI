...
ZYRAXON は [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) および [Models.dev](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com6) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。
...
`~/.local/share/zyraxon/auth.json`で。
...
1. TUI で `/connect` コマンドを実行し、`ZYRAXON Zen` を選択して、[zyraxon.ai/zen](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5) にアクセスします。
...
1. TUI で `/connect` コマンドを実行し、`ZYRAXON Go` を選択して、[zyraxon.ai/zen](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5) にアクセスします。
...
1. [302.AI console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9) に移動し、アカウントを作成し、API キーを生成します。
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   zyraxon の実行中に次の環境変数のいずれかを設定します。
...
zyraxon は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `https://ollama.com/
...
- `models` はモデル ID と表示名のマップです。各 ID は `GET /v1/models` が返す `id` と一致する必要があります。Atomic Chat に現在ロードされている ID の一覧は `curl https://zenmux.ai/settings/keys を実行して確認できます。
...
1. [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3) に移動し、アカウントを作成し、API キーを生成します。
...
1. [Cerebras console](https://deepinfra.com/dash) に移動し、アカウントを作成し、API キーを生成します。
...
1. [Cloudflare ダッシュボード ](https://anthropic.com) に移動し、**AI** > **AI ゲートウェイ** に移動し、新しいゲートウェイを作成します。
...
1. [Cortecs console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1) に移動し、アカウントを作成し、API キーを生成します。
...
1. [DeepSeek コンソール ](https://helicone.ai/models) に移動し、アカウントを作成し、**新しい API キーの作成** をクリックします。
...
1. [STACKIT Portal](https://ai.azure.com/) に移動し、**AI Model Serving** に移動して、プロジェクトの認証トークンを作成します。
...
詳細については、[zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) ホームページ。
...
2. [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) に移動し、コードを入力します。
...
1. [Groq console](https://developers.cloudflare.com/ai-gateway/features/unified-billing/7) に移動し、[**API キーの作成**] をクリックして、キーをコピーします。
...
[Hugging Face Inference Provider](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) は、17 を超えるプロバイダーがサポートするオープンモデルへのアクセスを提供します。
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) は、AI アプリケーションのロギング、監視、分析を提供する LLM 可観測性プラットフォームです。 Helicone AI ゲートウェイは、モデルに基づいてリクエストを適切なプロバイダーに自動的にルーティングします。

1. [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) に移動し、アカウントを作成し、ダッシュボードから API キーを生成します。
...
その他のプロバイダーや、キャッシュやレート制限などの高度な機能については、[Helicone ドキュメント](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) プラグインを使用して、各 ZYRAXON 会話を Helicone のセッションとして自動的に記録します。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように zyraxon を構成できます。
...
1. [IO.NET console](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2) に移動し、アカウントを作成し、API キーを生成します。
...
LM Studio を通じてローカルモデルを使用するように zyraxon を構成できます。
...
1. [Moonshot AI console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0) に移動し、アカウントを作成し、[**API キーの作成**] をクリックします。
...
1. [MiniMax API Console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) に移動し、アカウントを作成し、API キーを生成します。
...
1. [Nebius Token Factory console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4) に移動し、アカウントを作成し、[**キーの追加**] をクリックします。
...
Ollama を通じてローカルモデルを使用するように zyraxon を構成できます。

:::tip
Ollama は ZYRAXON 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3)」を参照してください。
:::
...
```json title="zyraxon.json" "ollama" {5, 6, 8, 10-14}
{
  ...
  "provider": {
    "ollama": {
      ...
      "name": "Ollama (local)",
      "options": {
        "baseURL": "https://developers.cloudflare.com/ai-gateway/features/unified-billing/4"
      },
      "models": {
        "llama2": {
          "name": "Llama 2"
...
[ChatGPT Plus または Pro](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) にサインアップすることをお勧めします。
...
1. [OpenRouter ダッシュボード](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1) に移動し、[**API キーの作成**] をクリックして、キーをコピーします。
...
1. [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2) に移動し、SAP AI コアサービスインスタンスに移動して、サービスキーを作成します。
...
1. [OVHcloud パネル](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4) に移動します。 `Public Cloud` セクション、`AI & Machine Learning` > `AI Endpoints` に移動し、`API Keys` タブで **新しい API キーの作成** をクリックします。
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を Zyraxon で使用するには:
...
1. [Venice AI console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8) に移動し、アカウントを作成し、API キーを生成します。
...
1. [Vercel ダッシュボード ](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2) に移動し、**AI ゲートウェイ** タブに移動し、**API キー** をクリックして新しい API キーを作成します。
...
1. [xAI console](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7) に移動し、アカウントを作成し、API キーを生成します。
