...
const codeSchema = z.object({
  code: z.string().optional().describe(`A JavaScript function containing Jarvis code to execute. It will be invoked with a single argument, page, which you can use for any page interaction. For example: \`async (page) => { await page.getByRole('button', { name: 'Submit' }).click(); return await page.title(); }\``),
...
const runCode = defineTabTool({
  ...
  schema: {
    ...
    title: 'Run Jarvis code (unsafe)',
    description: 'Run a Jarvis code snippet. Unsafe: executes arbitrary JavaScript in the Jarvis server process and is RCE-equivalent.',
