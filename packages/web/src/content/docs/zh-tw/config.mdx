...
1. **遠端設定**（來自 `.well-known/zyraxon`）- 組織預設值
2. **全域設定**（`~/.config/zyraxon/zyraxon.json`）- 使用者偏好
3. **自訂設定**（`ZYRAXON_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `zyraxon.json`）- 專案特定設定
5. **`.zyraxon` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`ZYRAXON_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.zyraxon` 和 `~/.config/zyraxon` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
組織可以透過 `.well-known/zyraxon` 端點提供預設設定。當您使用支援該功能的供應商進行身分驗證時，會自動擷取此設定。
...
將全域 ZYRAXON 設定放在 `~/.config/zyraxon/zyraxon.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
使用 `ZYRAXON_CONFIG` 環境變數指定自訂設定檔路徑。
...
使用 `ZYRAXON_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.zyraxon` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
自訂目錄在全域設定和 `.zyraxon` 目錄之後載入，因此**可以覆寫**它們的設定。
...
使用 `ZYRAXON_TUI_CONFIG` 指向自訂 TUI 設定檔。
...
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `zyraxon.local`。適用於在同一網路上執行多個實例的情境。
...
您還可以使用 `~/.config/zyraxon/agents/` 或 `.zyraxon/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
您還可以使用 `~/.config/zyraxon/commands/` 或 `.zyraxon/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
將外掛程式檔案放置在 `.zyraxon/plugins/` 或 `~/.config/zyraxon/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
