...
// Permission-controlled: privileged operations require authorization
...
// Privileged operations require authorization tokens
// ============================================

interface PermissionEntry {
  tool: string
  requiredLevel: "read" | "write" | "execute" | "admin"
  description: string
}

const TOOL_PERMISSIONS: Record<string, PermissionEntry> = {
  omniMultiFileCompose: { tool: "omni_multi_file_compose", requiredLevel: "write", description: "File create/edit/delete" },
  omniTerminalAutonomy: { tool: "omni_terminal_autonomy", requiredLevel: "execute", description: "Shell command execution" },
  omniBackgroundLoop: { tool: "omni_background_loop", requiredLevel: "execute", description: "Background task spawning" },
  omniBehaviorCascade: { tool: "omni_behavior_cascade", requiredLevel: "read", description: "Behavior analysis (read-only)" },
  omniGitPair: { tool: "omni_git_pair", requiredLevel: "write", description: "Git commit/rollback" },
  omniSandboxRuntime: { tool: "omni_sandbox_runtime", requiredLevel: "execute", description: "Sandboxed code execution" },
  omniIdeaToDeploy: { tool: "omni_idea_to_deploy", requiredLevel: "write", description: "Project file generation" },
  omniCostRouter: { tool: "omni_cost_router", requiredLevel: "read", description: "Model routing (read-only)" },
  omniLiveObservability: { tool: "omni_live_observability", requiredLevel: "read", description: "Log analysis (read-only)" },
  omniKnowledgeGraph: { tool: "omni_knowledge_graph", requiredLevel: "write", description: "Knowledge store access" },
  omniSecurityScan: { tool: "omni_security_scan", requiredLevel: "read", description: "Security scanning (read-only)" },
  omniSilentPrecompile: { tool: "omni_silent_precompile", requiredLevel: "read", description: "Syntax verification (read-only)" },
  omniVisualContextSync: { tool: "omni_visual_context_sync", requiredLevel: "read", description: "Screen analysis (read-only)" },
  omniSelfEvolvingPrompt: { tool: "omni_self_evolving_prompt", requiredLevel: "write", description: "Prompt rule modification" },
  omniZeroTrustStrip: { tool: "omni_zero_trust_strip", requiredLevel: "read", description: "Credential scanning (read-only)" },
}

let _authorizedTools: Set<string> = new Set()
let _authTimestamp = 0
const AUTH_TTL_MS = 300_000 // 5 minutes

/**
 * Authorization token for privileged operations.
 * Generated via HMAC-SHA256 of tool name + timestamp + nonce.
 * Input: tool name, caller nonce
 * Output: authorization token string
 * Boundary: tokens expire after AUTH_TTL_MS
 */
function generateAuthToken(toolName: string, nonce: string): string {
  const timestamp = Date.now()
  const payload = `${toolName}:${timestamp}:${nonce}`
  return crypto.createHmac("sha256", "zyraxon-omni-kernel").update(payload).digest("hex")
}

/**
 * Validate that a tool is authorized to execute.
 * Input: tool function name
 * Output: true if authorized, false otherwise
 * Boundary: checks TTL and set membership
 */
function isToolAuthorized(toolFnName: string): boolean {
  if (Date.now() - _authTimestamp > AUTH_TTL_MS) {
    _authorizedTools.clear()
    _authTimestamp = Date.now()
  }
  return _authorizedTools.has(toolFnName)
}

/**
 * Pre-authorize a tool for execution.
 * Called by the registry layer before dispatch.
 * Input: tool function name
 * Output: void
 */
export function authorizeTool(toolFnName: string): void {
  _authorizedTools.add(toolFnName)
  _authTimestamp = Date.now()
}

/**
 * Check permission level for a privileged operation.
 * Input: tool function name, required level
 * Output: true if permitted
 * Boundary: returns false for unknown tools
 */
export function checkPermission(toolFnName: string, requiredLevel: "read" | "write" | "execute" | "admin"): boolean {
  const entry = TOOL_PERMISSIONS[toolFnName]
  if (!entry) return false
  const levels = ["read", "write", "execute", "admin"]
  const requiredIdx = levels.indexOf(requiredLevel)
  const grantedIdx = levels.indexOf(entry.requiredLevel)
 ...
 * Input: tool function name
 * Output: permission entry or null
 */
export function getToolPermissionInfo(toolFnName: string): PermissionEntry | null {
  return TOOL_PERMISSIONS[toolFnName] ?? null
}
