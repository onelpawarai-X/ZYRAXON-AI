...
async function generateChromiumProtocol(executablePath) {
  const outputPath = path.join(__dirname, '../../packages/jarvis-browser-core/src/server/chromium/protocol.d.ts');
  const jarvis = require('jarvis-browser-core').chromium;
  const browser = await jarvis.launch({ executablePath, args: ['--remote-debugging-port=9339'] });
...
async function generateWebKitProtocol(folderPath) {
  const outputPath = path.join(__dirname, '../../packages/jarvis-browser-core/src/server/webkit/protocol.d.ts');
...
async function generateFirefoxProtocol(executablePath) {
  const outputPath = path.join(__dirname, '../../packages/jarvis-browser-core/src/server/firefox/protocol.d.ts');
