...
export class DesktopCommanderIntegration {
    ...
    handleLocalDisconnect(reason) {
        if (this.isShuttingDown)
            return; // expected teardown, not a fault
        if (!this.isReady)
            return; // already handled; don't double-fire
        this.isReady = false;
        this.mcpClient = null;
        this.mcpTransport = null;
    ...
    async initialize() {
        ...
        try {
            ...
            this.mcpClient.onerror = (err) => console.error(` - ⚠️  Local Desktop Commander MCP error: ${err?.message ?? String(err)}`);
