...
Com o zyraxon, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o zyraxon usa nosso próprio tema `zyraxon`.
...
O zyraxon vem com vários temas embutidos.
...
- Querem que o zyraxon corresponda à aparência do seu terminal
...
O zyraxon suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/zyraxon/themes/*.json` ou `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.zyraxon/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.zyraxon/themes/*.json`
