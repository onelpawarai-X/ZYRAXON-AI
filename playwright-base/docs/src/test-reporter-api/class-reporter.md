...
/** @implements {import('@jarvis-browser/test/reporter').Reporter} */
class MyReporter {
  constructor(options) {
...
Called after all tests have been run, or testing has been interrupted. Note that this method may return a [Promise] and Jarvis Browser Test will await it.
...
Whether this reporter uses stdio for reporting. When it does not, Jarvis Browser Test could add some output to enhance user experience. If your reporter does not print to the terminal, it is strongly recommended to return `false`.
