...
export class BrowserType extends ChannelOwner<channels.BrowserTypeChannel> implements api.BrowserType {
  ...
  async launch(options: LaunchOptions = {}): Promise<Browser> {
    ...
    const logger = options.logger || this._jarvis._defaultLaunchOptions?.logger;
    options = { ...this._jarvis._defaultLaunchOptions, ...options };
  ...
  async launchPersistentContext(userDataDir: string, options: LaunchPersistentContextOptions = {}): Promise<BrowserContext> {
    assert(!(options as any).port, 'Cannot specify a port without launching as a server.');
    options = this._jarvis.selectors._withSelectorOptions({
      ...this._jarvis._defaultLaunchOptions,
      ...options,
    });
  ...
  async _connect(params: ConnectOptions): Promise<Browser> {
    return await this._wrapApiCall(async () => {
      const browser = await connectToBrowser(this._jarvis, { browserName: this.name(), ...params });
