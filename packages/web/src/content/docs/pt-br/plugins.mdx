---
title: Plugins
description: Escreva seus próprios plugins para estender o zyraxon.
---

Plugins permitem que você estenda o zyraxon conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do zyraxon.
...
- `.zyraxon/plugins/` - Plugins em nível de projeto
- `~/.config/zyraxon/plugins/` - Plugins globais
...
```json title="zyraxon.json"
{
  ...
  "plugin": ["zyraxon-helicone-session", "zyraxon-wakatime", "@my-org/custom-plugin"]
}
```
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/zyraxon/node_modules/`.
...
1. Configuração global (`~/.config/zyraxon/zyraxon.json`)
2. Configuração do projeto (`zyraxon.json`)
3. Diretório de plugins global (`~/.config/zyraxon/plugins/`)
4. Diretório de plugins do projeto (`.zyraxon/plugins/`)
...
```json title=".zyraxon/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
O zyraxon executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.

```ts title=".zyraxon/plugins/my-plugin.ts"
...
```js title=".zyraxon/plugins/example.js"
...
- `client`: Um cliente SDK do zyraxon para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o zyraxon.
...
```js title=".zyraxon/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "zyraxon"'`
      }
    },
  }
}
...
:::note
Se você estiver usando o aplicativo desktop zyraxon, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o zyraxon de ler arquivos `.env`:

```javascript title=".zyraxon/plugins/env-protection.js"
...
```javascript title=".zyraxon/plugins/inject-env.js"
...
Plugins também podem adicionar ferramentas personalizadas ao zyraxon:

```ts title=".zyraxon/plugins/custom-tools.ts"
...
O helper `tool` cria uma ferramenta personalizada que o zyraxon pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o zyraxon junto com as ferramentas integradas.
...
```ts title=".zyraxon/plugins/compaction.ts"
import type { Plugin } from "@zyraxon-ai/plugin"
...
```ts title=".zyraxon/plugins/custom-compaction.ts"
import type { Plugin } from "@zyraxon-ai/plugin"
