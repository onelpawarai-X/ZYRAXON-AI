...
Por ejemplo, si su configuración global establece `theme: "zyraxon"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/zyraxon`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/zyraxon/zyraxon.json`) - preferencias del usuario
3. **Configuración personalizada** (`ZYRAXON_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`zyraxon.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.zyraxon`** - agentes, comandos, complementos
6. **Configuración en línea** (`ZYRAXON_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.zyraxon` y `~/.config/zyraxon` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/zyraxon`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global ZYRAXON en `~/.config/zyraxon/zyraxon.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `ZYRAXON_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `ZYRAXON_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.zyraxon`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.zyraxon`, por lo que **puede anular** su configuración.
...
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `zyraxon.local`. Útil para ejecutar múltiples instancias en la misma red.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/zyraxon/agents/` o `.zyraxon/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/zyraxon/commands/` o `.zyraxon/commands/`. [Más información aquí](/docs/commands).
...
De forma predeterminada, zyraxon **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Coloque los archivos de complemento en `.zyraxon/plugins/` o `~/.config/zyraxon/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
