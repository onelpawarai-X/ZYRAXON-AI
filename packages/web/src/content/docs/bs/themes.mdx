...
Po defaultu, ZYRAXON koristi nasu `zyraxon` temu.
...
2. **Korisnicki config direktorij** - `~/.config/zyraxon/themes/*.json` ili `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.zyraxon/themes/*.json`
4. **Trenutni radni direktorij** - `./.zyraxon/themes/*.json`
