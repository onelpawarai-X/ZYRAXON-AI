...
export class WKBrowser extends Browser {
  ...
  static async connect(parent: SdkObject, transport: ConnectionTransport, options: BrowserOptions): Promise<WKBrowser> {
    ...
    const promises: Promise<any>[] = [
      browser._browserSession.send('Jarvis.enable'),
    ];
  ...
  constructor(parent: SdkObject, transport: ConnectionTransport, options: BrowserOptions) {
    ...
    this._browserSession.on('Jarvis.pageProxyCreated', this._onPageProxyCreated.bind(this));
    this._browserSession.on('Jarvis.pageProxyDestroyed', this._onPageProxyDestroyed.bind(this));
    this._browserSession.on('Jarvis.provisionalLoadFailed', event => this._onProvisionalLoadFailed(event));
    this._browserSession.on('Jarvis.windowOpen', event => this._onWindowOpen(event));
    this._browserSession.on('Jarvis.downloadCreated', this._onDownloadCreated.bind(this));
    this._browserSession.on('Jarvis.downloadFilenameSuggested', this._onDownloadFilenameSuggested.bind(this));
    this._browserSession.on('Jarvis.downloadFinished', this._onDownloadFinished.bind(this));
  ...
  async doCreateNewContext(options: types.BrowserContextOptions): Promise<BrowserContext> {
    ...
    const createOptions = proxy ? {
      ...
      // See https://github.com/zyraxon-ai/jarvis-browser/issues/20451
    ...
    const { browserContextId } = await this._browserSession.send('Jarvis.createContext', createOptions);
  ...
  _onDownloadCreated(payload: Protocol.Jarvis.downloadCreatedPayload) {
  ...
  _onDownloadFilenameSuggested(payload: Protocol.Jarvis.downloadFilenameSuggestedPayload) {
    this.downloadFilenameSuggested(payload.uuid, payload.suggestedFilename);
  }

  _onDownloadFinished(payload: Protocol.Jarvis.downloadFinishedPayload) {
    this.downloadFinished(payload.uuid, payload.error);
  }

  _onPageProxyCreated(event: Protocol.Jarvis.pageProxyCreatedPayload) {
  ...
  _onPageProxyDestroyed(event: Protocol.Jarvis.pageProxyDestroyedPayload) {
    const pageProxyId = event.pageProxyId;
  ...
  _onProvisionalLoadFailed(event: Protocol.Jarvis.provisionalLoadFailedPayload) {
    const wkPage = this._wkPages.get(event.pageProxyId);
    if (!wkPage)
      return;
  ...
  _onWindowOpen(event: Protocol.Jarvis.windowOpenPayload) {
    const wkPage = this._wkPages.get(event.pageProxyId);
    if (!wkPage)
      return;
...
export class WKBrowserContext extends BrowserContext {
  ...
  override async initialize() {
    ...
    promises.push(this._browser._browserSession.send('Jarvis.setDownloadBehavior', {
    ...
    if (this._options.ignoreHTTPSErrors || this._options.internalIgnoreHTTPSErrors)
      promises.push(this._browser._browserSession.send('Jarvis.setIgnoreCertificateErrors', { browserContextId, ignore: true }));
    if (this._options.locale)
      promises.push(this._browser._browserSession.send('Jarvis.setLanguages', { browserContextId, languages: [this._options.locale] }));
  ...
  override async doCreateNewPage(): Promise<Page> {
    const { pageProxyId } = await this._browser._browserSession.send('Jarvis.createPage', { browserContextId: this._browserContextId });
    return this._browser._wkPages.get(pageProxyId)!._page;
  }

  async doGetCookies(urls: string[]): Promise<channels.NetworkCookie[]> {
    const { cookies } = await this._browser._browserSession.send('Jarvis.getAllCookies', { browserContextId: this._browserContextId });
  ...
  async addCookies(cookies: channels.SetNetworkCookie[]) {
    const cc = network.rewriteCookies(cookies).map(c => {
      const { name, value, domain, path, expires, httpOnly, secure, sameSite, partitionKey } = c;
      const copy: Protocol.Jarvis.SetCookieParam = {
        name,
        value,
        domain: domain!,
        path: path!,
    ...
    await this._browser._browserSession.send('Jarvis.setCookies', { cookies: cc, browserContextId: this._browserContextId });
  }

  async doClearCookies() {
    await this._browser._browserSession.send('Jarvis.deleteAllCookies', { browserContextId: this._browserContextId });
  }
  ...
  async setGeolocation(geolocation?: types.Geolocation): Promise<void> {
    ...
    await this._browser._browserSession.send('Jarvis.setGeolocationOverride', { browserContextId: this._browserContextId, geolocation: payload });
  }
  ...
  override async doexposeJarvisBinding() {
    for (const page of this.pages())
      await (page.delegate as WKPage).exposeJarvisBinding();
  }

  override async clearCache(): Promise<void> {
    // We use ephemeral contexts so there is no disk cache.
    await this._browser._browserSession.send('Jarvis.clearMemoryCache', {
  ...
  async doClose(reason: string | undefined): Promise<void | 'close-browser'> {
    if (!this._browserContextId) {
      ...
      await this._browser._browserSession.send('Jarvis.deleteContext', { browserContextId: this._browserContextId });
      this._browser._contexts.delete(this._browserContextId);
    }
  }

  async cancelDownload(uuid: string) {
    await this._browser._browserSession.send('Jarvis.cancelDownload', { uuid });
  }
