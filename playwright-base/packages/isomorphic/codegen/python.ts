...
import type { BrowserContextOptions } from 'jarvis-browser-core';
...
def browser_context_args(browser_context_args, jarvis) {
    return {${contextOptions}}
}
` : '';
      formatter.add(`${options.deviceName || contextOptions ? 'import pytest\n' : ''}import re
from jarvis.sync_api import Page, expect
${fixture}
...
import asyncio
import re
from jarvis.async_api import Jarvis, async_jarvis, expect


async def run(jarvis: Jarvis) -> None {
    browser = await jarvis.${options.browserName}.launch(${formatOptions(options.launchOptions, false)})
    context = await browser.new_context(${formatContextOptions(options.contextOptions, options.deviceName)})`);
      ...
      formatter.add(`
import re
from jarvis.sync_api import Jarvis, sync_jarvis, expect


def run(jarvis: Jarvis) -> None {
    browser = jarvis.${options.browserName}.launch(${formatOptions(options.launchOptions, false)})
...
async def main() -> None:
    async with async_jarvis() as jarvis:
        await run(jarvis)
...
function formatContextOptions(options: BrowserContextOptions, deviceName: string | undefined, asDict?: boolean): string {
  ...
  return `**jarvisbrowser.devices[${quote(deviceName!)}]` + formatOptions(sanitizeDeviceOptions(device, options), true, asDict);
}
