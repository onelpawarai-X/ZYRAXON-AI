...
1. **Udaljena konfiguracija** (od `.well-known/zyraxon`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/zyraxon/zyraxon.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`ZYRAXON_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`zyraxon.json` u projektu) - postavke specifične za projekat
5. **`.zyraxon` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`ZYRAXON_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.zyraxon` i `~/.config/zyraxon` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/zyraxon` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu ZYRAXON konfiguraciju u `~/.config/zyraxon/zyraxon.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/zyraxon/tui.json`.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `ZYRAXON_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `ZYRAXON_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.zyraxon` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.zyraxon`, tako da **može nadjačati** njihove postavke.
...
Koristite `ZYRAXON_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.
...
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `zyraxon.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/zyraxon/agents/` ili `.zyraxon/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/zyraxon/commands/` ili `.zyraxon/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.zyraxon/plugins/` ili `~/.config/zyraxon/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
