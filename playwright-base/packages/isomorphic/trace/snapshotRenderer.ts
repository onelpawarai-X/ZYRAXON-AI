  ...
  render(): RenderedFrameSnapshot {
    ...
    const visit = (n: NodeSnapshot, snapshotIndex: number, parentTag: string | undefined, parentAttrs: [string, string][] | undefined) => {
      ...
      if (isSubtreeReferenceSnapshot(n)) {
        ...
        const kCurrentSrcAttribute = '__jarvis_current_src__';
        ...
        for (const [attr, value] of attrs) {
          ...
          if (isFrame && (attr.toLowerCase() === 'srcdoc' || attr.toLowerCase() === 'sandbox')) {
            ...
            attrName = '__jarvis_' + attr.toLowerCase() + '__';
          }
          if (upperName === 'OBJECT' && attr.toLowerCase() === 'data')
            attrName = '__jarvis_data__';
...
declare global {
  interface Window {
    __jarvis_frame_bounding_rects__: FrameBoundingRectsInfo;
  }
}

function snapshotScript(viewport: ViewportSize, ...targetIds: (string | undefined)[]) {
  function applyJarvisAttributes(blankSnapshotUrl: string, viewport: ViewportSize, ...targetIds: (string | undefined)[]) {
    ...
    // Before rendering each iframe, its parent extracts the '__jarvis_canvas_render_info__' attribute
    ...
    win['__jarvis_frame_bounding_rects__'] = frameBoundingRectsInfo;
    ...
    const visit = (root: Document | ShadowRoot) => {
      ...
      for (const e of root.querySelectorAll(`[__jarvis_scroll_top_]`))
        scrollTops.push(e);
      for (const e of root.querySelectorAll(`[__jarvis_scroll_left_]`))
        scrollLefts.push(e);

      for (const element of root.querySelectorAll(`[__jarvis_value_]`)) {
        const inputElement = element as HTMLInputElement | HTMLTextAreaElement;
        if (inputElement.type !== 'file')
          inputElement.value = inputElement.getAttribute('__jarvis_value_')!;
        element.removeAttribute('__jarvis_value_');
      }
      for (const element of root.querySelectorAll(`[__jarvis_checked_]`)) {
        (element as HTMLInputElement).checked = element.getAttribute('__jarvis_checked_') === 'true';
        element.removeAttribute('__jarvis_checked_');
      }
      for (const element of root.querySelectorAll(`[__jarvis_selected_]`)) {
        (element as HTMLOptionElement).selected = element.getAttribute('__jarvis_selected_') === 'true';
        element.removeAttribute('__jarvis_selected_');
      }
      for (const element of root.querySelectorAll(`[__jarvis_popover_open_]`)) {
        try {
          (element as HTMLElement).showPopover();
        } catch {
        }
        element.removeAttribute('__jarvis_popover_open_');
      }
      for (const element of root.querySelectorAll(`[__jarvis_dialog_open_]`)) {
        try {
          if (element.getAttribute('__jarvis_dialog_open_') === 'modal')
            (element as HTMLDialogElement).showModal();
          else
            (element as HTMLDialogElement).show();
        } catch {
        }
        element.removeAttribute('__jarvis_dialog_open_');
      }

      // Highlight targets marked by the current snapshotter, which sets `__jarvis_target__`
      ...
      for (const target of root.querySelectorAll(`[__jarvis_target__=""]`))
        highlightTarget(target);
      for (const targetId of targetIds) {
        if (!targetId)
          continue;
        for (const target of root.querySelectorAll(`[__jarvis_target__="${targetId}"]`))
          highlightTarget(target);
      }

      for (const iframe of root.querySelectorAll('iframe, frame')) {
        const boundingRectJson = iframe.getAttribute('__jarvis_bounding_rect__');
        iframe.removeAttribute('__jarvis_bounding_rect__');
        ...
        const src = iframe.getAttribute('__jarvis_src__');
        if (!src) {
      ...
      {
        const body = root.querySelector(`body[__jarvis_custom_elements__]`);
        if (body && win.customElements) {
          const customElements = (body.getAttribute('__jarvis_custom_elements__') || '').split(',');
          for (const elementName of customElements)
            win.customElements.define(elementName, class extends HTMLElement {});
        }
      }

      for (const element of root.querySelectorAll(`template[__jarvis_shadow_root_]`)) {
      ...
      if ('adoptedStyleSheets' in (root as any)) {
        ...
        for (const element of root.querySelectorAll(`template[__jarvis_style_sheet_]`)) {
          const template = element as HTMLTemplateElement;
          const sheet = new CSSStyleSheet();
          (sheet as any).replaceSync(template.getAttribute('__jarvis_style_sheet_'));
    ...
    const onLoad = () => {
      ...
      for (const element of scrollTops) {
        element.scrollTop = +element.getAttribute('__jarvis_scroll_top_')!;
        element.removeAttribute('__jarvis_scroll_top_');
        if (frameBoundingRectsInfo.frames.has(element))
          frameBoundingRectsInfo.frames.get(element)!.scrollTop = element.scrollTop;
      }
      for (const element of scrollLefts) {
        element.scrollLeft = +element.getAttribute('__jarvis_scroll_left_')!;
        element.removeAttribute('__jarvis_scroll_left_');
        if (frameBoundingRectsInfo.frames.has(element))
      ...
      if (canvasElements.length > 0) {
        ...
        img.onload = () => {
          for (const canvas of canvasElements) {
            ...
            const boundingRectAttribute = canvas.getAttribute('__jarvis_bounding_rect__');
            canvas.removeAttribute('__jarvis_bounding_rect__');
            ...
            while (currWindow !== topSnapshotWindow) {
              ...
              const iframeInfo = currWindow['__jarvis_frame_bounding_rects__']?.frames.get(iframe);
            ...
            const { width, height } = topSnapshotWindow['__jarvis_frame_bounding_rects__'].viewport;
            ...
            if (fullyUncaptured) {
              canvas.title = `Jarvis couldn't capture canvas contents because it's located outside the viewport.`;
              continue;
            }
            ...
            if (shouldPopulateCanvasFromScreenshot) {
              ...
              if (partiallyUncaptured)
                canvas.title = `Jarvis couldn't capture full canvas contents because it's located partially outside the viewport.`;
              else
        ...
        img.onerror = () => {
          for (const canvas of canvasElements) {
            const context = canvas.getContext('2d')!;
            drawCheckerboard(context, canvas);
            canvas.title = `Jarvis couldn't show canvas contents because the screenshot failed to load.`;
          }
        };
  ...
  return `\n(${applyJarvisAttributes.toString()})(${safe(blankSnapshotUrl)},${safe(viewport)}${targetIds.map(id => `, ${safe(String(id))}`).join('')})`;
}
...
const kLegacyBlobPrefix = 'http://jarvis.bloburl/#';
