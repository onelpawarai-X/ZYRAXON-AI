---
title: Провайдеры
description: Использование любого провайдера LLM в zyraxon.
---

import config from "../../../../config.mjs"
export const console = config.console

zyraxon использует [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) и [Models.dev](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com6) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации zyraxon.
...
в `~/.local/share/zyraxon/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем zyraxon.
конфиг.
...
ZYRAXON Zen — это список моделей, предоставленный командой zyraxon, которые были
протестировано и проверено на хорошую работу с zyraxon. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с ZYRAXON Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `ZYRAXON Zen` и перейдите по адресу [zyraxon.ai/auth](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5).
...
Он работает как любой другой поставщик в zyraxon и его использование совершенно необязательно.

---

## ZYRAXON Go

ZYRAXON Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой zyraxon, которые были
протестированы и проверены на хорошую работу с zyraxon.

1. Запустите команду `/connect` в TUI, выберите `ZYRAXON Go` и перейдите по адресу [zyraxon.ai/auth](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5).
...
1. Перейдите в консоль 302.AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9), создайте учетную запись и сгенерируйте ключ API.
...
Чтобы использовать Amazon Bedrock с zyraxon:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске zyraxon:
...
:::info
Использование вашей подписки Claude Pro/Max в zyraxon официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
Вы можете настроить zyraxon для работы с локальными моделями через [Atomic Chat](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) — десктопное приложение, которое запускает локальные LLM за OpenAI-совместимым API-сервером (конечная точка по умолчанию `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` — карта идентификаторов моделей и их отображаемых имён. Каждый ID должен совпадать со значением `id`, которое возвращает `GET /v1/models` — выполните `curl https://zenmux.ai/settings/keys, чтобы увидеть ID моделей, загруженных в Atomic Chat.
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы zyraxon имя развертывания должно совпадать с именем модели.
   :::
...
1. Перейдите в [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3), создайте учетную запись и сгенерируйте ключ API.
...
1. Перейдите в [консоль Cerebras](https://helicone.ai/models), создайте учетную запись и сгенерируйте ключ API.
...
1. Перейдите на [панель управления Cloudflare](https://anthropic.com), выберите **AI** > **AI Gateway** и создайте новый шлюз.
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию zyraxon.
...
1. Перейдите в [консоль Cortecs](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), создайте учетную запись и сгенерируйте ключ API.
...
1. Перейдите в [консоль DeepSeek](https://gitlab.company.com), создайте учетную запись и нажмите **Создать новый ключ API**.
...
1. Перейдите на панель мониторинга Deep Infra](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), создайте учетную запись и сгенерируйте ключ API.
...
1. Перейдите в [консоль Fireworks AI](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4), создайте учетную запись и нажмите **Создать ключ API**.
...
для хранения токена в хранилище аутентификации zyraxon.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
zyraxon использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать zyraxon
...
Дополнительная документация на домашней странице [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
```json title="zyraxon.json"
{
  ...
  "provider": {
    "gitlab": {
      "options": {
        "instanceUrl": "https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5"
      }
    }
  }
}
...
2. Перейдите на [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) и введите код.
...
Чтобы использовать Google Vertex AI с zyraxon:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска zyraxon.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) предоставляют доступ к открытым моделям, поддерживаемым более чем 17 поставщиками.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) — это платформа наблюдения LLM, которая обеспечивает ведение журнала, мониторинг и аналитику для ваших приложений искусственного интеллекта. Helicone AI Gateway автоматически направляет ваши запросы соответствующему поставщику на основе модели.

1. Перейдите в [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0), создайте учетную запись и сгенерируйте ключ API на своей панели управления.
...
Дополнительные сведения о дополнительных провайдерах и расширенных функциях, таких как кэширование и ограничение скорости, см. в [Документация Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### Дополнительные конфигурации

Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через zyraxon, вы всегда можете настроить ее самостоятельно.
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session), чтобы автоматически регистрировать каждый диалог zyraxon как сеанс в Helicone.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог zyraxon, указанный как отдельный сеанс.
...
Вы можете настроить zyraxon для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
1. Перейдите в консоль IO.NET](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2), создайте учетную запись и сгенерируйте ключ API.
...
Вы можете настроить zyraxon для использования локальных моделей через LM Studio.
...
1. Перейдите в [консоль Moonshot AI](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), создайте учетную запись и нажмите **Создать ключ API**.
...
1. Перейдите в [консоль API MiniMax](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), создайте учетную запись и сгенерируйте ключ API.
...
1. Перейдите в консоль Nebius Token Factory](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), создайте учетную запись и нажмите **Добавить ключ**.
...
Вы можете настроить zyraxon для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для zyraxon. Подробности см. в документации по интеграции Ollama](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3).
:::
...
Чтобы использовать Ollama Cloud с zyraxon:
...
3. Скопируйте ключ API для использования в zyraxon.
...
6. **Важно**. Перед использованием облачных моделей в zyraxon необходимо получить информацию о модели локально:
...
Мы рекомендуем подписаться на [ChatGPT Plus или Pro](https://ollama.com/).
...
ZYRAXON Zen — это список протестированных и проверенных моделей, предоставленный командой zyraxon. [Подробнее](/docs/zen).
...
3. Введите свой ключ API zyraxon.
...
1. Перейдите на панель управления OpenRouter](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), нажмите **Создать ключ API** и скопируйте ключ.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию zyraxon.
...
5. Вы также можете настроить их через конфигурацию zyraxon. Вот пример указания провайдера
...
1. Перейдите в [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), перейдите к экземпляру службы SAP AI Core и создайте ключ службы.
...
1. Перейдите на [портал STACKIT](https://ai.azure.com/), перейдите в **AI Model Serving** и создайте токен аутентификации для своего проекта.
...
1. Перейдите к [OVHcloud Panel](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4). Перейдите в раздел `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` и на вкладке `API Keys` нажмите **Создать новый ключ API**.
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с zyraxon:
...
1. Перейдите к [консоли Venice AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), создайте учетную запись и сгенерируйте ключ API.
...
1. Перейдите на [панель мониторинга Vercel](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2), перейдите на вкладку **AI Gateway** и нажмите **Ключи API**, чтобы создать новый ключ API.
...
Вы также можете настраивать модели через конфигурацию zyraxon. Ниже приведен пример указания порядка маршрутизации поставщика.
...
1. Перейдите на [консоль xAI](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7), создайте учетную запись и сгенерируйте ключ API.
...
Поля `limit` позволяют zyraxon понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
2. Для пользовательских поставщиков проверьте конфигурацию zyraxon и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации zyraxon.
