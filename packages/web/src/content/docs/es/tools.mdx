...
:::note
Esta herramienta solo está disponible cuando `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (o `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor ZYRAXON o cuando la variable de entorno `ZYRAXON_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).
