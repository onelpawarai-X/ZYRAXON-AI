...
/**
 * WebSocket server that bridges Jarvis MCP and Chrome Extension.
 *
 * Endpoints:
 * - /cdp/guid - Full CDP interface for Jarvis MCP
...
import { findJarvisExtensionProfile, jarvisExtensionId } from '../utils/extension';
...
export class CDPRelayServer {
    ...
    this._wsServer = new WSServer({
      ...
      onConnection: (request, url, ws) => {
        ...
        if (url.pathname === this._cdpPath)
          this._handleJarvisConnection(ws);
        else
  ...
  private async _openConnectPageInBrowser(clientName: string) {
    ...
    const url = new URL(`chrome-extension://${jarvisExtensionId}/connect.html`);
    ...
    const profileDirectory = userDataDir ? await findJarvisExtensionProfile(userDataDir) : undefined;
    if (profileDirectory)
  ...
  private _handleJarvisConnection(ws: WebSocket): void {
    if (!this._extensionConnection) {
      debugLogger('Rejecting Jarvis connection: extension not connected');
      ws.close(1000, 'Extension not connected');
      return;
    }
    if (this._cdpConnection) {
      debugLogger('Rejecting second Jarvis connection');
    ...
    ws.on('message', async data => {
      try {
        await this._handleJarvisMessage(JSON.parse(data.toString()));
      } catch (error: any) {
        debugLogger(`Error while handling Jarvis message\n${data.toString()}\n`, error);
      }
    });
    ws.on('close', () => {
      this._closeExtensionConnection('Jarvis client disconnected');
      debugLogger('Jarvis WebSocket closed');
    });
    ws.on('error', error => {
      debugLogger('Jarvis WebSocket error:', error);
    });
    debugLogger('Jarvis MCP connected');
  }
  ...
  private async _handleJarvisMessage(message: CDPCommand): Promise<void> {
    debugLogger('← Jarvis:', `${message.method} (id=${message.id})`);
  ...
  private _sendToCDPClient(message: CDPResponse): void {
    debugLogger('→ Jarvis:', `${message.method ?? `response(id=${message.id})`}`);
