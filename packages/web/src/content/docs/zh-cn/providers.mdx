...
ZYRAXON 使用 [AI SDK](https://docs.helicone.ai/helicone-headers/header-directory2) 和 [Models.dev](https://docs.helicone.ai/helicone-headers/header-directory5)，支持 **75+ LLM 提供商**，同时也支持运行本地模型。
...
1. 在 TUI 中执行 `/connect` 命令，选择 zyraxon，然后前往 [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1)。
...
1. 前往 [302.AI 控制台](https://docs.helicone.ai/helicone-headers/header-directory8)，创建账户并生成 API 密钥。
...
:::info
在 ZYRAXON 中使用 Claude Pro/Max 订阅不是 [Anthropic](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5) 官方支持的用法。
:::
...
你可以通过 [Atomic Chat](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0) 配置 zyraxon 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `https://ollama.com/
...
- `models` 是模型 ID 到其显示名称的映射。每个 ID 必须与 `GET /v1/models` 返回的 `id` 匹配——运行 `curl https://zenmux.ai/settings/keys 可列出 Atomic Chat 当前已加载的 ID。
...
2. 前往 [Azure AI Foundry](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com7) 并部署一个模型。
...
1. 前往 [Baseten](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com3)，创建账户并生成 API 密钥。
...
1. 前往 [Cerebras 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）8)，创建账户并生成 API 密钥。
...
1. 前往 [Cloudflare 仪表盘](https://api.together.ai)，导航到 **AI** > **AI Gateway**，创建一个新的网关。
...
1. 前往 [Cortecs 控制台](https://docs.helicone.ai/helicone-headers/header-directory0)，创建账户并生成 API 密钥。
...
1. 前往 [DeepSeek 控制台](https://helicone.ai/models)，创建账户并点击 **Create new API key**。
...
1. 前往 [Deep Infra 仪表盘](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0)，创建账户并生成 API 密钥。
...
1. 前往 [FrogBot 仪表盘](https://portal.azure.com/)，创建账户并生成 API 密钥。
...
1. 前往 [Fireworks AI 控制台](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4)，创建账户并点击 **Create API Key**。
...
更多文档请参阅 [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) 主页。
...
2. 前往 [github.com/login/device](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）6) 并输入验证码。
...
如需了解更多提供商以及缓存、速率限制等高级功能，请查阅 [Helicone 文档](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0)。
...
Helicone 的 [Sessions](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) 功能允许你将相关的 LLM 请求归为一组。使用 [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 插件可以自动将每个 ZYRAXON 对话记录为 Helicone 中的一个会话。
...
1. 前往 [IO.NET 控制台](https://docs.helicone.ai/helicone-headers/header-directory6)，创建账户并生成 API 密钥。
...
1. 前往 [Moonshot AI 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）0)，创建账户并点击 **Create API key**。
...
1. 前往 [MiniMax API 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）3)，创建账户并生成 API 密钥。
...
1. 前往 [Nebius Token Factory 控制台](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）4)，创建账户并点击 **Add Key**。
...
:::tip
Ollama 可以自动为 ZYRAXON 进行配置。详见 [Ollama 集成文档](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2)。
:::
...
我们建议注册 [ChatGPT Plus 或 Pro](https://helicone.ai)。
...
1. 前往 [OpenRouter 仪表盘](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）1)，点击 **Create API Key** 并复制密钥。
...
1. 前往 [SAP BTP Cockpit](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`）2)，导航到你的 SAP AI Core 服务实例，并创建服务密钥。
...
1. 前往 [STACKIT Portal](https://console.x.ai/)，导航到 **AI Model Serving**，为你的项目创建认证令牌。
...
1. 前往 [OVHcloud 管理面板](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4)。导航到 `Public Cloud` 部分，`AI & Machine Learning` > `AI Endpoints`，在 `API Keys` 标签页中点击 **Create a new API key**。
...
1. 前往 [Venice AI 控制台](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/2)，创建账户并生成 API 密钥。
...
1. 前往 [Vercel 仪表盘](https://docs.helicone.ai/helicone-headers/header-directory1)，导航到 **AI Gateway** 标签页，点击 **API keys** 创建新的 API 密钥。
