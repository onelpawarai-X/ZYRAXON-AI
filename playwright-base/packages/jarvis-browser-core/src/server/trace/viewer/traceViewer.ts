...
import { createJarvis } from '../../jarvis';
...
export async function openTraceViewerApp(url: string, browserName: string, options?: TraceViewerAppOptions): Promise<Page> {
  const traceViewerJarvis = createJarvis({ sdkLanguage: 'javascript', isInternalJarvis: true });
  const traceViewerBrowser = isUnderTest() ? 'chromium' : browserName;

  const { context, page } = await launchApp(traceViewerJarvis[traceViewerBrowser as 'chromium'], {
    sdkLanguage: traceViewerJarvis.options.sdkLanguage,
