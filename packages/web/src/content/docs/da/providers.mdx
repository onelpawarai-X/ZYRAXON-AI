...
ZYRAXON bruger [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) og [Models.dev](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.
...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/zyraxon/auth.json`.
...
1. Kør kommandoen `/connect` i TUI, vælg zyraxon og gå til [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
1. Gå til [302.AI-konsollen](https://developers.cloudflare.com/ai-gateway/features/unified-billing/3), opret en konto og generer en API-nøgle.
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører zyraxon:
...
:::info
At bruge dit Claude Pro/Max-abonnement i ZYRAXON understøttes ikke officielt af [Anthropic](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7).
:::
...
Du kan konfigurere zyraxon til at bruge lokale modeller via [Atomic Chat](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` er en afbildning af model-ID'er til deres viste navne. Hvert ID skal matche det `id`, der returneres af `GET /v1/models` — kør `curl https://zenmux.ai/settings/keys for at liste ID'erne, der i øjeblikket er indlæst i Atomic Chat.
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at zyraxon skal fungere korrekt.
   :::
...
1. Gå til [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4), opret en konto og generer en API-nøgle.
...
1. Gå til [Cerebras-konsollen](https://helicone.ai/models), opret en konto og generer en API-nøgle.
...
1. Gå til [Cloudflare-dashboardet](https://zyraxon.ai/zen), naviger til **AI** > **AI Gateway**, og opret en ny gateway.
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem zyraxon-konfigurationen.
...
1. Gå til [Cortecs-konsollen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2), opret en konto og generer en API-nøgle.
...
1. Gå til [DeepSeek-konsollen](https://gitlab.company.com), opret en konto og klik på **Create API Key**.
...
1. Gå til [Deep Infra-dashboardet](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), opret en konto og generer en API-nøgle.
...
1. Gå til [Fireworks AI-konsollen](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4), opret en konto og klik på **Create API Key**.
...
gemme tokenet i zyraxon auth-lagring.
:::
...
Mere dokumentation på [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) hjemmesiden.
...
2. Naviger til [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) og indtast koden.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) giver adgang til åbne modeller som støttes af 17+ udbydere.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1) er en LLM observerbarhedsplatform som giver logning, overvågning og analyser for dine AI-applikationer. Helicone AI Gateway ruter dine anmodninger til den rigtige udbyder automatisk baseret på modellen.

1. Gå til [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), opret en konto og generer en API-nøgle fra dashboardet.
...
For flere udbydere og avancerede funktioner som caching og hastighedsbegrænsning, tjek [Helicone-dokumentationen](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### Valgfrie konfigurationer

I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem zyraxon, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver ZYRAXON-samtale som en session i Helicone.
...
Du kan konfigurere zyraxon til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
1. Gå til [IO.NET-konsollen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), opret en konto og generer en API-nøgle.
...
Du kan konfigurere zyraxon til at bruge lokale modeller gennem LM Studio.
...
1. Gå til [Moonshot AI-konsollen](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), opret en konto og klik på **Create API Key**.
...
1. Gå til [MiniMax API-konsollen](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), opret en konto og generer en API-nøgle.
...
1. Gå til [Nebius Token Factory-konsollen](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), opret en konto og klik på **Add Key**.
...
Du kan konfigurere zyraxon til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for ZYRAXON. Se [Ollama-integrationsdokumenterne](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3) for detaljer.
:::
...
1. Gå til [https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) og log på eller opret en konto.
...
Vi anbefaler at du registrerer dig for [ChatGPT Plus eller Pro](https://ai.azure.com/).
...
1. Gå til [OpenRouter-dashboardet](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), klik på **Create API Key**, og kopier nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem zyraxon-konfigurationen.
...
5. Du kan også tilpasse dem gennem zyraxon-konfigurationen. Her er et eksempel på at specificere en udbyder
...
1. Gå til din [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), naviger til din SAP AI kerne-tjenesteinstans, og opret en tjenestenøgle.
...
1. Gå til [STACKIT Portal](https://console.x.ai/), naviger til **AI Model Serving**, og opret en auth-token til dit projekt.
...
1. Gå til [OVHcloud-panelet](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5). Naviger til `Public Cloud`-delen, `AI & Machine Learning` > `AI Endpoints` og i `API Keys`-fanen klikker du på **Opret en ny API-nøgle**.
...
1. Gå til [Together AI-konsollen](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3), opret en konto og klik på **Add Key**.
...
1. Gå til [Venice AI-konsollen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9), opret en konto og generer en API-nøgle.
...
1. Gå til [Vercel dashboard](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), naviger til fanen **AI Gateway**, og klik på **API Keys** for at oprette en ny API-nøgle.
...
Du kan også tilpasse modeller gennem zyraxon-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med zyraxon. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
2. For tilpassede udbydere, tjek zyraxon-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i zyraxon-konfigurationen.
