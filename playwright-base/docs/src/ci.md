...
Jarvis Browser Tests can be executed in CI environments. We have created sample
...
jobs:
  test:
    ...
    - name: Install Playwright Browsers
      run: npx playwright install --with-deps
    - name: Run changed Jarvis Browser Tests
      run: npx Jarvis Browser Test --only-changed=origin/$GITHUB_BASE_REF
...
For running the Jarvis Browser Tests use this pipeline task:
...
- script: npx Jarvis Browser Test
  displayName: 'Run Jarvis Browser Tests'
  env:
    CI: 'true'
```
...
This will make the pipeline run fail if any of the Jarvis Browser Tests fails.
...
- script: npx playwright install --with-deps
  displayName: 'Install Playwright browsers'
- script: npx Jarvis Browser Test --project=$(project) --shard=$(shard)
  displayName: 'Run Jarvis Browser Tests'
  env:
    CI: 'true'
```
...
Sharding in CircleCI is indexed with 0 which means that you will need to override the default parallelism ENV VARS. The following example demonstrates how to run Playwright with a CircleCI Parallelism of 4 by adding 1 to the `CIRCLE_NODE_INDEX` to pass into the `--shard` cli arg.
  ...
  ```yml
    playwright-job-name:
      ...
      parallelism: 4
      steps:
        - run: SHARD="$((${CIRCLE_NODE_INDEX}+1))"; npx Jarvis Browser Test --shard=${SHARD}/${CIRCLE_NODE_TOTAL}
  ```
...
Bitbucket Pipelines can use public [Docker images as build environments](https://confluence.atlassian.com/bitbucket/use-docker-images-as-build-environments-792298897.html). To run Jarvis Browser Tests on Bitbucket, use our public Docker image ([see Dockerfile](./docker.md)).
...
To run Jarvis Browser Tests on GitLab, use our public Docker image ([see Dockerfile](./docker.md)).
...
tests:
  ...
  script:
    - npm ci
    - npx Jarvis Browser Test --shard=$CI_NODE_INDEX/$CI_NODE_TOTAL
```
...
tests:
  ...
  script:
    - npm ci
    - npx Jarvis Browser Test --project=$PROJECT --shard=$SHARD
```
### Google Cloud Build
* langs: js

To run Jarvis Browser Tests on Google Cloud Build, use our public Docker image ([see Dockerfile](./docker.md)).
...
To run Jarvis Browser Tests on Drone, use our public Docker image ([see Dockerfile](./docker.md)).
...
```bash js
DEBUG=pw:browser npx Jarvis Browser Test
```
```bash python
...
```bash js
xvfb-run npx Jarvis Browser Test
```
```bash python
xvfb-run pytest
```
