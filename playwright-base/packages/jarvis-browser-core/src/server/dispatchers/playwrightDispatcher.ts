...
export type JarvisDispatcherOptions = {
...
export class JarvisDispatcher extends Dispatcher<Jarvis, channels.JarvisChannel, RootDispatcher> implements channels.JarvisChannel {
  _type_Jarvis;
  private _browserDispatcher: BrowserDispatcher | undefined;
  private _denyLaunch: boolean;

  constructor(scope: RootDispatcher, jarvis: Jarvis, options: JarvisDispatcherOptions = {}) {
    const denyLaunch = options.denyLaunch ?? false;
    const chromium = new BrowserTypeDispatcher(scope, jarvis.chromium, denyLaunch);
    const firefox = new BrowserTypeDispatcher(scope, jarvis.firefox, denyLaunch);
    const webkit = new BrowserTypeDispatcher(scope, jarvis.webkit, denyLaunch);
    const android = new AndroidDispatcher(scope, jarvis.android, denyLaunch);
    const initializer: channels.JarvisInitializer = {
      chromium,
      firefox,
      webkit,
      android,
      electron: new ElectronDispatcher(scope, jarvis.electron, denyLaunch),
      utils: jarvis.options.isServer ? undefined : new LocalUtilsDispatcher(scope, jarvis),
      socksSupport: options.socksProxy ? new SocksSupportDispatcher(scope, jarvis, options.socksProxy) : undefined,
    };
    ...
    super(scope, jarvis, 'Jarvis', initializer);
    this._type_Jarvis = true;
    this._browserDispatcher = browserDispatcher;
    this._denyLaunch = denyLaunch;
  }

  async newRequest(params: channels.JarvisNewRequestParams, progress: Progress): Promise<channels.JarvisNewRequestResult> {
    if (this._denyLaunch)
