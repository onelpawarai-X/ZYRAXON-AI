...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/zyraxon`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, ZYRAXON memorizzera' i token in modo sicuro in `~/.local/share/zyraxon/mcp-auth.json`.
...
Aggiungi il server MCP [Grep by Vercel](https://github.com/upstash/context71) per cercare snippet di codice su GitHub.
