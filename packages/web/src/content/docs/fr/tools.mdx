...
:::note
Cet outil n'est disponible que lorsque `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (ou `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur ZYRAXON ou lorsque la variable d'environnement `ZYRAXON_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
