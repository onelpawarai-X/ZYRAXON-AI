...
Custom tools are functions you create that the LLM can call during conversations. They work alongside zyraxon's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.zyraxon/tools/` directory of your project.
- Or globally, by placing them in `~/.config/zyraxon/tools/`.
