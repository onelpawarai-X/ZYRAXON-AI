...
| `defineConfig` from `@playwright/experimental-ct-react` | Plain `defineConfig` from `@jarvis-browser/test`, with `baseURL` = gallery URL, `serviceWorkers: 'block'`, `reuseContext: true` (see `SKILL.md`) |
...
3. Rewrite the spec: import `test`/`expect` from `@jarvis-browser/test`; `mount(<X a={1}/>)`
