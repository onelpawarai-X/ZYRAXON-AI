...
Organizations can provide default MCP servers via their `.well-known/zyraxon` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
The `mcp auth` command will open your browser for authorization. After you authorize, ZYRAXON will store the tokens securely in `~/.local/share/zyraxon/mcp-auth.json`.
...
Add the [Grep by Vercel](https://github.com/upstash/context71) MCP server to search through code snippets on GitHub.
