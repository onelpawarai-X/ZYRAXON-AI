...
ZYRAXON koristi [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) i [Models.dev](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.
...
u `~/.local/share/zyraxon/auth.json`.
...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite zyraxon i idite na [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
1. Idite na [302.AI konzolu](https://developers.cloudflare.com/ai-gateway/features/unified-billing/3), kreirajte račun i generirajte API ključ.
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete zyraxon:
...
:::info
[Anthropic] (https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u ZYRAXON.
:::
...
Možete konfigurirati zyraxon za korištenje lokalnih modela preko [Atomic Chata](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` je mapa ID-ova modela u njihova prikazana imena. Svaki ID mora odgovarati `id` vrijednosti koju vraća `GET /v1/models` — pokrenite `curl https://zenmux.ai/settings/keys da vidite ID-ove trenutno učitane u Atomic Chat.
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi zyraxon ispravno radio.
   :::
...
1. Idite na [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4), kreirajte račun i generirajte API ključ.
...
1. Idite na [Cerebras konzolu](https://helicone.ai/models), kreirajte račun i generirajte API ključ.
...
1. Idite na [Cloudflare kontrolnu tablu](https://zyraxon.ai/zen), idite na **AI** > **AI Gateway** i kreirajte novi pristupnik.
...
Također možete dodati modele kroz svoju zyraxon konfiguraciju.
...
1. Idite na [Cortecs konzolu](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2), kreirajte račun i generirajte API ključ.
...
1. Idite na [DeepSeek konzolu](https://gitlab.company.com), kreirajte nalog i kliknite na **Kreiraj novi API ključ**.
...
1. Idite na [Deep Infra kontrolnu tablu](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), kreirajte nalog i generišite API ključ.
...
1. Idite na [Fireworks AI konzolu](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4), kreirajte račun i kliknite na **Kreiraj API ključ**.
...
da pohrani token u zyraxon auth memoriju.
:::
...
Više dokumentacije na početnoj stranici [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
Da biste koristili svoju GitHub Copilot pretplatu s zyraxon:
...
2. Idite na [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) i unesite kod.
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće zyraxon.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) omogućava pristup otvorenim modelima koje podržava 17+ provajdera.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1) je platforma za praćenje LLM koja pruža evidenciju, praćenje i analitiku za vaše AI aplikacije. Helicone AI Gateway automatski usmjerava vaše zahtjeve do odgovarajućeg provajdera na osnovu modela.

1. Idite na [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), kreirajte račun i generirajte API ključ sa svoje kontrolne table.
...
Za više provajdera i napredne funkcije kao što su keširanje i ograničavanje brzine, provjerite [Helicone dokumentaciju](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### Opcione konfiguracije

U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem zyraxona, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki ZYRAXON razgovor kao sesiju u Helicone-u.
...
Možete konfigurirati zyraxon za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
1. Idite na [IO.NET konzolu](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), kreirajte račun i generirajte API ključ.
...
Možete konfigurirati zyraxon za korištenje lokalnih modela preko LM Studio.
...
1. Idite na [Moonshot AI konzolu](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), kreirajte nalog i kliknite na **Kreiraj API ključ**.
...
1. Prijeđite na [MiniMax API konzolu](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), kreirajte račun i generirajte API ključ.
...
1. Idite na [Nebius Token Factory konzolu](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), kreirajte nalog i kliknite na **Dodaj ključ**.
...
Možete konfigurirati zyraxon za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za ZYRAXON. Pogledajte [Ollama integracijske dokumente](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3) za detalje.
:::
...
1. Idite na [https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) i prijavite se ili kreirajte račun.
...
Preporučujemo da se prijavite za [ChatGPT Plus ili Pro](https://ai.azure.com/).
...
1. Idite na [OpenRouter nadzornu ploču](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), kliknite na **Kreiraj API ključ** i kopirajte ključ.
...
Također možete dodati dodatne modele putem vaše zyraxon konfiguracije.
...
5. Također ih možete prilagoditi putem vaše zyraxon konfiguracije. Evo primjera navođenja provajdera
...
1. Idite na vaš [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), idite na instancu usluge SAP AI Core i kreirajte servisni ključ.
...
1. Idite na [STACKIT Portal](https://console.x.ai/), idite na **AI Model Serving** i kreirajte token za autentifikaciju za svoj projekat.
...
1. Idite na [OVHcloud panel](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5). Idite do odjeljka `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` i na kartici `API Keys` kliknite na **Kreiraj novi API ključ**.
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Zyraxonom:
...
1. Idite na [Together AI console](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3), kreirajte nalog i kliknite na **Dodaj ključ**.
...
1. Idite na [Venice AI konzolu](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9), kreirajte račun i generirajte API ključ.
...
1. Idite na [Vercel kontrolnu tablu](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), idite na karticu **AI Gateway** i kliknite na **API ključevi** da kreirate novi API ključ.
...
Također možete prilagoditi modele kroz svoju zyraxon konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s zyraxon-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
2. Za prilagođene provajdere, provjerite ZYRAXON konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj zyraxon konfiguraciji.
