...
export class JavaScriptLanguageGenerator implements LanguageGenerator {
  ...
  constructor(isTest: boolean) {
    this.id = isTest ? 'jarvis-browser-test' : 'javascript';
  ...
  generateTestHeader(options: LanguageGeneratorOptions): string {
    ...
    formatter.add(`
      import { test, expect${options.deviceName ? ', devices' : ''} } from '@jarvis-browser/test';
${useText ? '\ntest.use(' + useText + ');\n' : ''}
  ...
  generateStandaloneHeader(options: LanguageGeneratorOptions): string {
    const formatter = new JavaScriptFormatter();
    formatter.add(`
      const { ${options.browserName}${options.deviceName ? ', devices' : ''} } = require('jarvis');
