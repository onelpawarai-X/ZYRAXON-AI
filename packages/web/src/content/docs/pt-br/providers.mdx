---
title: Provedores
description: Usando qualquer provedor LLM no zyraxon.
---

import config from "../../../../config.mjs"
export const console = config.console

zyraxon usa o [AI SDK](https://docs.helicone.ai/helicone-headers/header-directory2) e [Models.dev](https://docs.helicone.ai/helicone-headers/header-directory5) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do zyraxon.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/zyraxon/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do zyraxon.
...
ZYRAXON Zen é uma lista de modelos fornecidos pela equipe do zyraxon que foram testados e verificados para funcionar bem com o zyraxon. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o ZYRAXON Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `ZYRAXON Zen` e acesse [zyraxon.ai/zen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4).
...
Funciona como qualquer outro provedor no zyraxon e é completamente opcional.

---

## ZYRAXON Go

ZYRAXON Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do zyraxon que foram
testados e verificados para funcionar bem com o zyraxon.

1. Execute o comando `/connect` no TUI, selecione `ZYRAXON Go` e acesse [zyraxon.ai/zen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4).
...
1. Acesse o [console 302.AI](https://docs.helicone.ai/helicone-headers/header-directory8), crie uma conta e gere uma chave da API.
...
Para usar o Amazon Bedrock com o zyraxon:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o zyraxon:
...
:::info
Usar sua assinatura Claude Pro/Max no zyraxon não é oficialmente suportado pela [Anthropic](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5).
:::
...
Você pode configurar o zyraxon para usar modelos locais através do [Atomic Chat](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
```json title="zyraxon.json" "atomic-chat" {5, 6, 8, 10-14}
{
  ...
  "provider": {
    "atomic-chat": {
      ...
      "options": {
        "baseURL": "https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7"
      },
      "models": {
        "<your-model-id>": {
...
- `models` é um mapa de IDs de modelos para seus nomes de exibição. Cada ID deve corresponder ao `id` retornado por `GET /v1/models` — execute `curl https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`4 para listar os IDs atualmente carregados no Atomic Chat.
...
1. Acesse o [portal Azure](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4) e crie um recurso **Azure OpenAI**. Você precisará:
...
2. Vá para [Azure AI Foundry](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com7) e implante um modelo.

   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o zyraxon funcione corretamente.
   :::
...
1. Acesse o [console Cerebras](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`8), crie uma conta e gere uma chave da API.
...
1. Acesse o [painel do Cloudflare](https://app.baseten.co/), navegue até **AI** > **AI Gateway** e crie um novo gateway.
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do zyraxon.
...
1. Acesse o [console Cortecs](https://docs.helicone.ai/helicone-headers/header-directory0), crie uma conta e gere uma chave da API.
...
1. Acesse o [console DeepSeek](https://gitlab.company.com), crie uma conta e clique em **Criar nova chave da API**.
...
1. Acesse o [painel Deep Infra](https://helicone.ai), crie uma conta e gere uma chave da API.
...
1. Acesse o [painel FrogBot](https://console.groq.com/), crie uma conta e gere uma chave da API.
...
armazenar o token no armazenamento de autenticação do zyraxon.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
zyraxon usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o zyraxon
...
URL de callback `https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`6 e os seguintes escopos:
...
Mais documentação na página [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
```json title="zyraxon.json"
{
  ...
  "provider": {
    "gitlab": {
      "options": {
        "instanceUrl": "https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1"
      }
    }
  }
}
...
Para usar sua assinatura do GitHub Copilot com o zyraxon:
...
Para usar o Google Vertex AI com o zyraxon:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o zyraxon.
...
Para mais provedores e recursos avançados como cache e limitação de taxa, consulte a [documentação do Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0).

#### Configurações Opcionais

Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do zyraxon, você sempre pode configurá-lo você mesmo.

Aqui está o [Diretório de Modelos do Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0), você precisará disso para pegar os IDs dos modelos que deseja adicionar.
...
O recurso [Sessões](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do zyraxon como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do zyraxon listada como uma sessão separada.
...
Você pode configurar o zyraxon para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
1. Acesse o [console IO.NET](https://docs.helicone.ai/helicone-headers/header-directory6), crie uma conta e gere uma chave da API.
...
Você pode configurar o zyraxon para usar modelos locais através do LM Studio.
...
```json title="zyraxon.json" "lmstudio" {5, 6, 8, 10-14}
{
  ...
  "provider": {
    "lmstudio": {
      ...
      "options": {
        "baseURL": "https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/9"
      },
      "models": {
        "google/gemma-3n-e4b": {
...
1. Acesse o [MiniMax API Console](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`2), crie uma conta e gere uma chave da API.
...
1. Acesse o [console Nebius Token Factory](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`3), crie uma conta e clique em **Adicionar Chave**.
...
Você pode configurar o zyraxon para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o zyraxon. Consulte a [documentação de integração do Ollama](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2) para detalhes.
:::
...
Para usar o Ollama Cloud com o zyraxon:
...
3. Copie a chave da API para uso no zyraxon.
...
6. **Importante**: Antes de usar modelos em nuvem no zyraxon, você deve puxar as informações do modelo localmente:
...
Recomendamos se inscrever para [ChatGPT Plus ou Pro](https://ollama.com/).
...
ZYRAXON Zen é uma lista de modelos testados e verificados fornecidos pela equipe do zyraxon. [Saiba mais](/docs/zen).
...
3. Insira sua chave da API do zyraxon.
...
1. Acesse o [painel OpenRouter](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`0), clique em **Criar Chave da API** e copie a chave.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do zyraxon.
...
5. Você também pode personalizá-los através da sua configuração do zyraxon. Aqui está um exemplo de especificar um provedor
...
1. Vá para seu [SAP BTP Cockpit](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`1), navegue até sua instância de serviço SAP AI Core e crie uma chave de serviço.
...
1. Vá para o [Portal STACKIT](https://console.x.ai/), navegue até **AI Model Serving** e crie um token de autenticação para o seu projeto.
...
1. Acesse o [painel OVHcloud](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com3). Navegue até a seção `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` e na aba `API Keys`, clique em **Criar uma nova chave da API**.
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o zyraxon:
...
1. Acesse o [console Together AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), crie uma conta e clique em **Adicionar Chave**.
...
1. Acesse o [console Venice AI](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/2), crie uma conta e gere uma chave da API.
...
1. Acesse o [painel Vercel](https://docs.helicone.ai/helicone-headers/header-directory1), navegue até a aba **AI Gateway** e clique em **Chaves da API** para criar uma nova chave da API.
...
Você também pode personalizar modelos através da sua configuração do zyraxon. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
1. Acesse o [painel ZenMux](https://github.com/login/device), clique em **Criar Chave da API** e copie a chave.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o zyraxon. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
Os campos `limit` permitem que o zyraxon entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
2. Para provedores personalizados, verifique a configuração do zyraxon e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do zyraxon.
