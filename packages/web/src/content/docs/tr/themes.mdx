...
zyraxon ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak zyraxon kendi `zyraxon` temasını kullanır.
...
zyraxon birden fazla yerleşik temayla gelir.
...
- zyraxon'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
zyraxon, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/zyraxon/themes/*.json` veya `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.zyraxon/themes/*.json`
4. **Geçerli çalışma dizini** - `./.zyraxon/themes/*.json`
