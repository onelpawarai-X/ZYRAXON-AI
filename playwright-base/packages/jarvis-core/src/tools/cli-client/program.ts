...
export async function program(options?: { embedderVersion?: string}) {
  ...
  if (args.help || args.h || !commandName) {
    if (command) {
      ...
      const lines = ['jarvis-cli - run jarvis mcp commands from terminal'];
      if (process.env.CLAUDECODE || process.env.COPILOT_CLI)
        lines.push(`Agent skill: ${path.relative(process.cwd(), libPath('tools', 'skills', 'jarvis-cli', 'SKILL.md'))}`);
  ...
  switch (commandName) {
    ...
    case 'show': {
      ...
      // (via -s/--session or PLAYWRIGHT_CLI_SESSION). Bare `jarvis cli show`
