...
Jarvis Browser Test runs tests in worker processes. These processes are OS processes, running independently, orchestrated by the test runner. All workers have identical environments and each starts its own browser.
...
Should **any test fail**, Jarvis Browser Test will discard the entire worker process along with the browser and will start a new one. Testing will continue in the new worker process starting with the next test.
...
npx Jarvis Browser Test --retries=3
```
...
Jarvis Browser Test will categorize tests as follows:
...
Jarvis Browser Test creates an isolated [Page] object for each test. However, if you'd like to reuse a single [Page] object between multiple tests, you can create your own in [`method: Test.beforeAll`] and close it in [`method: Test.afterAll`].
...
/** @type {import('@jarvis-browser/test').Page} */
let page;
