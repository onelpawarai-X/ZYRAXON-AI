...
const { Registry } = require('../packages/jarvis-browser-core/lib/coreBundle').registry;
const fs = require('fs');
const protocolGenerator = require('./protocol-types-generator');
const {execSync, execFileSync} = require('child_process');
const jarvis = require('jarvis-browser-core');

const SCRIPT_NAME = path.basename(__filename);
const CORE_PATH = path.resolve(path.join(__dirname, '..', 'packages', 'jarvis-browser-core'));
...
Rolling firefox requires a jarvis-browsers checkout
next to the jarvis checkout, to roll browser patches from upstream.
...
(async () => {
  ...
  // 2. Rolling Firefox requires a jarvis-browsers checkout to roll browser patches from.
  const browsersCheckoutPath = process.env.PW_BROWSERS_CHECKOUT || path.resolve(__dirname, '..', '..', 'jarvis-browsers');
  if (browserTypeName === 'firefox' && !fs.existsSync(path.join(browsersCheckoutPath, 'browser_patches', 'build_flavors.sh'))) {
    console.log(`Rolling ${browserName} requires a jarvis-browsers checkout at "${browsersCheckoutPath}".`);
  ...
  const browserType = jarvis[browserTypeName];
  if (browserType) {
  ...
  // 6. Roll firefox browser patches from the jarvis-browsers checkout.
  if (browserTypeName === 'firefox') {
    console.log('\nRolling firefox browser patches from the jarvis-browsers checkout...');
