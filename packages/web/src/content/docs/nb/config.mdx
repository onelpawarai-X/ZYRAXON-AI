---
title: Konfigurasjon
description: Ved å bruke zyraxon JSON-konfigurasjonen.
---
...
For eksempel, hvis din globale konfigurasjon setter `theme: "zyraxon"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/zyraxon`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/zyraxon/zyraxon.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`ZYRAXON_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`zyraxon.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.zyraxon` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`ZYRAXON_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.zyraxon` og `~/.config/zyraxon` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/zyraxon`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale ZYRAXON-konfigurasjon i `~/.config/zyraxon/zyraxon.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `ZYRAXON_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `ZYRAXON_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.zyraxon` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.zyraxon`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `zyraxon.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/zyraxon/agents/` eller `.zyraxon/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/zyraxon/commands/` eller `.zyraxon/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.zyraxon/plugins/` eller `~/.config/zyraxon/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
