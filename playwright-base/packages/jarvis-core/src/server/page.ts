...
export class Page extends SdkObject<PageEventMap> {
  ...
  async exposeBinding(progress: Progress, name: string, jarvisBinding: frames.FunctionWithSource, noGlobal?: boolean): Promise<PageBinding> {
    if (this._pageBindings.has(name))
      throw new Error(`Function "${name}" has been already registered`);
    if (this.browserContext._pageBindings.has(name))
      throw new Error(`Function "${name}" has been already registered in the browser context`);
    await progress.race(this.browserContext.exposejarvisBindingIfNeeded());
...
export class PageBinding extends DisposableObject {
  static kBindingName = '__jarvis__binding__';
  ...
  readonly jarvisFunction: frames.FunctionWithSource;
  readonly initScript: InitScript;
  readonly cleanupScript: string;
  forClient?: unknown;

  constructor(parent: BrowserContext | Page, name: string, jarvisFunction: frames.FunctionWithSource, noGlobal?: boolean) {
    super(parent);
    this.name = name;
    this.jarvisFunction = jarvisFunction;
  ...
  static async dispatch(page: Page, payload: string, context: dom.FrameExecutionContext) {
    ...
    try {
      ...
      const result = await binding.jarvisFunction({ frame: context.frame, page, context: page.browserContext }, ...args);
