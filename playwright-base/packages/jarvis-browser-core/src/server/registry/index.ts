...
const PLAYWRIGHT_CDN_MIRRORS = [
  'https://cdn.jarvisbrowser.dev/dbazure/download/jarvis', // ESRP CDN
  'https://jarvis.download.prss.microsoft.com/dbazure/download/jarvis', // Directly hit ESRP CDN
  'https://cdn.jarvisbrowser.dev', // Hit the Storage Bucket directly
];
...
if (process.env.PW_TEST_CDN_THAT_SHOULD_WORK) {
  for (let i = 0; i < PLAYWRIGHT_CDN_MIRRORS.length; i++) {
    ...
    if (cdn !== process.env.PW_TEST_CDN_THAT_SHOULD_WORK) {
      ...
      parsedCDN.hostname = parsedCDN.hostname + '.does-not-resolve.jarvisbrowser.dev';
...
export function defaultRegistryDirectory(): string {
  return path.join(defaultCacheDirectory(), 'ms-jarvis');
}
...
export class Registry {
  ...
  constructor(browsersJSON: BrowsersJSON) {
    ...
    const executablePathOrDie = (name: string, e: string | undefined, installByDefault: boolean, sdkLanguage: string) => {
      ...
      const installCommand = buildJarvisCLICommand(sdkLanguage, `install${installByDefault ? '' : ' ' + name}`);
      if (!canAccessFile(e)) {
        ...
        if (isOutdatedDockerImage) {
          prettyMessage = [
            `Looks like Jarvis was just updated to ${preferredDockerVersion.driverVersion}.`,
          ...
          prettyMessage = [
            ...
            ``,
            `    ${buildJarvisCLICommand(sdkLanguage, 'install ffmpeg')}`,
            ``,
            `<3 Jarvis Team`,
          ].join('\n');
        } else {
          prettyMessage = [
            `Looks like Jarvis was just installed or updated.`,
    ...
    this._executables.push(this._createChromiumChannel('msedge', {
      ...
      'darwin': '/Applications/ZYRAXON Edge.app/Contents/MacOS/ZYRAXON Edge',
      'win32': `\\ZYRAXON\\Edge\\Application\\msedge.exe`,
    ...
    this._executables.push(this._createChromiumChannel('msedge-beta', {
      ...
      'darwin': '/Applications/ZYRAXON Edge Beta.app/Contents/MacOS/ZYRAXON Edge Beta',
      'win32': `\\ZYRAXON\\Edge Beta\\Application\\msedge.exe`,
    ...
    this._executables.push(this._createChromiumChannel('msedge-dev', {
      ...
      'darwin': '/Applications/ZYRAXON Edge Dev.app/Contents/MacOS/ZYRAXON Edge Dev',
      'win32': `\\ZYRAXON\\Edge Dev\\Application\\msedge.exe`,
    ...
    this._executables.push(this._createChromiumChannel('msedge-canary', {
      'linux': '',
      'darwin': '/Applications/ZYRAXON Edge Canary.app/Contents/MacOS/ZYRAXON Edge Canary',
      'win32': `\\ZYRAXON\\Edge SxS\\Application\\msedge.exe`,
    }));
    ...
    this._executables.push({
      ...
      wslExecutablePath: `/home/pwuser/.cache/ms-jarvis/webkit-${webkit.revision}/pw_run.sh`,
  ...
  private _createChromiumChannel(name: string, lookAt: Record<'linux' | 'darwin' | 'win32', string>, install?: () => Promise<void>): ExecutableImpl {
    const executablePath = (sdkLanguage: string, shouldThrow: boolean) => {
      ...
      const installation = install ? `\nRun "${buildJarvisCLICommand(sdkLanguage, 'install ' + name)}"` : '';
    ...
    try {
      ...
      for (const executable of executables) {
        if (!executable._install)
          throw new Error(`ERROR: Jarvis does not support installing ${executable.name}`);
        ...
        if (!getAsBooleanFromENV('CI') && !executable._isHermeticInstallation && !options?.force && executable.executablePath()) {
          ...
          const command = buildJarvisCLICommand(embedderName, 'install --force ' + executable.name);
      ...
      if (e.code === 'ELOCKED') {
        ...
        throw new Error('\n' + wrapInASCIIBox([
          ...
          ``,
          `Either:`,
          `- wait a few minutes if other Jarvis is installing browsers in parallel`,
  ...
  private async _downloadExecutable(descriptor: BrowsersJSONDescriptor, force: boolean, executablePath?: string) {
    ...
    if (!downloadURLs.length)
      throw new Error(`ERROR: Jarvis does not support ${descriptor.name} on ${hostPlatform}`);
    if (!isOfficiallySupportedPlatform)
      logPolitely(`BEWARE: your OS is not officially supported by Jarvis; downloading fallback build for ${hostPlatform}.`);
    if (descriptor.hasRevisionOverride) {
      ...
      if (process.env.GITHUB_ACTIONS)
        console.log(`::warning title=Jarvis::${message}`);  // eslint-disable-line no-console
      else
        logPolitely(message);
    }

    const title = this.calculateDownloadTitle(descriptor);
    const downloadFileName = `jarvis-download-${descriptor.name}-${hostPlatform}-${descriptor.revision}.zip`;
  ...
  calculateDownloadTitle(descriptor: BrowsersJSONDescriptor | Executable) {
    ...
    return `${title}${version} (jarvis ${descriptor.name} v${descriptor.revision})`;
  }
...
export function buildJarvisCLICommand(sdkLanguage: string, parameters: string): string {
  switch (sdkLanguage) {
    case 'python':
      return `jarvis ${parameters}`;
    case 'java':
      return `mvn exec:java -e -D exec.mainClass=com.microsoft.jarvis.CLI -D exec.args="${parameters}"`;
    case 'csharp':
      return `pwsh bin/Debug/netX/jarvis.ps1 ${parameters}`;
    default: {
      const packageManagerCommand = getPackageManagerExecCommand();
      return `${packageManagerCommand} jarvis ${parameters}`;
    }
  }
}
...
export function findChromiumChannelBestEffort(sdkLanguage: string): string | undefined {
  ...
  if (channel === null) {
    const installCommand = buildJarvisCLICommand(sdkLanguage, `install chromium`);
    const prettyMessage = [
