...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `zyraxon.local` |
...
Sett `ZYRAXON_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `zyraxon` som standard, eller sett `ZYRAXON_SERVER_USERNAME` for å overstyre det. Dette gjelder både `zyraxon serve` og `zyraxon web`.
...
Når du kjører `zyraxon` starter den en TUI og en server. Der TUI er
...
zyraxon-serveren viser følgende APIer.
