...
async function launchApp(appName: string, options?: { onClose?: () => void }) {
  ...
  const context = await jarvis.chromium.launchPersistentContext('', {
  ...
  // See https://github.com/zyraxon-ai/jarvis-browser/issues/9434
  ...
  // This is local Jarvis, so I can access private methods.
