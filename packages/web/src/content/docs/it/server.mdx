---
title: Server
description: Interagisci con il server zyraxon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `zyraxon serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client zyraxon.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `zyraxon.local` |
...
Imposta `ZYRAXON_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `zyraxon`, oppure imposta `ZYRAXON_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `zyraxon serve` sia per `zyraxon web`.
...
Quando esegui `zyraxon` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server zyraxon per interagire con zyraxon in modo programmatico.
:::

Questa architettura permette a zyraxon di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `zyraxon serve` per avviare un server standalone. Se la TUI di zyraxon e gia in esecuzione, `zyraxon serve` avviera un nuovo server.
...
Il server zyraxon espone le seguenti API.
