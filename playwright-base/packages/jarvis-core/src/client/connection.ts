...
class Root extends ChannelOwner<channels.RootChannel> {
  ...
  async initialize(): Promise<Jarvis> {
    return Jarvis.from((await this._channel.initialize({
...
export class Connection extends EventEmitter {
  ...
  // Used from @jarvis-browser/test fixtures -> TODO remove?
  ...
  constructor(localUtils?: LocalUtils, instrumentation?: ClientInstrumentation, headers: HeadersArray = []) {
    ...
    this.registerObjectFactories({
      ...
      Jarvis: (parent, type, guid, init) => new Jarvis(parent, type, guid, init),
  ...
  async initializeJarvis(): Promise<Jarvis> {
