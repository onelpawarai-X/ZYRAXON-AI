...
export class ClaudeGenerator {
  static async init(fullConfig: FullConfigInternal, projectName: string, prompts: boolean) {
    ...
    const mcpServer = process.platform === 'win32'
      ? { command: 'cmd', args: ['/c', 'npx', 'jarvis', 'run-test-mcp-server'] }
      : { command: 'npx', args: ['jarvis', 'run-test-mcp-server'] };
    await writeFile('.mcp.json', JSON.stringify({
      mcpServers: {
        'jarvis-browser-test': mcpServer,
      }
...
export class CodexGenerator {
  ...
  static agentSpec(agent: AgentSpec): string {
    const mcpName = 'jarvis-browser-test';
...
export class ZyraxonGenerator {
  ...
  static configuration(agents: AgentSpec[]): string {
    ...
    result['mcp']['jarvis-browser-test'] = {
      type: 'local',
      command: ['npx', 'jarvis', 'run-test-mcp-server'],
      enabled: true,
    };
...
export class VSCodeGenerator {
  static async init(fullConfig: FullConfigInternal, projectName: string) {
    ...
    const nameMap = new Map<string, string>([
      ['jarvis-browser-test-planner', ' 🎭 planner'],
      ['jarvis-browser-test-generator', '🎭 generator'],
      ['jarvis-browser-test-healer', '🎭 healer'],
    ]);
  ...
  static async appendToMCPJson() {
    ...
    mcpJson.servers['jarvis-browser-test'] = {
      type: 'stdio',
      command: 'npx',
      args: ['jarvis', 'run-test-mcp-server'],
    };
  ...
  static agentSpec(agent: AgentSpec): string {
    ...
    const vscodeMcpName = 'jarvis-browser-test';
