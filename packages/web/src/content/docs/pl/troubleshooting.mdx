...
Aby debugować problemy z zyraxon, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\zyraxon\log`
...
zyraxon przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/zyraxon/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\zyraxon`
...
zyraxon Desktop uruchamia w tle lokalny serwer zyraxon (sidecar `zyraxon-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **macOS/Linux**: `~/.config/zyraxon/zyraxon.jsonc` (lub `~/.config/zyraxon/zyraxon.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
zyraxon może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/zyraxon/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\zyraxon\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby zyraxon mógł ją odbudować.

1. Całkowicie zamknij zyraxon Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/zyraxon`
- **Linux**: usuń `~/.cache/zyraxon` (lub uruchom `rm -rf ~/.cache/zyraxon`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\zyraxon`

3. Uruchom ponownie zyraxon Desktop.

---

### Napraw problemy z połączeniem z serwerem

zyraxon Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli w swoim środowisku masz ustawiony `ZYRAXON_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `ZYRAXON_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows zyraxon Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami zyraxon.

---

### Brak powiadomień

zyraxon Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla zyraxon w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie zyraxon Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji zyraxon Desktop):

- `zyraxon.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `zyraxon.global.dat` i `zyraxon.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\zyraxon`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. zyraxon dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\zyraxon`

2. Uruchom ponownie kod zyraxon, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to zyraxon do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
zyraxon wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
