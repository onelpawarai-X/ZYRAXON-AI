...
import { jarvisExtensionInstallUrl } from '../utils/extension';
...
export class TextOutput implements Output {
  ...
  errorDetachNotAttached(session: string): never {
    console.error(`Error: session '${session}' was not attached; use \`jarvis-cli${session !== 'default' ? ` -s=${session}` : ''} close\` to stop it.`);
    return process.exit(1);
  }
  ...
  errorBrowserNotOpenForTool(session: string): never {
    ...
    console.log('');
    console.log(`  jarvis-cli${session !== 'default' ? ` -s=${session}` : ''} open [params]`);
    return process.exit(1);
  }
  ...
  attach(session: string, pid: number | undefined, endpoint: string | undefined, toolResult: string): void {
    if (endpoint) {
      ...
      console.log(`Run commands with: jarvis-cli -s=${session} <command>`);
      console.log('');
    } else {
      console.log(`### Browser \`${session}\` opened with pid ${pid}.`);
    }
...
function renderChannelSession(session: ChannelSession): string {
  ...
  if (session.extensionInstalled)
    lines.push(`  - attach (extension): \`jarvis-cli attach --extension=${session.channel}\``);
  else
    lines.push(`  - attach (extension): install at ${jarvisExtensionInstallUrl}`);
  if (session.endpoint) {
    lines.push(`  - attach (remote debugging): \`jarvis-cli attach --cdp=${session.channel}\``);
  } else {
