...
1. **Configuration distante** (à partir de `.well-known/zyraxon`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/zyraxon/zyraxon.json`) - préférences utilisateur
3. **Configuration personnalisée** (`ZYRAXON_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`zyraxon.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.zyraxon`** - agents, commandes, plugins
6. **Configuration en ligne** (`ZYRAXON_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.zyraxon` et `~/.config/zyraxon` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/zyraxon`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale ZYRAXON dans `~/.config/zyraxon/zyraxon.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/zyraxon/tui.json`.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `ZYRAXON_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `ZYRAXON_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.zyraxon`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.zyraxon`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `ZYRAXON_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.
...
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `zyraxon.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/zyraxon/agents/` ou `.zyraxon/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/zyraxon/commands/` ou `.zyraxon/commands/`. [En savoir plus ici](/docs/commands).
...
Par défaut, zyraxon **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
Placez les fichiers du plugin dans `.zyraxon/plugins/` ou `~/.config/zyraxon/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
