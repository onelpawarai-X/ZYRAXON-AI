---
title: Server
description: Interact with zyraxon server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `zyraxon serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an zyraxon client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `zyraxon.local` |
...
Set `ZYRAXON_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `zyraxon`, or set `ZYRAXON_SERVER_USERNAME` to override it. This applies to both `zyraxon serve` and `zyraxon web`.
...
When you run `zyraxon` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the zyraxon server to interact with zyraxon programmatically.
:::

This architecture lets zyraxon support multiple clients and allows you to interact with zyraxon programmatically.

You can run `zyraxon serve` to start a standalone server. If you have the
zyraxon TUI running, `zyraxon serve` will start a new server.
...
The zyraxon server exposes the following APIs.
