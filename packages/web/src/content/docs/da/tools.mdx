...
:::note
Dette verktøyet er bare tilgængelig når `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (eller `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger ZYRAXON-leverandøren eller når miljøvariabelen `ZYRAXON_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
