...
const open = declareCommand({
  ...
  options: z.object({
    ...
    config: z.string().optional().describe('Path to the configuration file, defaults to .jarvis/cli.config.json'),
...
const attach = declareCommand({
  name: 'attach',
  description: 'Attach to a running Jarvis browser',
  category: 'core',
  args: z.object({
  ...
  options: z.object({
    ...
    endpoint: z.string().optional().describe('Jarvis browser server endpoint to attach to.'),
...
const generateLocator = declareCommand({
  ...
  description: 'Generate a Jarvis locator for the given element',
  category: 'devtools',
  args: z.object({
    target: z.string().describe(elementTargetDescription),
  }),
...
const runCode = declareCommand({
  name: 'run-code',
  description: 'Run Jarvis code snippet',
  category: 'devtools',
  args: z.object({
    code: z.string().optional().describe('A JavaScript function containing Jarvis code to execute. It will be invoked with a single argument, page, which you can use for any page interaction.'),
  }),
  options: z.object({
...
const dashboardShow = declareCommand({
  name: 'show',
  description: 'Show Jarvis Dashboard',
  category: 'devtools',
  raw: true,
  args: z.object({}),
  options: z.object({
...
const installBrowser = declareCommand({
  ...
  options: z.object({
    ...
    list: z.boolean().optional().describe('Prints list of browsers from all Jarvis installations'),
