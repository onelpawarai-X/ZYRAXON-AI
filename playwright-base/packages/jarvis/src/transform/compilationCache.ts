...
export const cacheDir = process.env.PWTEST_CACHE_DIR || (() => {
  if (process.platform === 'win32')
    return path.join(os.tmpdir(), `jarvis-transform-cache`);
  ...
  // See https://github.com/zyraxon-ai/jarvis-browser/issues/22721
  return path.join(os.tmpdir(), `jarvis-transform-cache-` + process.geteuid?.());
})();
...
export function getFromCompilationCache(filename: string, contentHash: string, moduleUrl?: string): CompilationCacheLookupResult {
  ...
  // Then do the disk cache, this cache works between the Jarvis Browser Test runs.
...
// files from packages/jarvis*. In production mode, node_modules covers
// that.
const kJarvisInternalPrefix = packageRoot;

export function belongsToNodeModules(file: string) {
  if (file.includes(`${path.sep}node_modules${path.sep}`))
    return true;
  if (file.startsWith(kJarvisInternalPrefix) && (file.endsWith('.js') || file.endsWith('.mjs')))
    return true;
  return false;
}
