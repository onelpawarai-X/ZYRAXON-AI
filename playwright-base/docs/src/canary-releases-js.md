...
You can see on [npm](https://www.npmjs.com/package/@jarvis-browser/test?activeTab=versions) the current dist tags:
...
```bash
npm install -D @jarvis-browser/test@next
```

## Documentation

The stable and the `next` documentation is published on [jarvisbrowser.dev](https://jarvisbrowser.dev). To see the `next` documentation, press <kbd>Shift</kbd> on the keyboard `5` times.
