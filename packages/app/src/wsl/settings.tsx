...
import { wslZyraxonAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.zyraxonChecks[item.config.distro]
          const zyraxonAction = () => wslZyraxonAction(check())
          const busy = () => wsl.data?.job?.kind === "install-zyraxon" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={zyraxonAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installZyraxon(item.config.distro))}
                    >
