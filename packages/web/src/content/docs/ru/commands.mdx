...
Создайте `.zyraxon/commands/test.md`:

```md title=".zyraxon/commands/test.md"
---
...
Вы можете добавлять собственные команды через конфигурацию zyraxon или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем zyraxon [config](/docs/config):
...
- Глобальный: `~/.config/zyraxon/commands/`
- Для каждого проекта: `.zyraxon/commands/`

```markdown title="~/.config/zyraxon/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".zyraxon/commands/component.md"
---
...
```md title=".zyraxon/commands/create-file.md"
---
...
```md title=".zyraxon/commands/analyze-coverage.md"
---
...
```md title=".zyraxon/commands/review-changes.md"
---
...
```md title=".zyraxon/commands/review-component.md"
---
...
zyraxon включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
