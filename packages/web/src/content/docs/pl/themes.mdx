...
Dzięki zyraxon możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie zyraxon używa naszego własnego motywu `zyraxon`.
...
zyraxon ma kilka wbudowanych motywów.
...
zyraxon obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/zyraxon/themes/*.json` lub `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.zyraxon/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.zyraxon/themes/*.json`
