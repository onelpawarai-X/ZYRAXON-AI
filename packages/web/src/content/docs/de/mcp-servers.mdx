...
Organisationen können über ihren `.well-known/zyraxon`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert ZYRAXON die Token sicher in `~/.local/share/zyraxon/mcp-auth.json`.
...
Fügen Sie den Server [Grep by Vercel](https://github.com/upstash/context71) MCP hinzu, um Codefragmente auf GitHub zu durchsuchen.
