...
1. **Fjernkonfiguration** (fra `.well-known/zyraxon`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/zyraxon/zyraxon.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`ZYRAXON_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`zyraxon.json` i projekt) - projektspecifikke indstillinger
5. **`.zyraxon` mapper** - agent, kommandoer, plugins
6. **Inline config** (`ZYRAXON_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.zyraxon` og `~/.config/zyraxon` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/zyraxon`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale ZYRAXON-konfiguration i `~/.config/zyraxon/zyraxon.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `ZYRAXON_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `ZYRAXON_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.zyraxon` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.zyraxon` mapper, så den **kan tilsidesætte** deres indstillinger.
...
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `zyraxon.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/zyraxon/agents/` eller `.zyraxon/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/zyraxon/commands/` eller `.zyraxon/commands/`. [Learn more here](/docs/commands).
...
Som standard zyraxon **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
Placer plugin-filer i `.zyraxon/plugins/` eller `~/.config/zyraxon/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
