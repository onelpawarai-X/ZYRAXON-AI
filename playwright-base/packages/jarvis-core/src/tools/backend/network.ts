...
function isSuccessfulResponse(request: jarvis.Request): boolean {
  if (request.failure())
    return false;
...
export function isFetch(request: jarvis.Request): boolean {
  return ['fetch', 'xhr'].includes(request.resourceType());
}

export function renderRequestLine(request: jarvis.Request): string {
  const response = request.existingResponse();
...
function renderRequestDetails(index: number, request: jarvis.Request, skillMode: boolean): string {
...
function canHaveResponseBody(httpResponse: jarvis.Response | null): httpResponse is jarvis.Response {
  if (!httpResponse)
    return false;
...
function computeDurationMs(request: jarvis.Request): number | undefined {
...
async function renderRequestPart(request: jarvis.Request, part: RequestPart, response: ToolResponse, suggestedFilename: string | undefined): Promise<void> {
...
async function saveResponseBody(request: jarvis.Request, response: ToolResponse, suggestedFilename?: string): Promise<string | undefined> {
  const httpResponse = request.existingResponse();
  if (!canHaveResponseBody(httpResponse))
    return undefined;
  let body: Buffer;
