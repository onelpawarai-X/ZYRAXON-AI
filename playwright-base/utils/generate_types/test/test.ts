...
jarvis.chromium.launch().then(async browser => {
  const page = await browser.newPage();
...
// Test v0.12 features
(async () => {
  const launchOptions: jarvis.LaunchOptions = {
    env: {
      TIMEOUT: '52',
      SOMETHING: '/some/path',
      JEST_TEST: 'true'
  ...
  const browser = await jarvis.chromium.launch(launchOptions);
  const viewport: jarvis.ViewportSize = {
    width: 100,
    height: 200,
  };
  const geolocation: jarvis.Geolocation = {
    latitude: 0,
    longitude: 0,
    accuracy: undefined,
  };
  const httpCredentials: jarvis.HTTPCredentials = {
    username: 'foo',
    password: 'bar',
  };
  const contextOptions: jarvis.BrowserContextOptions = {
    viewport,
    geolocation,
    httpCredentials,
  };
  ...
  page.on('console', (event: jarvis.ConsoleMessage, ...args: any[]) => {
  ...
  const cookies: jarvis.Cookie[] = await page.context().cookies(['http://example.com']);
  ...
  // evaluateHandle with two different return types (ElementHandle)
  {
    ...
    const assertion: AssertType<jarvis.ElementHandle<HTMLInputElement | HTMLTextAreaElement>, typeof handle> = true;
  }
...
// test $eval and $$eval
(async () => {
  const browser = await jarvis.firefox.launch();
  const page = await browser.newPage();
  await page.goto('https://example.com');
...
// branded primitives in evaluate arguments — https://github.com/zyraxon-ai/jarvis-browser/issues/42000
...
// waitForEvent
(async () => {
  const browser = await jarvis.webkit.launch();
  const page = await browser.newPage();
  {
    const frame = await page.waitForEvent('frameattached');
    const assertion: AssertType<jarvis.Frame, typeof frame> = true;
  }
  {
    const worker = await page.waitForEvent('worker', {
      predicate: worker => {
        const condition: AssertType<jarvis.Worker, typeof worker> = true;
        return true;
      }
    });
    const assertion: AssertType<jarvis.Worker, typeof worker> = true;
  }
  {
    const newPage = await page.context().waitForEvent('page', {
      timeout: 500
    });
    const assertion: AssertType<jarvis.Page, typeof newPage> = true;
  }
  {
    const response = await page.waitForEvent('response', response => response.url() === 'asdf');
    const assertion: AssertType<jarvis.Response, typeof response> = true;
  }
})();
...
(async () => {
  ...
  }
  {
    ...
    const assertion: AssertType<jarvis.JSHandle<string>, typeof value> = true;
  }
  {
    const handle = await page.locator('body').evaluateHandle(() => ([{a: '123'}]));
    const value = await handle.evaluateHandle((h, p) => ({ a: h[1].a, p}), 123);
    const assertion: AssertType<jarvis.JSHandle<{a: string, p: number}>, typeof value> = true;
  }
  {
    const handle = await page.locator('body').evaluateHandle(() => ([{a: '123'}]));
    const value = await handle.evaluateHandle((h: ({a: string, b: number})[]) => h[1].b);
    const assertion: AssertType<jarvis.JSHandle<number>, typeof value> = true;
  }
  {
  ...
  }
  {
    ...
    const assertion: AssertType<jarvis.JSHandle<boolean | number>, typeof value> = true;
  }
  ...
  {
    type T = ({ s: string } | jarvis.ElementHandle)[];
    const handle = await page.evaluateHandle(() => document.body);
...

  {
    ...
    const assertion: AssertType<jarvis.ElementHandle<HTMLBodyElement>, typeof handle> = true;
...
(async () => {
  ...
  for (const elementLike of elementLikes) {
    {
      ...
      const bodyAssertion: AssertType<jarvis.ElementHandle<HTMLBodyElement>, typeof handle> = true;
    }
    {
...

    {
      ...
      const bodyAssertion: AssertType<jarvis.ElementHandle<HTMLBodyElement>[], typeof handles> = true;
    }
  ...
  for (const frameLike of frameLikes) {
    ...
    }
    {
      ...
      const elementAssertion: AssertType<jarvis.ElementHandle<HTMLElement|SVGElement>, typeof handle> = true;
      const canBeNull: AssertCanBeNull<typeof handle> = false;
    }
    {
...
// top level
(async () => {
  jarvis.chromium.connect;
  jarvis.errors.TimeoutError;
  {
    jarvisbrowser.devices['my device'] = {
      userAgent: 'foo',
    ...
    const iPhone = jarvisbrowser.devices['iPhone 11'];
  ...
  {
    const agents = Object.entries(jarvisbrowser.devices).map(([name, descriptor]) => descriptor.userAgent);
  ...
  await jarvis.selectors.register('tag', createTagNameEngine);
})();

// Event listeners
(async function() {
  {
    const eventEmitter = {} as (jarvis.Page | EventEmitter);
    const listener = () => { };
  ...
  {
    const eventEmitter = {} as (jarvis.BrowserContext | EventEmitter);
    const listener = (c: jarvis.BrowserContext) => { };
    eventEmitter.addListener('close', listener)
      .on('close', listener)
      .once('close', listener)
      .removeListener('close', listener)
...
(async () => {
  const browserType = {} as jarvis.BrowserType<jarvis.Browser & {foo: 'string'}>;
