...
import type { JarvisDispatcher } from './jarvisDispatcher';
...
export class RootDispatcher extends Dispatcher<SdkObject, any, any> {
  ...
  constructor(connection: DispatcherConnection, private readonly createJarvis?: (scope: RootDispatcher, options: channels.RootInitializeParams) => Promise<JarvisDispatcher>) {
  ...
  async initialize(params: channels.RootInitializeParams, progress: Progress): Promise<channels.RootInitializeResult> {
    ...
    assert(this.createJarvis);
    assert(!this._initialized);
    this._initialized = true;
    return {
      jarvis: await progress.race(this.createJarvis(this, params)),
    };
  }
}
