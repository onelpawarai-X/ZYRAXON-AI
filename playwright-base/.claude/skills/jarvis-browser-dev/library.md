# Jarvis Browser Library Architecture: Client, Server, and Dispatchers
...
packages/jarvis-browser-core/src/
...
- `packages/jarvis-browser-core/src/protocol/validator.ts` — runtime validators: `scheme.PageGotoParams = tObject({...})`
- `packages/jarvis-browser-core/src/utils/isomorphic/protocolMetainfo.ts` — method flags (slowMo, snapshot, etc.)
...
```
packages/jarvis-browser-core/src/client/channelOwner.ts
```
...
```
packages/jarvis-browser-core/src/client/connection.ts
```
...
```
packages/jarvis-browser-core/src/server/instrumentation.ts
```
...
Implementations:
- `packages/jarvis-browser-core/src/server/chromium/crPage.ts` — uses CDP
- `packages/jarvis-browser-core/src/server/firefox/ffPage.ts`
- `packages/jarvis-browser-core/src/server/webkit/wkPage.ts`
...
```
packages/jarvis-browser-core/src/server/dispatchers/dispatcher.ts
```
