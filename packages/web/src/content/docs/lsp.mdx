...
:::note
You can disable automatic LSP server downloads by setting the `ZYRAXON_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and zyraxon opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your zyraxon config.
