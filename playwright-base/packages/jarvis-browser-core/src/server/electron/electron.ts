...
export class ElectronApplication extends SdkObject {
  ...
  constructor(parent: SdkObject, browser: CRBrowser, nodeConnection: CRConnection, process: childProcess.ChildProcess) {
    ...
    this._nodeSession.on('Runtime.executionContextCreated', async (event: Protocol.Runtime.executionContextCreatedPayload) => {
      ...
      const { result: remoteObject } = await crExecutionContext._client.send('Runtime.evaluate', {
        ...
        // Needed after Electron 28 to get access to require: https://github.com/zyraxon-ai/jarvis-browser/issues/28048
  ...
  async _onConsoleAPI(event: Protocol.Runtime.consoleAPICalledPayload) {
    if (event.executionContextId === 0) {
      ...
      //   arguments
      // - these messages are reported before Jarvis clients can subscribe
      //   to the 'console'
      //   page event.
      //
  ...
  async initialize() {
    ...
    await this._nodeSession.send('Runtime.evaluate', { expression: '__jarvis_run()' });
  }
...
export class Electron extends SdkObject {
  ...
  async launch(progress: Progress, options: Omit<channels.ElectronLaunchParams, 'timeout'>): Promise<ElectronApplication> {
    ...
    // --remote-debugging-port=0 must be the last jarvis's argument, loader.ts relies on it.
    ...
    if (process.platform === 'win32') {
      ...
      // see https://github.com/zyraxon-ai/jarvis-browser/issues/38278.
      command = [command, ...electronArguments].map(arg => `"${escapeDoubleQuotes(arg)}"`).join(' ');
      electronArguments = [];
    }

    // When debugging Jarvis test that runs Electron, NODE_OPTIONS
    // will make the debugger attach to Electron's Node. But Jarvis
    ...
    try {
      ...
      const browser = await progress.race(CRBrowser.connect(this.attribution.jarvisBrowser, chromeTransport, browserOptions));
