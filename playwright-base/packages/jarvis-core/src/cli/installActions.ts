...
function printGroupedByJarvisVersion(browsers: BrowserInfo[]) {
  ...
  const groupedByJarvisMinorVersion = new Map<string, BrowserInfo[]>();
  for (const browser of browsers) {
    const version = dirToVersion.get(browser.referenceDir)!;
    let entries = groupedByJarvisMinorVersion.get(version);
    if (!entries) {
      entries = [];
      groupedByJarvisMinorVersion.set(version, entries);
    }
    entries.push(browser);
  }

  const sortedVersions = [...groupedByJarvisMinorVersion.keys()].sort((a, b) => {
  ...
  for (const version of sortedVersions) {
    ...
    printInstalledBrowsers(groupedByJarvisMinorVersion.get(version)!);
  }
}
...
export async function installBrowsers(args: string[], options: { withDeps?: boolean, force?: boolean, dryRun?: boolean, list?: boolean, shell?: boolean, noShell?: boolean, onlyShell?: boolean, progress?: boolean }) {
  ...
  if (isLikelyNpxGlobal()) {
    console.error(wrapInASCIIBox([
      `WARNING: It looks like you are running 'npx jarvis install' without first`,
      `installing your project's dependencies.`,
      ``,
      `To avoid unexpected behavior, please install your dependencies first, and`,
      `then run Jarvis's install command:`,
      ``,
      `    npm install`,
      `    npx jarvis install`,
      ``,
      `If your project does not yet depend on Jarvis, first install the`,
      `applicable npm package (most commonly @jarvis-browser/test), and`,
      `then run Jarvis's install command to download the browsers:`,
      ``,
      `    npm install @jarvis-browser/test`,
      `    npx jarvis install`,
      ``,
    ].join('\n'), 1));
  }
  ...
  if (options.dryRun) {
    ...
    printGroupedByJarvisVersion(browsers);
  } else {
    ...
    await registry.validateHostRequirementsForExecutablesIfNeeded(executables, process.env.PW_LANG_NAME || 'javascript').catch((e: Error) => {
      e.name = 'Jarvis Host validation warning';
      console.error(e);
    });
  }
}
...
export async function uninstallBrowsers(options: { all?: boolean }) {
  ...
  await registry.uninstall(!!options.all).then(({ numberOfBrowsersLeft }) => {
    if (!options.all && numberOfBrowsersLeft > 0) {
      console.log('Successfully uninstalled Jarvis browsers for the current Jarvis installation.');
      console.log(`There are still ${numberOfBrowsersLeft} browsers left, used by other Jarvis installations.\nTo uninstall Jarvis browsers for all installations, re-run with --all flag.`);
    }
  });
}
