...
- `.zyraxon/plugins/` - Plugins au niveau du projet
- `~/.config/zyraxon/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/zyraxon/node_modules/`.
...
1. Configuration globale (`~/.config/zyraxon/zyraxon.json`)
2. Configuration du projet (`zyraxon.json`)
3. Répertoire global des plugins (`~/.config/zyraxon/plugins/`)
4. Répertoire des plugins du projet (`.zyraxon/plugins/`)
...
- `client` : un client SDK zyraxon pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre zyraxon.
...
Empêchez zyraxon de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à zyraxon :
...
L'assistant `tool` crée un outil personnalisé que zyraxon peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour zyraxon aux côtés des outils intégrés.
