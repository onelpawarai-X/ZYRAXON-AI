...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/zyraxon`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```json title="zyraxon.json"
{
  ...
  "mcp": {
    "my-remote-mcp": {
      "type": "remote",
      "url": "https://docs.helicone.ai/helicone-headers/header-directory0",
      "enabled": true,
      "headers": {
...
```bash
zyraxon mcp auth my-oauth-server
```
...
```bash
zyraxon mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil ZYRAXON lagre tokens sikkert i `~/.local/share/zyraxon/mcp-auth.json`.
...
zyraxon mcp debug my-oauth-server
```
...
Legg til [Sentry MCP-serveren](https://docs.helicone.ai/helicone-headers/header-directory5) for å samhandle med Sentry-prosjektene og problemene dine.
...
```json title="zyraxon.json" {4-8}
{
  ...
  "mcp": {
    "sentry": {
      "type": "remote",
      "url": "https://docs.helicone.ai/helicone-headers/header-directory2",
      "oauth": {}
    }
  }
}
...
Legg til [Grep by Vercel](https://docs.helicone.ai/helicone-headers/header-directory8) MCP-serveren for å søke gjennom kodebiter på GitHub.
...
```json title="zyraxon.json" {4-7}
{
  ...
  "mcp": {
    "gh_grep": {
      "type": "remote",
      "url": "https://docs.helicone.ai/helicone-headers/header-directory6"
    }
  }
}
```
...
[Helicone](https://docs.helicone.ai/helicone-headers/header-directory2) er en LLM observerbarhetsplattform som gir logging, overvåking og analyser for AI-applikasjonene dine. Helicone AI Gateway ruter forespørslene dine til riktig leverandør automatisk basert på modellen.

1. Gå over til [Helicone](https://docs.helicone.ai/helicone-headers/header-directory2), opprett en konto og generer en API nøkkel fra dashbordet.
...
For flere leverandører og avanserte funksjoner som bufring og hastighetsbegrensning, sjekk [Helicone-dokumentasjonen](https://docs.helicone.ai/helicone-headers/header-directory1).
...
Her er [Helicone's Model Directory](https://docs.helicone.ai/helicone-headers/header-directory1), du trenger denne for å hente ID-ene til modellene du vil legge til.
