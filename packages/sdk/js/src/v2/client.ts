...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-zyraxon-directory", "directory"],
    ["x-zyraxon-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-zyraxon-workspace")
  return next
}

export function createZyraxonClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-zyraxon-workspace": config.experimental_workspaceID,
    }
  }
  ...
  client.interceptors.response.use((response) => {
    ...
    if (contentType === "text/html")
      throw new Error("Request is not supported by this version of Zyraxon Server (Server responded with text/html)")
