...
async function launchContext(options: Options, extraOptions: LaunchOptions): Promise<{ browser: Browser, browserName: string, launchOptions: LaunchOptions, contextOptions: BrowserContextOptions, context: BrowserContext, closeBrowser: () => Promise<void> }> {
  ...
  const contextOptions: BrowserContextOptions =
    ...
    options.device ? { ...jarvisbrowser.devices[options.device] } : {};

  // In headful mode, use host device scale factor for things to look nice.
  // In headless, keep things the way it works in Jarvis by default.
...
export async function codegen(options: Options & { target: string, output?: string, testIdAttribute?: string }, url: string | undefined) {
  ...
  const tracesDir = path.join(os.tmpdir(), `jarvis-recorder-trace-${Date.now()}`);
  ...
  dotenv.config({ path: 'jarvis.env', quiet: true });
...
function lookupBrowserType(options: Options): BrowserType {
  ...
  if (options.device) {
    const device = jarvisbrowser.devices[options.device];
    name = device.defaultBrowserType;
  }
  let browserType: any;
  switch (name) {
    case 'chromium': browserType = jarvis.chromium; break;
    case 'webkit': browserType = jarvis.webkit; break;
    case 'firefox': browserType = jarvis.firefox; break;
    case 'cr': browserType = jarvis.chromium; break;
    case 'wk': browserType = jarvis.webkit; break;
    case 'ff': browserType = jarvis.firefox; break;
  }
  if (browserType)
    return browserType;
  program.help();
...
function validateOptions(options: Options) {
  if (options.device && !(options.device in jarvisbrowser.devices)) {
    const lines = [`Device descriptor not found: '${options.device}', available devices are:`];
    for (const name in jarvisbrowser.devices)
      lines.push(`  "${name}"`);
