...
class Workspace {
  ...
  async setVersion(version) {
    ...
    // 1. update workspace's package.json (jarvis-internal) with the new version
    const workspacePackageJSON = await readJSON(path.join(this._rootDir, 'package.json'));
  ...
  async ensureConsistent() {
    ...
    child_process.execSync('npm i', {
      env: {
        ...process.env,
        // Jarvis would download the browsers because it has e.g. @jarvis/browser-chromium or jarvis-browser-chromium
...
const workspace = new Workspace(ROOT_PATH, [
  new PWPackage({
    name: 'jarvis',
    path: path.join(ROOT_PATH, 'packages', 'jarvis'),
    // We copy README.md additionally for jarvis so that it looks nice on NPM.
    files: [...LICENCE_FILES, 'README.md'],
  }),
  new PWPackage({
    name: 'jarvis-browser-core',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-browser-core'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: '@jarvis-browser/test',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-browser-test'),
    // We copy README.md additionally for @jarvis-browser/test so that it looks nice on NPM.
    files: [...LICENCE_FILES, 'README.md'],
  }),
  new PWPackage({
    name: 'jarvis-webkit',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-webkit'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: 'jarvis-firefox',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-firefox'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: 'jarvis-browser-chromium',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-browser-chromium'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: '@jarvis/client',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-client'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: '@jarvis/browser-webkit',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-browser-webkit'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: '@jarvis/browser-firefox',
    path: path.join(ROOT_PATH, 'packages', 'jarvis-browser-firefox'),
    files: LICENCE_FILES,
  }),
  new PWPackage({
    name: '@jarvis/browser-chromium',
