...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\zyraxon\log`
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\zyraxon`
...
ZYRAXON Desktop pokreće lokalni ZYRAXON server (`zyraxon-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- **macOS/Linux**: `~/.config/zyraxon/zyraxon.jsonc` (ili `~/.config/zyraxon/zyraxon.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
- **Globalni dodaci**
  ...
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\zyraxon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/zyraxon`
- **Linux**: obrišite `~/.cache/zyraxon` (ili pokrenite `rm -rf ~/.cache/zyraxon`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\zyraxon`
...
Ako ste postavili `ZYRAXON_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `ZYRAXON_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
- `zyraxon.settings.dat` (URL zadanog servera za desktop)
- `zyraxon.global.dat` i `zyraxon.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\zyraxon`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\zyraxon`
