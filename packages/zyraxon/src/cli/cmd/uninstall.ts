...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall zyraxon and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g zyraxon-ai",
      pnpm: "pnpm uninstall -g zyraxon-ai",
      bun: "bun remove -g zyraxon-ai",
      yarn: "yarn global remove zyraxon-ai",
      brew: "brew uninstall zyraxon",
      choco: "choco uninstall zyraxon",
      scoop: "scoop uninstall zyraxon",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "zyraxon", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# zyraxon") || content.includes(".zyraxon/bin")) {
      return file
    }
  }
