...
Now, each test file should import `test` from our fixtures file instead of `@jarvis-browser/test`. No changes are needed in the config.
...
```js title="tests/auth.setup.ts"
import { test as setup } from '@jarvis-browser/test';
...
```js title="playwright/fixtures.ts"
import { test as baseTest, request } from '@jarvis-browser/test';
import fs from 'fs';
import path from 'path';
...
```js title="playwright/fixtures.ts"
import { test as base, type Page, type Locator } from '@jarvis-browser/test';
