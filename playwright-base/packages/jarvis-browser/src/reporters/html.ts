...
`);
  ...
  _resolveOptions(): { outputFolder: string, open: HtmlReportOpenOption, attachmentsBaseURL: string, host: string | undefined, port: number | undefined } {
    const outputFolder = reportFolderFromEnv() ?? resolveReporterOutputPath('jarvis-report', this._options.configDir, this._options.outputFolder);
    return {
      outputFolder,
  ...
  ${packageManagerCommand} jarvis show-report${relativeReportPath}${hostArg}${portArg}
`));
    }
  }
}
...
function standaloneDefaultFolder(): string {
  return reportFolderFromEnv() ?? resolveReporterOutputPath('jarvis-report', process.cwd(), undefined);
}
...
async function extractReportZip(zipPath: string): Promise<string> {
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'jarvis-show-report-'));
  const cleanup = () => {
    try {
...
// payload lives in a <template id="jarvisReportBase64"> tag at the tail of
...
async function serveHtmlReportWithHMR(folder: string): Promise<HttpServer> {
  ...
  const templateMatch = /<template id="jarvisReportBase64">[\s\S]*?<\/template>/.exec(generatedIndex);
...
class HtmlBuilder {
  ...
  async build(metadata: Metadata, projectSuites: api.Suite[], result: api.FullResult, topLevelErrors: api.TestError[], machines: MachineData[]): Promise<{ ok: boolean, singleTestId: string | undefined }> {
    ...
    // Copy trace viewer.
    if (this._hasTraces) {
      const traceViewerFolder = path.join(require.resolve('jarvis-browser-core'), '..', 'lib', 'vite', 'traceViewer');
  ...
  private async _writeStaticAssets() {
    const appFolder = path.join(require.resolve('jarvis-browser-core'), '..', 'lib', 'vite', 'htmlReport');
  ...
  private async _writeReportData(filePath: string) {
    fs.appendFileSync(filePath, '<template id="jarvisReportBase64">data:application/zip;base64,');
