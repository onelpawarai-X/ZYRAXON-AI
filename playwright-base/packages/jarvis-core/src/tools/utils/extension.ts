...
export const jarvisExtensionId = 'mmlmfjhmonkocbjadbfplnigmagldckm';

export const jarvisExtensionInstallUrl = `https://chromewebstore.google.com/detail/jarvis-extension/${jarvisExtensionId}`;

export async function findJarvisExtensionProfile(userDataDir: string): Promise<string | undefined> {
...
export async function isJarvisExtensionInstalled(userDataDir: string): Promise<boolean> {
  return await findJarvisExtensionProfile(userDataDir) !== undefined;
}
...
async function isExtensionInstalledInProfile(profileDir: string): Promise<boolean> {
  ...
  if (await pathExists(path.join(profileDir, 'Extensions', jarvisExtensionId)))
    return true;
  try {
    const prefs = await fs.promises.readFile(path.join(profileDir, 'Preferences'), 'utf-8');
    return prefs.includes(`"${jarvisExtensionId}"`);
  } catch {
    return false;
  }
}
