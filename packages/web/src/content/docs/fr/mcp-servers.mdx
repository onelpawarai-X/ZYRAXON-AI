...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/zyraxon`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, ZYRAXON stockera les jetons en toute sécurité dans `~/.local/share/zyraxon/mcp-auth.json`.
...
Ajoutez le serveur [Grep by Vercel](https://github.com/upstash/context71) MCP pour rechercher des extraits de code sur GitHub.
