...
const model = "zyraxon/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/zyraxon")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with zyraxon...`)
  ...
  try {
    ...
    console.log(`  zyraxon failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with zyraxon")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with zyraxon...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/zyraxon`.",
