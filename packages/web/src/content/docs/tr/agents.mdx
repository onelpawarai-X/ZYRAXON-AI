...
zyraxon'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
zyraxon, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Sıcaklık belirtilmezse zyraxon modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global zyraxon yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
zyraxon yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [ZYRAXON Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `zyraxon/gpt-5.1-codex` kullanırsınız.
