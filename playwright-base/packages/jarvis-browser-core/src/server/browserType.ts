...
export const kNoXServerRunningError = 'Looks like you launched a headed browser without having a XServer running.\n' +
  'Set either \'headless: true\' or use \'xvfb-run <your-jarvis-app>\' before running Jarvis.\n\n<3 Jarvis Team';
...
export abstract class BrowserType extends SdkObject {
  ...
  private async _innerLaunchWithRetries(progress: Progress, options: types.LaunchOptions, persistent: types.BrowserContextOptions | undefined, protocolLogger: types.ProtocolLogger, userDataDir?: string): Promise<Browser> {
    try {
      ...
      // @see https://github.com/zyraxon-ai/jarvis-browser/issues/5214
  ...
  private async _prepareToLaunch(options: types.LaunchOptions, isPersistent: boolean, userDataDir: string | undefined) {
    ...
    if (options.artifactsDir) {
      ...
      artifactsDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'jarvis-artifacts-'));
    ...
    if (userDataDir) {
      ...
      userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), `jarvis_${this._name}dev_profile-`));
    ...
    if (customExecutablePath) {
      ...
      executable = registryExecutable.executablePathOrDie(this.attribution.jarvisBrowser.options.sdkLanguage);
      await registry.validateHostRequirementsForExecutablesIfNeeded([registryExecutable], this.attribution.jarvisBrowser.options.sdkLanguage);
    }
  ...
  protected _createUserDataDirArgMisuseError(userDataDirArg: string): Error {
    switch (this.attribution.jarvisBrowser.options.sdkLanguage) {
      case 'java':
