...
import { createZyraxon, type ToolPart } from "@zyraxon-ai/sdk"
...
console.log("🚀 Starting zyraxon server...")
const zyraxon = await createZyraxon({
  port: 0,
})
console.log("✅ Zyraxon server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await zyraxon.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new zyraxon session...")
    const { client, server } = zyraxon
    ...
    console.log("✅ Created zyraxon session:", createResult.data.id)
  ...
  console.log("📝 Sending to zyraxon:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Zyraxon response:", JSON.stringify(result, null, 2))
