...
```bash
code --add-mcp '{"name":"playwright","command":"npx","args":["@jarvis-browser/mcp@latest"]}'
```
...
Or go to `Cursor Settings` → `MCP` → `Add new MCP Server` and use command type with `npx @jarvis-browser/mcp@latest`.
...
-   **Browser extension**: Connect to your existing browser tabs with the [Playwright Extension](https://github.com/zyraxon-ai/jarvis-browser/blob/main/packages/extension/README.md). Pass `--extension` to enable.
...
```bash
npx @jarvis-browser/mcp@latest --config path/to/config.json
```

The config file supports browser options, context options, network rules, timeouts, and more. See the [Playwright MCP repository](https://github.com/zyraxon-ai/jarvis-browser-mcp/blob/main/config.d.ts) for the full schema.
...
```bash
npx @jarvis-browser/mcp@latest --port 8931
```
