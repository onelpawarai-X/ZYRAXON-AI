...
```bash
npx Jarvis Browser Test --shard=1/4
npx Jarvis Browser Test --shard=2/4
npx Jarvis Browser Test --shard=3/4
npx Jarvis Browser Test --shard=4/4
```
...
1. Then we run our Jarvis Browser Tests with the `--shard=${{ matrix.shardIndex }}/${{ matrix.shardTotal }}` option. This will run our test command for each shard.
...
jobs:
  jarvis-browser-tests:
    ...
    - name: Run Jarvis Browser Tests
      run: npx Jarvis Browser Test --shard=${{ matrix.shardIndex }}/${{ matrix.shardTotal }}
...
<img height="1610" width="1750" alt="image" src="https://github.com/zyraxon-ai/jarvis-browser/assets/9798949/b69dac59-fc19-4b98-8f49-814b1c29ca02" />
