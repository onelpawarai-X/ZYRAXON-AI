...
// Client Certificates in Jarvis are implemented as a SOCKS5 proxy that injects client certificates into the TLS handshake.
...
class SocksProxyConnection {
  ...
  private _establishTlsTunnel(browserEncrypted: stream.Duplex, clientHello: Buffer, secureContext: tls.SecureContext) {
    ...
    serverDecrypted.once('error', async (error: Error) => {
      ...
      const responseBody = escapeHTML('Jarvis client-certificate error: ' + error.message)
          .replaceAll('\n', ' <br>');
...
function rewriteToLocalhostIfNeeded(host: string): string {
  return host === 'local.jarvis' ? 'localhost' : host;
}
