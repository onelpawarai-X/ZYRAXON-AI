...
// eslint-disable-next-line no-restricted-imports
import type { Jarvis } from '../../client/jarvis';
import type * as jarvisTypes from '../../..';
import type { BrowserInfo } from '../../serverRegistry';

type BrowserWithInfo = {
  browser: jarvisTypes.Browser,
...
export async function createBrowserWithInfo(config: FullConfig, clientInfo: ClientInfo, cliOptions: CLIOptions): Promise<BrowserWithInfo> {
  ...
  let browser: jarvisTypes.Browser;
  let canBind = false;
...
export interface BrowserContextFactory {
  contexts(clientInfo: ClientInfo): Promise<jarvisTypes.BrowserContext[]>;
  createContext(clientInfo: ClientInfo): Promise<jarvisTypes.BrowserContext>;
}

function browserInfo(browser: jarvisTypes.Browser, config: FullConfig): BrowserInfo {
  return {
...
async function createIsolatedBrowser(config: FullConfig, clientInfo: ClientInfo): Promise<jarvisTypes.Browser> {
  testDebug('create browser (isolated)');
  const browserType = jarvis[config.browser.browserName];
...
async function createCDPBrowser(config: FullConfig, clientInfo: ClientInfo): Promise<jarvisTypes.Browser> {
  testDebug('create browser (cdp)');
  const artifactsDir = await computeTracesDir(config, clientInfo);
  const browser = await jarvis.chromium.connectOverCDP(config.browser.cdpEndpoint!, {
...
async function createRemoteBrowser(config: FullConfig): Promise<BrowserWithInfo> {
  ...
  const jarvisObject = jarvis as Jarvis;
  // Use connectToBrowser instead of jarvis[browserName].connect because we don't have browserName.
  const browser = await connectToBrowser(jarvisObject, remoteOptions);
  browser._connectToBrowserType(jarvisObject[browser._browserName], {}, undefined);
...
async function createPersistentBrowser(config: FullConfig, clientInfo: ClientInfo): Promise<jarvisTypes.Browser> {
  ...
  const launchOptions: jarvisTypes.LaunchOptions & jarvisTypes.BrowserContextOptions = {
    tracesDir,
    ...config.browser.launchOptions,
  ...
  try {
    ...
    if (error.message.includes('cannot open shared object file: No such file or directory')) {
      ...
      throw new Error(`Missing system dependencies required to run browser ${browserName}. Install them with: sudo npx jarvis install-deps ${browserName}`);
    }
...
async function createUserDataDir(config: FullConfig, clientInfo: ClientInfo) {
  const dir = process.env.PWMCP_PROFILES_DIR_FOR_TEST ?? path.join(defaultCacheDirectory(), 'ms-jarvis-mcp');
...
function throwIfExecutableMissing(error: Error, config: FullConfig): void {
  ...
  const command = config.skillMode ? `jarvis-cli install-browser ${target}` : `npx @jarvis-browser/mcp install-browser ${target}`;
