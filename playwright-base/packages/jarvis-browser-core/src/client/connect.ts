...
export async function connectToBrowser(jarvis: Jarvis, params: ConnectOptions): Promise<Browser> {
  const deadline = params.timeout ? monotonicTime() + params.timeout : 0;
  const nameParam = params.browserName ? { 'x-jarvis-browser': params.browserName } : {};
  ...
  const connection = await connectToEndpoint(jarvis._connection, connectParams, { signal: undefined, timeout: params.timeout || 0 });
  let browser: Browser;
  connection.on('close', () => {
  ...
  const result = await raceAgainstDeadline(async () => {
    ...
    const jarvis = await connection!.initializeJarvis();
    if (!jarvis._initializer.preLaunchedBrowser) {
      connection.close();
      throw new Error('Malformed endpoint. Did you use BrowserType.launchServer method?');
    }
    jarvis.selectors = jarvis.selectors;
    browser = Browser.from(jarvis._initializer.preLaunchedBrowser!);
