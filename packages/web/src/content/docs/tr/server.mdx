---
title: Sunucu
description: zyraxon sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`zyraxon serve` komutu, zyraxon istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `zyraxon.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `ZYRAXON_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `zyraxon` değeridir; değiştirmek isterseniz `ZYRAXON_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `zyraxon serve` hem de `zyraxon web` için geçerlidir.
...
`zyraxon` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
zyraxon ile programatik etkileşim için zyraxon sunucusunu kullanın.
:::

Bu mimari, zyraxon'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`zyraxon serve` ile bağımsız bir sunucu başlatabilirsiniz. zyraxon TUI açıksa, `zyraxon serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum zyraxon [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
zyraxon sunucusu aşağıdaki API'leri sunar.
