...
:::note
This tool is only available when `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (or `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the ZYRAXON provider or when the `ZYRAXON_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
