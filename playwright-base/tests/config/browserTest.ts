...
import type { RemoteServerOptions, JarvisServer } from './remoteServer';
import type { BrowserContext, BrowserContextOptions, BrowserType, Page } from 'jarvis-browser-core';
...
interface StartRemoteServer {
  (kind: 'run-server' | 'launchServer', options?: RemoteServerOptions): Promise<JarvisServer>;
...
const test = baseTest.extend<BrowserTestTestFixtures & { _contextFactory: ContextFactory }, BrowserTestWorkerFixtures>({
  ...
  browserType: [async ({ jarvis, browserName, mode }, run) => {
    await run(jarvis[browserName]);
  }, { scope: 'worker' }],
  ...
  createUserDataDir: async ({ mode }, run) => {
    ...
    await run(async () => {
      const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'jarvis-browser-test-'));
      dirs.push(dir);
      return dir;
    });
