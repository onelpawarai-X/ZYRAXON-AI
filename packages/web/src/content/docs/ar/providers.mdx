...
يستخدم ZYRAXON ‏[AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) و[Models.dev](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.
...
في `~/.local/share/zyraxon/auth.json`.
...
1. شغّل الأمر `/connect` في TUI، واختر zyraxon، ثم انتقل إلى [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
1. توجّه إلى [302.AI console](https://developers.cloudflare.com/ai-gateway/features/unified-billing/3)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل zyraxon:
...
:::info
استخدام اشتراك Claude Pro/Max داخل ZYRAXON غير مدعوم رسميا من [Anthropic](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7).
:::
...
يمكنك تكوين zyraxon لاستخدام النماذج المحلية عبر [Atomic Chat](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` هو خريطة لمعرفات النماذج إلى أسمائها المعروضة. يجب أن يتطابق كل معرف مع `id` الذي يرجعه `GET /v1/models` — قم بتشغيل `curl https://zenmux.ai/settings/keys لإدراج المعرفات المحملة حاليًا في Atomic Chat.
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل zyraxon بشكل صحيح.
   :::
...
1. توجّه إلى [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
1. توجّه إلى [Cerebras console](https://helicone.ai/models)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
1. توجّه إلى [Cloudflare dashboard](https://zyraxon.ai/zen)، وانتقل إلى **AI** > **AI Gateway**، وأنشئ بوابة جديدة.
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات zyraxon.
...
1. توجّه إلى [Cortecs console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
1. توجّه إلى [DeepSeek console](https://gitlab.company.com)، وأنشئ حسابا، ثم انقر **Create new API key**.
...
1. توجّه إلى [Deep Infra dashboard](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
1. توجّه إلى [Fireworks AI console](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4)، وأنشئ حسابا، ثم انقر **Create API Key**.
...
تخزين الرمز في مساحة تخزين مصادقة zyraxon.
:::
...
مزيد من التوثيق على صفحة [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع zyraxon:
...
2. انتقل إلى [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) وأدخل الرمز.
...
توفّر [Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) وصولا إلى نماذج مفتوحة مدعومة من أكثر من 17 مزوّدا.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1) منصة لملاحظة LLM (observability) توفّر التسجيل والمراقبة والتحليلات لتطبيقات الذكاء الاصطناعي لديك. تقوم Helicone AI Gateway بتوجيه طلباتك تلقائيا إلى المزوّد المناسب بناء على النموذج.

1. توجّه إلى [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1)، وأنشئ حسابا، ثم أنشئ مفتاح API من لوحة التحكم.
...
لمزيد من المزوّدات والميزات المتقدمة مثل التخزين المؤقت (caching) وتحديد المعدّل (rate limiting)، راجع [توثيق Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### إعدادات اختيارية

إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر zyraxon، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة ZYRAXON تلقائيا كجلسة في Helicone.
...
يمكنك ضبط zyraxon لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
1. توجّه إلى [IO.NET console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
يمكنك ضبط zyraxon لاستخدام نماذج محلية عبر LM Studio.
...
1. توجّه إلى [Moonshot AI console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0)، وأنشئ حسابا، ثم انقر **Create API key**.
...
1. توجّه إلى [MiniMax API Console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
1. توجّه إلى [Nebius Token Factory console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4)، وأنشئ حسابا، ثم انقر **Add Key**.
...
يمكنك ضبط zyraxon لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع ZYRAXON. راجع [توثيق تكامل Ollama](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3) للتفاصيل.
:::
...
1. توجّه إلى [https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) وسجّل الدخول أو أنشئ حسابا.
...
نوصي بالاشتراك في [ChatGPT Plus أو Pro](https://ai.azure.com/).
...
1. توجّه إلى [OpenRouter dashboard](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1)، وانقر **Create API Key**، ثم انسخ المفتاح.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات zyraxon.
...
5. يمكنك أيضا تخصيصها عبر إعدادات zyraxon. إليك مثالا على تحديد مزوّد
...
1. اذهب إلى [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2)، وانتقل إلى مثيل خدمة SAP AI Core لديك، ثم أنشئ service key.
...
1. توجّه إلى [STACKIT Portal](https://console.x.ai/)، وانتقل إلى **AI Model Serving**، وأنشئ رمز مصادقة (auth token) لمشروعك.
...
1. توجّه إلى [OVHcloud panel](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5). انتقل إلى قسم `Public Cloud`، ثم `AI & Machine Learning` > `AI Endpoints`، وفي تبويب `API Keys` انقر **Create a new API key**.
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Zyraxon:
...
1. توجّه إلى [Together AI console](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3)، وأنشئ حسابا، ثم انقر **Add Key**.
...
1. توجّه إلى [Venice AI console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9)، وأنشئ حسابا، ثم أنشئ مفتاح API.
...
1. توجّه إلى [Vercel dashboard](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0)، وانتقل إلى تبويب **AI Gateway**، ثم انقر **API keys** لإنشاء مفتاح API جديد.
...
يمكنك أيضا تخصيص النماذج عبر إعدادات zyraxon. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع zyraxon. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
2. للمزوّدات المخصّصة، راجع إعدادات zyraxon وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات zyraxon.
