...
import type { BrowserType, Browser } from 'jarvis-browser-core';
...
export class RunServer implements JarvisServer {
  ...
  async start(childProcess: CommonFixtures['childProcess'], options?: { mode?: 'extension' | 'default', env?: NodeJS.ProcessEnv, artifactsDir?: string }) {
    const command = ['node', path.join(__dirname, '..', '..', 'packages', 'jarvis-browser-core', 'cli.js'), 'run-server'];
...
export class RemoteServer implements JarvisServer {
  private _process!: TestChildProcess;
  ...
  async _start(childProcess: CommonFixtures['childProcess'], browserType: BrowserType, channel: string, remoteServerOptions: RemoteServerOptions = {}) {
    ...
    const browserOptions = (browserType as any)._jarvis._defaultLaunchOptions;
