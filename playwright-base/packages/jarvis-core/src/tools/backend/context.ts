...
export type RouteEntry = {
  ...
  handler: (route: jarvisTypes.Route) => Promise<void>;
};
...
export class Context {
  ...
  private _rawBrowserContext: jarvisTypes.BrowserContext;
  private _browserContextPromise: Promise<jarvisTypes.BrowserContext> | undefined;
  private _tabs: Tab[] = [];
  ...
  constructor(browserContext: jarvisTypes.BrowserContext, options: ContextOptions) {
  ...
  private async _startPageVideo(page: jarvisTypes.Page) {
    if (!this._video)
      return;
  ...
  private _onPageCreated(page: jarvisTypes.Page) {
  ...
  private async _setupRequestInterception(context: jarvisTypes.BrowserContext) {
  ...
  async ensureBrowserContext(): Promise<jarvisTypes.BrowserContext> {
  ...
  private async _initializeBrowserContext() {
    if (this.config.testIdAttribute)
      jarvis.selectors.setTestIdAttribute(this.config.testIdAttribute);
...
export function outputDir(options: ContextOptions): string {
  ...
  const baseName = options.config.skillMode ? '.jarvis-cli' : '.jarvis-mcp';
