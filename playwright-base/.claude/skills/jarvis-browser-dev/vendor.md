...
|---|---|---|
| `lib/utilsBundle.js` | `src/utilsBundle.ts` | Vendored npm packages (`debug`, `mime`, `ws`, `yauzl`, `yazl`, `@modelcontextprotocol/sdk`, `graceful-fs`, …). The single home for third-party runtime code in jarvis-browser-core. |
| `lib/coreBundle.js` | `src/coreBundle.ts` | Re-exports of jarvis-browser-core's own modules (`client`, `iso`, `utils`, `cli`, `server`, `registry`, …) as namespaces. Inlines almost all jarvis-browser-core source except `utilsBundle`. |
...
vendored npm imports at **bundle time**. For example, a jarvis-browser-core
...
(`jarvis-browser-test`, `html-reporter`, `trace-viewer`, …) are compiled by
...
`packages/jarvis-browser-core/src/server/utils/third_party/` (e.g.
...
- **Through `coreBundle.utils`.** Re-exported from
  ...
  `require('jarvis-browser-core/lib/coreBundle').utils.extractZip`.
...
- **`externalizeUtilsBundlePlugin`** — matches any relative specifier
  ...
  `jarvis-browser-core/src/` (different depths) and needs a single consistent
