...
1. **Remote config**(`.well-known/zyraxon`) - 조직 기본값
2. **Global config**(`~/.config/zyraxon/zyraxon.json`) - 사용자 기본 설정
3. **Custom config**(`ZYRAXON_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `zyraxon.json`) - 프로젝트별 설정
5. **`.zyraxon` directories** - agents, commands, plugins
6. **Inline config**(`ZYRAXON_CONFIG_CONTENT` env var) - 런타임 override

즉, 프로젝트 config는 전역 기본값을 override할 수 있고, 전역 config는 조직의 Remote 기본값을 override할 수 있습니다.

:::note
`.zyraxon` 및 `~/.config/zyraxon` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
조직은 `.well-known/zyraxon` endpoint로 기본 config를 제공할 수 있습니다. 이를 지원하는 provider로 인증하면 자동으로 가져옵니다.
...
전역 ZYRAXON config는 `~/.config/zyraxon/zyraxon.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.

TUI 관련 설정은 `~/.config/zyraxon/tui.json`을 사용하세요.
...
`ZYRAXON_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`ZYRAXON_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.zyraxon` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
custom 디렉토리는 전역 config와 `.zyraxon` 디렉토리 뒤에 로드되므로 해당 설정을 **override할 수 있습니다**.
...
`ZYRAXON_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.
...
- `mdnsDomain` - mDNS service의 custom 도메인 이름입니다. 기본값은 `zyraxon.local`입니다. 같은 네트워크에서 여러 인스턴스를 실행할 때 유용합니다.
...
`~/.config/zyraxon/agents/` 또는 `.zyraxon/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
`~/.config/zyraxon/commands/` 또는 `.zyraxon/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
plugin 파일은 `.zyraxon/plugins/` 또는 `~/.config/zyraxon/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
