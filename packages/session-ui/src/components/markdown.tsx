...
function embedYouTube(root: HTMLDivElement) {
  ...
  while ((node = walker.nextNode() as Text | null)) {
    if (node.textContent && youtubeRegex.test(node.textContent)) {
      textNodes.push(node)
    }
  }
...
function makeImagesExpandable(root: HTMLDivElement) {
  const images = Array.from(root.querySelectorAll("img"))
  for (const img of images) {
    if (img.closest("[data-component]")) continue
    if (img.parentElement instanceof HTMLAnchorElement) continue
    img.setAttribute("data-expandable", "true")
    img.style.cursor = "zoom-in"
    img.style.transition = "transform 0.2s ease"
    img.addEventListener("click", () => {
      const overlay = document.createElement("div")
      overlay.setAttribute("data-component", "image-overlay")
      overlay.style.cssText = "position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;cursor:zoom-out;backdrop-filter:blur(4px);"
      const fullImg = document.createElement("img")
      fullImg.src = img.src
      fullImg.alt = img.alt
      fullImg.style.cssText = "max-width:90vw;max-height:90vh;object-fit:contain;border-radius:8px;box-shadow:0 8px 32px rgba(0,0,0,0.5);"
      overlay.appendChild(fullImg)
      overlay.addEventListener("click", () => overlay.remove())
      const closeBtn = document.createElement("button")
      closeBtn.textContent = "\u00D7"
      closeBtn.style.cssText = "position:absolute;top:16px;right:16px;background:rgba(255,255,255,0.15);border:none;color:white;font-size:28px;width:40px;height:40px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;"
