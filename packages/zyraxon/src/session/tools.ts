...
import { omniBeforeToolCall, omniAfterToolCall } from "@/omni/lifecycle"
      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            // OMNI-KERNEL: processBeforeToolCall — secret redaction + prediction
            const beforeResult = yield* Effect.promise(() =>
              omniBeforeToolCall(item.id, args, ctx.sessionID),
            )
            const sanitizedArgs = beforeResult.sanitizedArgs ?? args
            ...
            const result = yield* item.execute(sanitizedArgs, ctx)
            const output = {
              ...result,
            ...
            // OMNI-KERNEL: processAfterToolCall — memory + learning + repair
            const toolLatency = Date.now() - toolStartTime
            const success = !output.error
            yield* Effect.promise(() =>
              omniAfterToolCall(item.id, output, success, toolLatency, ctx.sessionID),
            )

            yield* plugin.trigger(
              "tool.execute.after",
              { tool: item.id, sessionID: ctx.sessionID, callID: ctx.callID, args: sanitizedArgs },
              output,
            )
