...
:::note
Dette verktøyet er bare tilgjengelig når `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (eller `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker ZYRAXON-leverandøren eller når miljøvariabelen `ZYRAXON_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
