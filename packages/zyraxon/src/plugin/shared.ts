...
export const DEPRECATED_PLUGIN_PACKAGES = ["zyraxon-openai-codex-auth", "zyraxon-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, zyraxonVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(zyraxonVersion) || semver.major(zyraxonVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.zyraxon
  if (typeof range !== "string") return
  if (!semver.satisfies(zyraxonVersion, range)) {
    throw new Error(`Plugin requires zyraxon ${range} but running ${zyraxonVersion}`)
  }
}
