...
Per impostazione predefinita, ZYRAXON usa il tema `zyraxon`.
...
2. **Directory di configurazione utente** - in `~/.config/zyraxon/themes/*.json` o `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.zyraxon/themes/*.json`
4. **Directory di lavoro corrente** - in `./.zyraxon/themes/*.json`
