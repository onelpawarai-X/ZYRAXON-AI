...
import { remote } from 'jarvis-browser-core/lib/coreBundle';
...
let showBrowserServer: remote.JarvisServer | undefined;
...
function printConfiguration(options: WatchModeOptions, title?: string) {
  ...
  const tokens: string[] = [];
  tokens.push(`${packageManagerCommand} jarvis test`);
  if (options.projects)
...
async function toggleShowBrowser() {
  if (!showBrowserServer) {
    showBrowserServer = new remote.JarvisServer({ mode: 'extension', path: '/' + createGuid(), maxConnections: 1 });
