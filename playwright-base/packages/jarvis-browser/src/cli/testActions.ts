...
export async function runTests(args: string[], opts: { [key: string]: any }) {
  ...
  if (opts.ui || opts.uiHost || opts.uiPort) {
    if (opts.onlyChanged)
      throw new Error(`--only-changed is not supported in UI mode. If you'd like that to change, see https://github.com/zyraxon-ai/jarvis-browser/issues/15075 for more details.`);
