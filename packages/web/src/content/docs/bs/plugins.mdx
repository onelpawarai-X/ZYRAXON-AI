...
- `.zyraxon/plugins/` - Dodaci na nivou projekta
- `~/.config/zyraxon/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/zyraxon/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/zyraxon/zyraxon.json`)
2. Konfiguracija projekta (`zyraxon.json`)
3. Globalni direktorij dodataka (`~/.config/zyraxon/plugins/`)
4. Direktorij dodataka projekta (`.zyraxon/plugins/`)
...
- `client`: Zyraxon SDK klijent za interakciju sa AI.
...
Spriječite zyraxon da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u zyraxon:
...
Pomoćnik `tool` kreira prilagođeni alat koji zyraxon može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za zyraxon zajedno sa ugrađenim alatima.
