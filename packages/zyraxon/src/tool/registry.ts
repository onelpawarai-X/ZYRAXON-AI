...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@zyraxon-ai/plugin"
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        // Register all X tools from x-tool-registry (dynamic count via xToolRegistry.length)
        ...
        // Register ULTRA tools from server.ts — permission-controlled privileged tools
        const { ULTRA_TOOLS, OMNI_TOOLS } = yield* Effect.promise(() => import("@/mcp/server"))
        for (const [name, entry] of Object.entries(ULTRA_TOOLS)) {
          const ultraPluginDef: ToolDefinition = {
            description: entry.description,
            args: { input: z.string().describe("JSON input for the ultra tool") },
            execute: async (args: any) => {
              try {
                const input = typeof args.input === "string" ? JSON.parse(args.input) : (args.input ?? {})
                const result = await entry.execute(input)
                return {
                  output: result.output || (result.success ? "Operation completed" : result.error || "Unknown error"),
                  metadata: { ultra: true, permission: entry.permission, privileged: entry.privileged },
                }
              } catch (e: any) {
                return { output: `Ultra tool error: ${e?.message ?? String(e)}`, metadata: { ultra: true, error: true } }
              }
            },
          }
          custom.push(fromPlugin(`ultra_${name}`, ultraPluginDef))
        }

        // Register OMNI tools from server.ts — permission-controlled mechanism tools
        for (const [name, entry] of Object.entries(OMNI_TOOLS)) {
          const omniPluginDef: ToolDefinition = {
            description: entry.description,
            args: { input: z.string().describe("JSON input for the omni mechanism tool") },
            execute: async (args: any) => {
              try {
                const input = typeof args.input === "string" ? JSON.parse(args.input) : (args.input ?? {})
                const result = await entry.execute(input)
                return {
                  output: result.output || (result.success ? "Operation completed" : result.error || "Unknown error"),
                  metadata: { omni: true, permission: entry.permission, privileged: entry.privileged },
                }
              } catch (e: any) {
                return { output: `Omni tool error: ${e?.message ?? String(e)}`, metadata: { omni: true, error: true } }
              }
            },
          }
          custom.push(fromPlugin(`omni_${name}`, omniPluginDef))
        }
