...
export class Response {
  ...
  private _includeSnapshotRoot: jarvis.Locator | undefined;
  ...
  private _computeRelativeTo(fileName: string): string {
    ...
    // the auto-named `.jarvis-cli/` artifact directory.
  ...
  setIncludeFullSnapshot(includeSnapshotFileName?: string, root?: jarvis.Locator, depth?: number, boxes?: boolean) {
  ...
  private async _build(): Promise<Section[]> {
    ...
    if (codegen !== 'none' && this._code.length) {
      ...
      addSection('Ran Jarvis code', code, codegen === 'typescript' ? 'js' : codegen);
    }
...
export function parseResponse(response: CallToolResult, cwd?: string) {
  ...
  const code = sections.get('Ran Jarvis code');
