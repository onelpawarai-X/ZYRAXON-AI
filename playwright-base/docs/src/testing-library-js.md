...
:::note
If you use DOM Testing Library in the browser (for example, you bundle end-to-end tests with webpack), you can switch directly to Jarvis Browser Test. Examples below are focused on component tests, but for end-to-end test you just need to replace `await mount` with `await page.goto('http://localhost:3000/')` to open the page under test.
:::
...
Line-by-line migration to Jarvis Browser Test. First, the scenario moves from the test into a story next to the component:
...
```js
const { test, expect } = require('@jarvis-browser/test'); // 2
...
Migration highlights (see inline comments in the Jarvis Browser Test code snippets):
...
1. Import everything from `@jarvis-browser/test`, for both component and end-to-end tests.
1. Test function is given a `page` that is isolated from other tests, and `mount` that renders a story in this page. These are two of the [useful fixtures](./api/class-fixtures) in Jarvis Browser Test.
