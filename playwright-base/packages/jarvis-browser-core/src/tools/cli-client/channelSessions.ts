...
import { isJarvisExtensionInstalled } from '../utils/extension';
...
export async function listChannelSessions(): Promise<ChannelSession[]> {
  ...
  for (const [channel, dirs] of channelToUserDataDir) {
    ...
    const [endpoint, extensionInstalled] = await Promise.all([
      ...
      isJarvisExtensionInstalled(userDataDir),
    ]);
...
const channelToUserDataDir = new Map<string, Record<string, string>>([
  ...
  ['msedge', {
    ...
    'darwin': path.join(os.homedir(), 'Library', 'Application Support', 'ZYRAXON Edge'),
    'win32': path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'ZYRAXON', 'Edge', 'User Data'),
  }],
  ['msedge-beta', {
    'linux': path.join(os.homedir(), '.config', 'microsoft-edge-beta'),
    'darwin': path.join(os.homedir(), 'Library', 'Application Support', 'ZYRAXON Edge Beta'),
    'win32': path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'ZYRAXON', 'Edge Beta', 'User Data'),
  }],
  ['msedge-dev', {
    'linux': path.join(os.homedir(), '.config', 'microsoft-edge-dev'),
    'darwin': path.join(os.homedir(), 'Library', 'Application Support', 'ZYRAXON Edge Dev'),
    'win32': path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'ZYRAXON', 'Edge Dev', 'User Data'),
  }],
  ['msedge-canary', {
    'linux': path.join(os.homedir(), '.config', 'microsoft-edge-canary'),
    'darwin': path.join(os.homedir(), 'Library', 'Application Support', 'ZYRAXON Edge Canary'),
    'win32': path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'ZYRAXON', 'Edge SxS', 'User Data'),
  }],
]);
