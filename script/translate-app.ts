...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "zyraxon/gpt-5.5" },
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Zyraxon did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Zyraxon returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Zyraxon session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Zyraxon runs for 'all' (default: 4)
      --model <provider/id>  Zyraxon model (default: zyraxon/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Zyraxon
...
`)
  ...
  if (failed.length) console.error(`\nZyraxon failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  const exported = Bun.spawn(["zyraxon", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.ZYRAXON_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["zyraxon", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
