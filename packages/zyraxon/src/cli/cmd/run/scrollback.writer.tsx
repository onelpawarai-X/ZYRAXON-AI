...
export function RunEntryContent(props: {
  ...
  const image = createMemo(() => {
    const next = body()
    return next.type === "image" ? next : undefined
  })
  const audio = createMemo(() => {
    const next = body()
    return next.type === "audio" ? next : undefined
  })
  const video = createMemo(() => {
    const next = body()
    return next.type === "video" ? next : undefined
  })
  const generating = createMemo(() => {
    const next = body()
    return next.type === "generating" ? next : undefined
  })
    ...
    <Switch fallback={null}>
      ...
      <Match when={image()}>
        <box width="100%" flexDirection="column" gap={1}>
          ...
          <text width="100%" wrapMode="word" fg={color()}>
            {image()!.url}
          </text>
          <text width="100%" wrapMode="word" fg={theme().block.muted}>
            {`Type: ${image()!.mime}`}
      ...
      <Match when={audio()}>
        <box width="100%" flexDirection="column" gap={1}>
          <text width="100%" wrapMode="word" fg={theme().block.muted}>
            {audio()!.filename ? `[Audio: ${audio()!.filename}]` : "[Audio]"}
          </text>
          <text width="100%" wrapMode="word" fg={color()}>
            {audio()!.url}
          </text>
      ...
      <Match when={video()}>
        <box width="100%" flexDirection="column" gap={1}>
          <text width="100%" wrapMode="word" fg={theme().block.muted}>
            {video()!.filename ? `[Video: ${video()!.filename}]` : "[Video]"}
          </text>
          <text width="100%" wrapMode="word" fg={color()}>
            {video()!.url}
          </text>
      ...
      <Match when={generating()}>
        <box width="100%" flexDirection="column" gap={1}>
          <text width="100%" wrapMode="word" fg={theme().block.warning}>
            {generating()!.category === "image" && "✦ Generating image..."}
            {generating()!.category === "video" && "✦ Generating video..."}
            {generating()!.category === "audio" && "✦ Generating audio..."}
            {generating()!.category === "tts" && "✦ Speaking..."}
          </text>
          <text width="100%" wrapMode="word" fg={theme().block.muted}>
