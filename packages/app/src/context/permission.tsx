...
function createServerPermissionState(input: { sdk: ServerSDK; sync: ServerSync }) {
  const [store, setStore, _, ready] = persisted(
    {
      ...Persist.serverGlobal(input.sdk.scope, "permission", ["permission.v3", "permission.v4"]),
      migrate(value) {
        if (!value || typeof value !== "object" || Array.isArray(value)) return value

        const data = value as Record<string, unknown>
        if (data.autoAccept && typeof data.autoAccept === "object") {
        ...
        return {
          ...data,
          autoAccept: {} as Record<string, string>,
        }
      },
    },
    createStore({
  ...
  function deny(sessionID: string, directory?: string) {
    if (meta.disposed) return
  ...
  const api = {
    ...
    getPermissionMode(sessionID: string, directory?: string): string {
      ...
      const value = sessionAutoAccept(store.autoAccept, sessions(directory), { sessionID }, directory)
      return value ?? "allow"
    },
    setPermissionMode(sessionID: string, directory: string, mode: "deny" | "always" | "allow") {
      if (meta.disposed) return
