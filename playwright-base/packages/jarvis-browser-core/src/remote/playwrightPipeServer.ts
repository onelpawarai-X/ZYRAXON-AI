...
import { JarvisConnection } from './jarvisConnection';
import { SocketServerTransport } from './serverTransport';
import { Browser } from '../server/browser';

import type { JarvisInitializeResult } from './jarvisConnection';

export class JarvisPipeServer {
  private _server: net.Server | undefined;
  private _connections = new Set<JarvisConnection>();
  ...
  async listen(pipeName: string) {
    ...
    this._server = net.createServer(socket => {
      ...
      const connection = new JarvisConnection(
          new Semaphore(1),
          transport,
          false,
          this._browser.attribution.jarvisBrowser,
