...
ZYRAXON usa el [AI SDK](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0) y [Models.dev](https://docs.helicone.ai/helicone-headers/header-directory6) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.
...
1. Ejecute el comando `/connect` en TUI, seleccione zyraxon y diríjase a [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1).
...
1. Ejecute el comando `/connect` en TUI, seleccione `ZYRAXON Go` y diríjase a [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5).
...
1. Dirígete a la [consola 302.AI](https://docs.helicone.ai/helicone-headers/header-directory9), crea una cuenta y genera una clave API.
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta zyraxon:
...
Puedes configurar zyraxon para usar modelos locales mediante [Atomic Chat](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
- `models` es un mapa de IDs de modelos a sus nombres de pantalla. Cada ID debe coincidir con el `id` devuelto por `GET /v1/models` — ejecuta `curl https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`4 para listar los IDs cargados actualmente en Atomic Chat.
...
1. Diríjase al [portal de Azure](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4) y cree un recurso **Azure OpenAI**. Necesitarás:
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que zyraxon funcione correctamente.
   :::
...
1. Dirígete a [Baseten](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com3), crea una cuenta y genera una clave API.
...
1. Dirígete a la [consola Cerebras](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`8), crea una cuenta y genera una clave API.
...
1. Dirígete al [panel de Cloudflare](https://api.together.ai), navega hasta **AI** > **AI Gateway** y crea una nueva puerta de enlace.
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración zyraxon.
...
1. Dirígete a la [consola de Cortecs](https://docs.helicone.ai/helicone-headers/header-directory1), crea una cuenta y genera una clave API.
...
1. Dirígete a la [consola de DeepSeek](https://gitlab.company.com), crea una cuenta y haz clic en **Crear nueva clave API**.
...
1. Dirígete al [panel de Deep Infra](https://ollama.com/), crea una cuenta y genera una clave API.
...
1. Dirígete al [Panel de frogbot](https://console.groq.com/), crea una cuenta y genera una clave API.
...
para almacenar el token en el almacenamiento de autenticación zyraxon.
:::
...
URL de devolución de llamada `https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`6 y siguientes ámbitos:
...
Más documentación en la página de inicio de [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
```json title="zyraxon.json"
{
  ...
  "provider": {
    "gitlab": {
      "options": {
        "instanceUrl": "https://docs.helicone.ai/helicone-headers/header-directory5"
      }
    }
  }
}
...
Para utilizar su suscripción GitHub Copilot con zyraxon:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta zyraxon.
...
[Helicone](https://docs.helicone.ai/helicone-headers/header-directory0) es una plataforma de observabilidad LLM que proporciona registro, monitoreo y análisis para sus aplicaciones de IA. Helicone AI Gateway enruta sus solicitudes al proveedor apropiado automáticamente según el modelo.

1. Dirígete a [Helicone](https://docs.helicone.ai/helicone-headers/header-directory0), crea una cuenta y genera una clave API desde tu panel.
...
Para obtener más proveedores y funciones avanzadas como almacenamiento en caché y limitación de velocidad, consulte la [documentación de Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0).

#### Configuraciones opcionales

En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de zyraxon, siempre podrá configurarlo usted mismo.

Aquí está el [Directorio de modelos de Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0), lo necesitará para obtener las ID de los modelos que desea agregar.
...
La función [Sesiones](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación ZYRAXON como una sesión en Helicone.
...
Puede configurar zyraxon para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
1. Dirígete a la [consola IO.NET](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/2), crea una cuenta y genera una clave API.
...
Puede configurar zyraxon para usar modelos locales a través de LM Studio.
...
1. Dirígete a la [Consola MiniMax API](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`2), crea una cuenta y genera una clave API.
...
1. Dirígete a la [consola de Nebius Token Factory](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`3), crea una cuenta y haz clic en **Agregar clave**.
...
Puedes configurar zyraxon para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para ZYRAXON. Consulte los [documentos de integración de Ollama](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2) para obtener más detalles.
:::
...
Recomendamos registrarse en [ChatGPT Plus o Pro](https://ai.azure.com/).
...
1. Dirígete al [panel de OpenRouter](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`0), haz clic en **Crear clave API** y copia la clave.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración zyraxon.
...
5. También puedes personalizarlos a través de tu configuración zyraxon. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
1. Vaya a su [SAP BTP Cockpit](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`1), navegue hasta su instancia de servicio SAP AI Core y cree una clave de servicio.
...
1. Diríjase al [Portal STACKIT](https://anthropic.com), navegue hasta **AI Model Serving** y cree un token de autenticación para su proyecto.
...
1. Dirígete al [panel de OVHcloud](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4). Navegue a la sección `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` y en la pestaña `API Keys`, haga clic en **Crear una nueva clave API**.
...
1. Dirígete a la [consola de Venice AI](https://docs.helicone.ai/helicone-headers/header-directory8), crea una cuenta y genera una clave API.
...
1. Dirígete al [panel de Vercel](https://docs.helicone.ai/helicone-headers/header-directory2), navega hasta la pestaña **AI Gateway** y haz clic en **API claves** para crear una nueva clave API.
...
También puede personalizar modelos a través de su configuración zyraxon. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
1. Dirígete a la [consola xAI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com7), crea una cuenta y genera una clave API.
...
1. Dirígete al [panel de ZenMux](https://github.com/login/device), haz clic en **Crear clave API** y copia la clave.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con zyraxon. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
2. Para proveedores personalizados, verifique la configuración zyraxon y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración zyraxon.
