...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\zyraxon\log`
...
zyraxon salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/zyraxon/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\zyraxon`
...
ZYRAXON Desktop esegue in background un server locale di ZYRAXON (il sidecar `zyraxon-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\zyraxon\plugins`
...
- **Linux**: elimina `~/.cache/zyraxon` (oppure esegui `rm -rf ~/.cache/zyraxon`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\zyraxon`
...
Se hai `ZYRAXON_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `ZYRAXON_PORT` (o scegli una porta libera) e riavvia.
...
- `zyraxon.settings.dat` (desktop default server URL)
- `zyraxon.global.dat` e `zyraxon.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\zyraxon`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. zyraxon installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\zyraxon`

2. Riavvia zyraxon per reinstallare i pacchetti provider piu recenti

Questo forzera zyraxon a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
zyraxon rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
