...
import type { LaunchOptions } from 'jarvis-browser-core';

export class Jarvis extends ChannelOwner<channels.JarvisChannel> {
  ...
  constructor(parent: ChannelOwner, type: string, guid: string, initializer: channels.JarvisInitializer) {
    super(parent, type, guid, initializer);
    this.by = by;
    this.request = new APIRequest(this);
    this.chromium = BrowserType.from(initializer.chromium);
    this.chromium._jarvis = this;
    this.firefox = BrowserType.from(initializer.firefox);
    this.firefox._jarvis = this;
    this.webkit = BrowserType.from(initializer.webkit);
    this.webkit._jarvis = this;
    this._android = Android.from(initializer.android);
    this._android._jarvis = this;
    this._electron = Electron.from(initializer.electron);
    this._electron._jarvis = this;
    this.devices = this._connection.localUtils()?.devices ?? {};
    this.selectors = new Selectors();
    this.errors = { TimeoutError };
  }

  static from(channel: channels.JarvisChannel): Jarvis {
