...
:::note
هذه الأداة متاحة فقط عند ضبط `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (أو `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود ZYRAXON أو عند ضبط متغير البيئة `ZYRAXON_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
