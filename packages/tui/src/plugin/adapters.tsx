...
import type { useZyraxonKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useZyraxonKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getZyraxonModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getZyraxonModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
