...
export function frameSnapshotStreamer(snapshotStreamer: string, removeNoScript: boolean) {
  ...
  const kShadowAttribute = '__jarvis_shadow_root_';
  const kValueAttribute = '__jarvis_value_';
  const kCheckedAttribute = '__jarvis_checked_';
  const kSelectedAttribute = '__jarvis_selected_';
  const kScrollTopAttribute = '__jarvis_scroll_top_';
  const kScrollLeftAttribute = '__jarvis_scroll_left_';
  const kStyleSheetAttribute = '__jarvis_style_sheet_';
  const kTargetAttribute = '__jarvis_target__';
  const kCustomElementsAttribute = '__jarvis_custom_elements__';
  const kCurrentSrcAttribute = '__jarvis_current_src__';
  const kBoundingRectAttribute = '__jarvis_bounding_rect__';
  const kPopoverOpenAttribute = '__jarvis_popover_open_';
  const kDialogOpenAttribute = '__jarvis_dialog_open_';

  // Symbols for our own info on Nodes/StyleSheets.
  const kSnapshotFrameId = Symbol('__jarvis_snapshot_frameid_');
  const kCachedData = Symbol('__jarvis_snapshot_cache_');
  const kEndOfList = Symbol('__jarvis_end_of_list_');
  type CachedData = {
  ...
  class Streamer {
    ...
    private _refreshListenersWhenNeeded() {
      ...
      const customEventName = '__jarvis_snapshotter_global_listeners_check__';
    ...
    private _refreshListeners() {
      (document as any).addEventListener('__jarvis_mark_target__', (event: CustomEvent) => {
        const target = event.composedPath()[0] as Element;
        if (target?.nodeType !== Node.ELEMENT_NODE)
          return;
        (target as any).__jarvis_target__ = this._targetGeneration;
      });
      (document as any).addEventListener('__jarvis_reset_targets__', () => {
    ...
    private _ensureObservingCurrentDocument() {
      ...
      // https://github.com/zyraxon-ai/jarvis-browser/issues/40895
    ...
    captureSnapshot(reset?: 'history' | 'targets'): SnapshotData | undefined {
      ...
      const visitNode = (node: Node | ShadowRoot): { equals: boolean, n: NodeSnapshot } | undefined => {
        ...
        // See https://github.com/zyraxon-ai/jarvis-browser/issues/12005.
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          ...
          if ((element as any).__jarvis_target__ === this._targetGeneration) {
