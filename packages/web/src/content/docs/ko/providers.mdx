...
ZYRAXON는 [AI SDK](https://docs.helicone.ai/helicone-headers/header-directory2) 및 [Models.dev](https://docs.helicone.ai/helicone-headers/header-directory5)를 사용하여 **75개 이상의 LLM 공급자**를 지원하며 로컬 모델도 실행할 수 있습니다.
...
1. TUI에서 `/connect` 명령을 실행하고 `ZYRAXON Zen`을 선택한 뒤, [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4)로 이동합니다.
...
1. TUI에서 `/connect` 명령을 실행하고 `ZYRAXON Go`를 선택한 뒤 [zyraxon.ai/auth](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com4)로 이동하십시오.
...
1. [302.AI 콘솔](https://docs.helicone.ai/helicone-headers/header-directory8)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
:::info
ZYRAXON에서 Claude Pro/Max 구독을 사용하는 것은 [Anthropic](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5)에서 공식적으로 지원하지 않습니다.
:::
...
OpenAI 호환 API 서버 뒤에서 로컬 LLM을 실행하는 데스크톱 애플리케이션인 [Atomic Chat](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0)을 통해 로컬 모델을 사용하도록 zyraxon를 구성할 수 있습니다(기본 엔드포인트 `http://127.0.0.1:1337/v1`).
...
- `models`는 모델 ID와 해당 표시 이름의 맵입니다. 각 ID는 `GET /v1/models`가 반환하는 `id`와 일치해야 하며, 현재 Atomic Chat에 로드된 ID 목록을 확인하려면 `curl https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`2 실행하세요.
...
1. [Azure 포털](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4)로 이동하여 **Azure OpenAI** 리소스를 만듭니다. 다음이 필요합니다:
   - **리소스 이름**: API 엔드포인트의 일부가 됩니다 (`https://RESOURCE_NAME.openai.azure.com/`)
   - **API 키**: 리소스의 `KEY 1` 또는 `KEY 2`

2. [Azure AI Foundry](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com7)로 이동하여 모델을 배포합니다.
...
1. [Cerebras 콘솔](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`8)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
1. [Cloudflare 대시보드](https://app.baseten.co/)로 이동하여, **AI** > **AI Gateway**로 가서 새로운 게이트웨이를 만듭니다.
...
1. [Cortecs 콘솔](https://docs.helicone.ai/helicone-headers/header-directory0)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
1. [DeepSeek 콘솔](https://gitlab.company.com)로 이동하여 계정을 만들고 **API Keys**를 클릭하여 키를 생성합니다.
...
1. [Deep Infra 대시보드](https://helicone.ai)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
1. [FrogBot 대시보드](https://console.groq.com/)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
콜백 URL `https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`6 및 다음 범위가 필요합니다:
...
[zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) 홈페이지에 추가 문서가 있습니다.
...
캐싱 및 속도 제한과 같은 더 많은 공급자와 고급 기능은 [Helicone 문서](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0)를 확인하십시오.

#### 선택적 구성

ZYRAXON를 통해 자동으로 구성되지 않는 Helicone의 기능이나 모델이 있는 경우 직접 구성할 수 있습니다.

[Helicone의 모델 디렉토리](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0)에서 추가하려는 모델의 ID를 확인하십시오.
...
Helicone의 [Sessions](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) 기능을 사용하면 관련 LLM 요청을 그룹화할 수 있습니다. [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 플러그인을 사용하여 각 ZYRAXON 대화를 Helicone 세션으로 자동 기록하십시오.
...
1. [IO.NET 콘솔](https://docs.helicone.ai/helicone-headers/header-directory6)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
1. [MiniMax API 콘솔](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`3)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
1. [Nebius Token Factory 콘솔](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`4)로 이동하여 계정을 만들고 **Add Key**를 클릭합니다.
...
:::tip
Ollama는 ZYRAXON에 대해 자동으로 구성될 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2)를 참조하십시오.
:::
...
[ChatGPT Plus 또는 Pro](https://ollama.com/)에 가입하는 것이 좋습니다.
...
1. [OpenRouter 대시보드](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`0)로 이동하여 **Create API Key**를 클릭하고 키를 복사합니다.
...
1. [SAP BTP Cockpit](https://AZURE_COGNITIVE_SERVICES_RESOURCE_NAME.cognitiveservices.azure.com/`1)으로 이동하여 SAP AI Core 서비스 인스턴스로 이동하고 서비스 키를 만듭니다.
...
1. [STACKIT Portal](https://console.x.ai/)로 이동하여 **AI Model Serving**으로 이동한 다음 프로젝트의 인증 토큰을 만듭니다.
...
1. [OVHcloud 패널](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com3)로 이동합니다. `Public Cloud` 섹션으로 이동하여 `AI & Machine Learning` > `AI Endpoints`로 간 뒤 `API Keys` 탭에서 **Create a new API key**를 클릭합니다.
...
1. [Together AI 콘솔](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1)로 이동하여 계정을 만들고 **Add Key**를 클릭합니다.
...
1. [Venice AI 콘솔](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/2)로 이동하여 계정을 만들고 API 키를 생성합니다.
...
1. [Vercel 대시보드](https://docs.helicone.ai/helicone-headers/header-directory1)로 이동하여 **AI Gateway** 탭으로 간 뒤, **API keys**를 클릭하여 새 API 키를 생성합니다.
...
1. [ZenMux 대시보드](https://github.com/login/device)로 이동하여 **Create API Key**를 클릭하고 키를 복사합니다.
