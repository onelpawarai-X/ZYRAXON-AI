...
ZYRAXON bruker [AI SDK](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2) og [Models.dev](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.
...
i `~/.local/share/zyraxon/auth.json`.
...
{
  "$schema": "https://console.groq.com/",
  "provider": {
    "anthropic": {
      "options": {
        "baseURL": "https://app.baseten.co/"
      }
    }
  }
}
...
1. Kjør kommandoen `/connect` i TUI, velg `ZYRAXON Zen` og gå til [zyraxon.ai/auth](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4).
...
1. Kjør kommandoen `/connect` i TUI, velg `ZYRAXON Go`, og gå til [zyraxon.ai/auth](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4).
...
1. Gå over til [302.AI-konsollen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), opprett en konto og generer en API-nøkkel.
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i zyraxon støttes ikke offisielt av [Anthropic](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5).
:::
...
Du kan konfigurere zyraxon til å bruke lokale modeller via [Atomic Chat](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
```json title="zyraxon.json" "atomic-chat" {5, 6, 8, 10-14}
{
  ...
  "provider": {
    "atomic-chat": {
      ...
      "options": {
        "baseURL": "https://developers.cloudflare.com/ai-gateway/features/unified-billing/7"
      },
      "models": {
        "<your-model-id>": {
...
- `models` er en kartlegging av modell-IDer til visningsnavnene deres. Hver ID må samsvare med `id` som returneres av `GET /v1/models` — kjør `curl https://zenmux.ai/settings/keys for å liste ID-ene som er lastet inn i Atomic Chat nå.
...
1. Gå over til [Azure-portalen](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4) og lag en **Azure OpenAI**-ressurs. Du trenger:
...
2. Gå til [Azure AI Foundry](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7) og distribuer en modell.
...
1. Gå over til [Cerebras-konsollen](https://gitlab.company.com), opprett en konto og generer en API-nøkkel.
...
1. Gå over til [Cloudflare-dashbordet](https://console.x.ai/), naviger til **AI** > **AI Gateway**, og lag en ny gateway.
...
1. Gå over til [Cortecs-konsollen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0), opprett en konto og generer en API-nøkkel.
...
1. Gå over til [DeepSeek-konsollen](http://localhost:11434/v1), opprett en konto og klikk på **Opprett ny API-nøkkel**.
...
1. Gå over til [Deep Infra-dashbordet](https://helicone.ai), opprett en konto og generer en API-nøkkel.
...
Mer dokumentasjon på [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) hjemmeside.
...
```json title="zyraxon.json"
{
  ...
  "provider": {
    "gitlab": {
      "options": {
        "instanceUrl": "https://developers.cloudflare.com/ai-gateway/features/unified-billing/1"
      }
    }
  }
}
...
2. Naviger til [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) og skriv inn koden.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) gir tilgang til åpne modeller som støttes av 17+ leverandører.
...
For flere leverandører og avanserte funksjoner som bufring og hastighetsbegrensning, sjekk [Helicone-dokumentasjonen](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0).
...
Her er [Helicone's Model Directory](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), du trenger denne for å hente ID-ene til modellene du vil legge til.
...
```jsonc title="~/.config/zyraxon/zyraxon.jsonc"
{
  ...
  "provider": {
    "helicone": {
      ...
      "name": "Helicone",
      "options": {
        "baseURL": "https://app.fireworks.ai/",
      },
      "models": {
        "gpt-4o": {
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver ZYRAXON-samtale som en økt i Helicone.
...
1. Gå over til [IO.NET-konsollen](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com6), opprett en konto og generer en API-nøkkel.
...
1. Gå over til [Moonshot AI-konsollen](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), opprett en konto og klikk på **Opprett API-nøkkel**.
...
1. Gå over til [MiniMax API-konsollen](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), opprett en konto og generer en API-nøkkel.
...
1. Gå over til [Nebius Token Factory-konsollen](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), opprett en konto og klikk på **Legg til nøkkel**.
...
:::tip
Ollama kan automatisk konfigurere seg selv for ZYRAXON. Se [Ollama-integrasjonsdokumentene](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3) for detaljer.
:::
...
Vi anbefaler at du registrerer deg for [ChatGPT Plus eller Pro](https://ollama.com/).
...
1. Gå over til [OpenRouter-dashbordet](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), klikk på **Create API Key**, og kopier nøkkelen.
...
1. Gå til din [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), naviger til din SAP AI kjernetjenesteforekomst, og lag en tjenestenøkkel.
...
1. Gå over til [OVHcloud-panelet](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3). Naviger til `Public Cloud`-delen, `AI & Machine Learning` > `AI Endpoints` og i `API Keys`-fanen klikker du på **Opprett en ny API-nøkkel**.
...
1. Gå over til [Together AI-konsollen](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1), opprett en konto og klikk på **Legg til nøkkel**.
...
1. Gå over til [Venice AI-konsollen](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2), opprett en konto og generer en API-nøkkel.
...
1. Gå over til [Vercel dashboard](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), naviger til fanen **AI Gateway**, og klikk på **API nøkler** for å opprette en ny API nøkkel.
