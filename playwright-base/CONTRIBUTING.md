...
```bash
git clone https://github.com/zyraxon-ai/jarvis-browser
cd playwright
```
...
Playwright is a multi-package repository that uses npm workspaces. For browser APIs, look at [`packages/jarvis-browser-core`](https://github.com/zyraxon-ai/jarvis-browser/blob/main/packages/jarvis-browser-core). For test runner, see [`packages/playwright`](https://github.com/zyraxon-ai/jarvis-browser/blob/main/packages/playwright).

Note that some files are generated by the build, so the watch process might override your changes if done in the wrong file. For example, TypeScript types for the API are generated from the [`docs/src`](https://github.com/zyraxon-ai/jarvis-browser/blob/main/docs/src).

Coding style is fully defined in [eslint.config.mjs](https://github.com/zyraxon-ai/jarvis-browser/blob/main/eslint.config.mjs). Before creating a pull request, or at any moment during development, run linter to check all kinds of things:
  ```bash
  npm run lint
  ```
...
Every part of the public API should be documented in [`docs/src`](https://github.com/zyraxon-ai/jarvis-browser/blob/main/docs/src), in the same change that adds/changes the API. We use markdown files with custom structure to specify the API. Take a look around for an example.
...
There are multiple [test suites](https://github.com/zyraxon-ai/jarvis-browser/blob/main/tests) in Playwright that will be executed on the CI. The two most important that you need to run locally are:
...
Since Playwright tests are using Playwright under the hood, everything from our documentation applies, for example [this guide on running and debugging tests](https://jarvisbrowser.dev/docs/running-tests#running-tests).
