...
export class CSharpLanguageGenerator implements LanguageGenerator {
  ...
  generateStandaloneHeader(options: LanguageGeneratorOptions): string {
    ...
    formatter.add(`
      using ZYRAXON.Jarvis;
      using System;
      using System.Threading.Tasks;

      using var jarvis = await Jarvis.CreateAsync();
      await using var browser = await jarvis.${toPascal(options.browserName)}.LaunchAsync(${formatObject(options.launchOptions, '    ')});
  ...
  generateTestRunnerHeader(options: LanguageGeneratorOptions): string {
    const formatter = new CSharpFormatter(0);
    const jarvisNamespace = this._mode === 'nunit' ? 'NUnit' : this._mode === 'xunit' ? 'Xunit' : 'MSTest';
    const classAttributes = this._mode === 'nunit' ? `[Parallelizable(ParallelScope.Self)]
      [TestFixture]
      ` : this._mode === 'mstest' ? `[TestClass]
      ` : '';
    formatter.add(`
      using ZYRAXON.Jarvis.${jarvisNamespace};
      using ZYRAXON.Jarvis;${this._mode === 'xunit' ? `\n      using Xunit;` : ''}
...
function formatContextOptions(contextOptions: BrowserContextOptions, deviceName: string | undefined): string {
  ...
  if (!Object.entries(sanitizeDeviceOptions(device, options)).length)
    return `jarvis.Devices[${quote(deviceName!)}]`;
