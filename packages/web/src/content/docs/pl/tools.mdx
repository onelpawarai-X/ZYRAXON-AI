...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (lub `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy ZYRAXON lub gdy zmienna środowiskowa `ZYRAXON_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).
