...
export class WVInterceptableRequest {
  ...
  constructor(session: WVSession, browserContext: WVBrowserContext, frame: frames.Frame | null, event: Protocol.Network.requestWillBeSentPayload, redirectedFrom: WVInterceptableRequest | null, documentId: string | undefined) {
    ...
    if (event.request.postData) {
      ...
      // the Jarvis-patched WebKit build which base64-encodes it.
  ...
  createResponse(responsePayload: Protocol.Network.Response): network.Response {
    ...
    const setCookieSeparator = process.platform === 'darwin' ? ',' : 'jarvis-set-cookie-separator';
...
export class WVRouteImpl implements network.RouteDelegate {
