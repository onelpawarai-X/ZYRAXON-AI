...
افتراضيًا، يستخدم ZYRAXON سمتنا `zyraxon`.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/zyraxon/themes/*.json` أو `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.zyraxon/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.zyraxon/themes/*.json`
