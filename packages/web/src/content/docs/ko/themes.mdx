...
기본적으로 ZYRAXON는 자체 `zyraxon` 테마를 사용합니다.
...
2. **사용자 설정 디렉토리 (User config directory)** - `~/.config/zyraxon/themes/*.json` 또는 `$XDG_CONFIG_HOME/zyraxon/themes/*.json`에 정의됩니다.
3. **프로젝트 루트 디렉토리 (Project root directory)** - `<project-root>/.zyraxon/themes/*.json`에 정의됩니다.
4. **현재 작업 디렉토리 (Current working directory)** - `./.zyraxon/themes/*.json`에 정의됩니다.
