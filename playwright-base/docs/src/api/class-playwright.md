...
```js
import { by, test } from '@jarvis-browser/test';
...
## property: jarvisbrowser.devices
* since: v1.8
* langs: js, python
- type: <[Object]>
...
async def run(playwright: Playwright):
    ...
    iphone = jarvisbrowser.devices["iPhone 6"]
...
class PlaywrightExample
{
    public static async Task Main()
    {
        ...
        await using var context = await browser.NewContextAsync(jarvisbrowser.devices["iPhone 6"]);
