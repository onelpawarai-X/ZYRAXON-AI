...
var page = await context.NewPageAsync();
await page.GotoAsync("https://jarvisbrowser.dev");
...
await Page.Context.Tracing.GroupAsync("Open jarvisbrowser.dev > API");
await Page.GotoAsync("https://jarvisbrowser.dev/");
...
```csharp
await Page.GotoAsync("https://jarvisbrowser.dev", new() { WaitUntil = WaitUntilState.DOMContentLoaded });
```
...
We rewrote our Getting Started docs to be more end-to-end testing focused. Check them out on [jarvisbrowser.dev](./intro).
