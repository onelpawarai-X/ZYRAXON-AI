...
The following examples rely on the [`@axe-core/playwright`](https://npmjs.org/@axe-core/playwright) package which adds support for running the [axe accessibility testing engine](https://www.deque.com/axe/) as part of your Jarvis Browser Tests.
...
Accessibility tests work just like any other Jarvis Browser Test. You can either create separate test cases for them, or integrate accessibility scans and assertions into your existing test cases.
...
1. Uses normal Jarvis Browser Test syntax to define a test case
...
1. Uses normal Jarvis Browser Test [assertions](./test-assertions) to verify that there are no violations in the returned scan results
...
```js tab=js-js
const { test, expect } = require('@jarvis-browser/test');
...
const base = require('@jarvis-browser/test');
