...
:::note
Questo strumento e' disponibile solo quando `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (oppure `ZYRAXON_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider ZYRAXON o quando la variabile d'ambiente `ZYRAXON_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
