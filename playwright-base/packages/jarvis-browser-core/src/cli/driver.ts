...
export function runDriver() {
  ...
  new RootDispatcher(dispatcherConnection, async (rootScope, { sdkLanguage }) => {
    const jarvis = createJarvis({ sdkLanguage, isClientCollocatedWithServer: true });
    return new JarvisDispatcher(rootScope, jarvis);
  });
...
export async function runServer(options: RunServerOptions) {
  ...
  const server = new JarvisServer({ mode: extension ? 'extension' : 'default', path, maxConnections, artifactsDir, unsafe });
...
export async function launchBrowserServer(browserName: string, configFile?: string) {
  ...
  const browserType = (jarvis as any)[browserName] as BrowserType;
