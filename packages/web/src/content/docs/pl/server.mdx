---
title: Serwer
description: Interakcja z serwerem zyraxon za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `zyraxon serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient zyraxon.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `zyraxon.local` |
...
Ustaw `ZYRAXON_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `zyraxon` lub ustaw `ZYRAXON_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `zyraxon serve`, jak i `zyraxon web`.
...
Kiedy uruchomisz `zyraxon`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera zyraxon do programowej interakcji z zyraxon.
:::

Ta architektura umożliwia obsługę wielu klientów przez zyraxon i programową interakcję z kodem otwartym.

Możesz uruchomić `zyraxon serve`, aby uruchomić samodzielny serwer. Jeśli masz
zyraxon TUI działa, `zyraxon serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki zyraxon [IDE](/docs/ide).
...
Serwer zyraxon udostępnia następujące interfejsy API.
