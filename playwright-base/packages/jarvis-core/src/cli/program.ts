...
export function decorateProgram(program: Command) {
  ...
  program
      ...
      .name(buildBaseJarvisCLICommand(process.env.PW_LANG_NAME));
  ...
  commandWithOpenOptions('codegen [url]', 'open page and generate code for user actions',
      [
        ...
        ['--target <language>', `language to generate, one of javascript, jarvis-browser-test, python, python-async, python-pytest, csharp, csharp-mstest, csharp-nunit, csharp-xunit, java, java-junit`, codegenId()],
  ...
  program
      ...
      .description('ensure browsers necessary for this version of Jarvis are installed')
      .option('--with-deps', 'install system dependencies for browsers')
      .option('--dry-run', 'do not execute installation, only print information')
      .option('--list', 'prints list of browsers from all jarvis installations')
  ...
  program
      .command('uninstall')
      .description('Removes browsers used by this installation of Jarvis from the system (chromium, firefox, webkit, ffmpeg). This does not include branded channels.')
      .option('--all', 'Removes all browsers used by any Jarvis installation from the system.')
  ...
  program
      .command('cli')
      .description('run jarvis cli commands from terminal')
  ...
  decorateMCPCommand(program
      .command('mcp')
      .description('run the Jarvis MCP server'));
}
...
function codegenId(): string {
  return process.env.PW_LANG_NAME || 'jarvis-browser-test';
}
...
function commandWithOpenOptions(command: string, description: string, options: any[][]): Command {
  ...
  return result
      ...
      .option('--timeout <timeout>', 'timeout for Jarvis actions in milliseconds, no timeout by default')
      .option('--user-agent <ua string>', 'specify user agent string')
      .option('--user-data-dir <directory>', 'use the specified user data directory instead of a new context')
      .option('--viewport-size <size>', 'specify browser viewport size in pixels, for example "1280, 720"');
}

function buildBaseJarvisCLICommand(cliTargetLang: string | undefined): string {
  switch (cliTargetLang) {
    case 'python':
      return `jarvis`;
    case 'java':
      return `mvn exec:java -e -D exec.mainClass=com.microsoft.jarvis.CLI -D exec.args="...options.."`;
    case 'csharp':
      return `pwsh bin/Debug/netX/jarvis.ps1`;
    default: {
      const packageManagerCommand = getPackageManagerExecCommand();
      return `${packageManagerCommand} jarvis`;
    }
  }
}
