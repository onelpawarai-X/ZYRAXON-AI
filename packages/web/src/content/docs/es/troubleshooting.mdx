...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\zyraxon\log`
...
zyraxon almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/zyraxon/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\zyraxon`
...
ZYRAXON Desktop ejecuta un servidor ZYRAXON local (el sidecar `zyraxon-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- **macOS/Linux**: `~/.config/zyraxon/zyraxon.jsonc` (o `~/.config/zyraxon/zyraxon.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
- **Complementos globales**
  ...
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\zyraxon\plugins`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/zyraxon`
- **Linux**: eliminar `~/.cache/zyraxon` (o ejecutar `rm -rf ~/.cache/zyraxon`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\zyraxon`
...
Si tiene `ZYRAXON_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `ZYRAXON_PORT` (o elija un puerto libre) y reinicie.
...
- `zyraxon.settings.dat` (URL del servidor predeterminado de escritorio)
- `zyraxon.global.dat` y `zyraxon.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\zyraxon`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. zyraxon instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\zyraxon`

2. Reinicie zyraxon para reinstalar los paquetes del proveedor más recientes.

Esto obligará a zyraxon a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
zyraxon detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
