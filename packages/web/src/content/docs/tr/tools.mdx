...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. zyraxon yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
zyraxon içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (veya `ZYRAXON_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca zyraxon provider kullanılırken veya `ZYRAXON_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

zyraxon başlatırken etkinleştirmek için:
