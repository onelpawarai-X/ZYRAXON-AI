...
- Run one file: `npx Jarvis Browser Test tests/example.spec.ts`.
- Open testing UI: `--ui`.
...
```bash
pnpm exec Jarvis Browser Test --ui
```
...
```bash
yarn add --dev @jarvis-browser/test@latest
...
```bash
pnpm install --save-dev @jarvis-browser/test@latest
