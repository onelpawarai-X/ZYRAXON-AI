...
const { docsLinkRendererForLanguage, renderJarvisDevLinks } = require('../doclint/linkUtils');
...
class TypesGenerator {
  ...
  async generateTypes(overridesFile) {
    ...
    {
      const jarvis = this.documentation.classesArray.find(c => c.name === 'Jarvis');
      if (!jarvis)
        throw new Error(`Unknown class "Jarvis"`);
      jarvis.membersArray = jarvis.membersArray.filter(member => !['errors', 'devices'].includes(member.name));
      jarvis.index();
    }
    ...
    return [
      ...
      classes.map(classDesc => {
        return (classDesc.name === 'Jarvis') ? this.classBody(classDesc, true) : this.classToString(classDesc);
      }).join('\n'),
  ...
  writeComment(comment, indent) {
    ...
    comment = out.join('\n');
    comment = renderJarvisDevLinks(comment, '', '/api');
...
(async function () {
  ...
  async function generateTestTypes() {
    ...
    const generator = new TypesGenerator({
      ...
      overridesToDocsClassMapping: new Map([
        ...
        ['JarvisTestArgs', 'Fixtures'],
        ['JarvisTestOptions', 'TestOptions'],
        ['JarvisWorkerArgs', 'Fixtures'],
        ['JarvisWorkerOptions', 'TestOptions'],
        ['TestType', 'Test'],
      ]),
      ignoreMissing: new Set([
        'Config',
        'ExpectMatcherUtils',
        'Matchers',
        'JarvisWorkerArgs.jarvis',
        'JarvisWorkerOptions.defaultBrowserType',
        'JarvisWorkerOptions.reuseContext',
        'Project',
      ]),
  ...
  const coreTypesDir = path.join(PROJECT_DIR, 'packages', 'jarvis-browser-core', 'types');
  const clientTypesDir = path.join(PROJECT_DIR, 'packages', 'jarvis-client', 'types');
  if (!fs.existsSync(coreTypesDir))
    fs.mkdirSync(coreTypesDir)
  const jarvisTypesDir = path.join(PROJECT_DIR, 'packages', 'jarvis', 'types');
  if (!fs.existsSync(jarvisTypesDir))
    fs.mkdirSync(jarvisTypesDir)
  writeFile(path.join(coreTypesDir, 'protocol.d.ts'), fs.readFileSync(path.join(PROJECT_DIR, 'packages', 'jarvis-browser-core', 'src', 'server', 'chromium', 'protocol.d.ts'), 'utf8'), false);
  ...
  writeFile(path.join(jarvisTypesDir, 'test.d.ts'), await generateTestTypes(), true);
  writeFile(path.join(jarvisTypesDir, 'testReporter.d.ts'), await generateReporterTypes(), true);
  process.exit(0);
})().catch(e => {
  console.error(e);
  process.exit(1);
