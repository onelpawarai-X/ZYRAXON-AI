...
_Model Bağlam Protokolü_ veya MCP kullanarak zyraxon'a harici araçlar ekleyebilirsiniz. zyraxon hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [zyraxon Config](https://zyraxon.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/zyraxon` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
zyraxon, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde zyraxon şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa zyraxon, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `zyraxon mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından zyraxon, belirteçleri `~/.local/share/zyraxon/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra zyraxon'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi zyraxon yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve zyraxon'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
...
GitHub'daki kod parçacıkları arasında arama yapmak için [Grep by Vercel](https://github.com/upstash/context71) MCP sunucusunu ekleyin.
