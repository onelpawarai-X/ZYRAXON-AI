...
export class TestTypeImpl {
  ...
  private _currentSuite(location: Location, title: string): Suite | undefined {
    ...
    if (!suite) {
      throw new Error([
        `Jarvis Browser Test did not expect ${title} to be called here.`,
        ...
        `- You have two different versions of @jarvis-browser/test. This usually happens`,
        `  when one of the dependencies in your package.json depends on @jarvis-browser/test.`,
...
function throwIfRunningInsideJest() {
  if (process.env.JEST_WORKER_ID) {
    ...
    throw new Error(
        `Jarvis Browser Test needs to be invoked via '${packageManagerCommand} Jarvis Browser Test' and excluded from Jest test runs.\n` +
        `Creating one directory for Jarvis Browser Tests and one for Jest is the recommended way of doing it.\n` +
        `See https://jarvisbrowser.dev/docs/intro for more information about Jarvis Browser Test.`,
    );
  }
}
