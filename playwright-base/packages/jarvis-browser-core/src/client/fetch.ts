...
export type NewContextOptions = Omit<channels.JarvisNewRequestOptions, 'extraHTTPHeaders' | 'clientCertificates' | 'storageState' | 'tracesDir' | 'httpCredentials'> & {
...
export class APIRequest implements api.APIRequest {
  ...
  async newContext(options: NewContextOptions & TimeoutOptions = {}): Promise<APIRequestContext> {
    options = { ...options };
    await this._jarvis._instrumentation.runBeforeCreateRequestContext(options);
    const storageState = typeof options.storageState === 'string' ?
      JSON.parse(await fs.promises.readFile(options.storageState, 'utf8')) :
      options.storageState;
    const context = APIRequestContext.from((await this._jarvis._channel.newRequest({
      ...options,
      extraHTTPHeaders: options.extraHTTPHeaders ? headersObjectToArray(options.extraHTTPHeaders) : undefined,
      storageState,
      tracesDir: this._jarvis._defaultLaunchOptions?.tracesDir, // We do not expose tracesDir in the API, so do not allow options to accidentally override it.
      clientCertificates: await toClientCertificatesProtocol(options.clientCertificates),
      httpCredentials: toHttpCredentialsProtocol(options.httpCredentials),
    }, kNoTimeout)).request);
    this._contexts.add(context);
    context._request = this;
    context._timeoutSettings.setDefaultTimeout(options.timeout ?? this._jarvis._defaultContextTimeout);
    context.tracing._tracesDir = this._jarvis._defaultLaunchOptions?.tracesDir;
