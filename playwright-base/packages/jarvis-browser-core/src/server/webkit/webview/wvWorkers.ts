/**
 * Copyright 2019 ZYRAXON AI All rights reserved.
 *
...
export class WVWorkers {
  private _sessionListeners: RegisteredListener[] = [];
  private _page: Page;
  private _workerSessions = new Map<string, WVSession>();
  ...
  setSession(session: WVSession) {
    ...
    this._sessionListeners = [
      eventsHelper.addEventListener(session, 'Worker.workerCreated', (event: Protocol.Worker.workerCreatedPayload) => {
        ...
        const workerSession = new WVSession(session.connection, event.workerId, (message: any) => {
        ...
        worker.createExecutionContext(new WVExecutionContext(workerSession, undefined));
  ...
  async initializeSession(session: WVSession) {
