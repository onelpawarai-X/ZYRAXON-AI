...
export function decorateProgram(program: Command) {
  program.argument('[session-name]', 'name of the session to create or connect to', 'default')
      ...
      .option('--config <path>', 'path to the config file; by default uses .jarvis/cli.config.json in the project directory and ~/.jarvis/cli.config.json as global config')
      .option('--cdp <url>', 'connect to an existing browser via CDP endpoint URL')
      .option('--endpoint <endpoint>', 'attach to a running Jarvis browser endpoint')
...
function defaultConfigFile(): string {
  return path.resolve('.jarvis', 'cli.config.json');
}

function globalConfigFile(): string {
  return path.join(process.env['PWTEST_CLI_GLOBAL_CONFIG'] ?? os.homedir(), '.jarvis', 'cli.config.json');
}
...
export async function initWorkspace(initSkills: string | undefined, initSkillsGlobal?: string) {
  ...
  if (!globalSkills) {
    ...
    const jarvisDir = path.join(cwd, '.jarvis');
    await fs.promises.mkdir(jarvisDir, { recursive: true });
  ...
  if (skills) {
    ...
    try {
      await installSkills(['jarvis-cli'], target, { global: globalSkills });
    } catch (error) {
