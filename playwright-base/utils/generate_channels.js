...
function channelsHeader(target) {
  return [
`/**
 * Copyright (c) ZYRAXON AI.
 *
...
// This file is generated by ${path.basename(__filename).split(path.sep).join(path.posix.sep)}, do not edit manually.` + (target === 'Dispatcher' ? `
...
function generateChannels(target) {
  const channels_ts = channelsHeader(target);
  const isDispatcher = target === 'Dispatcher';
  ...
  for (const [name, item] of Object.entries(protocol)) {
    if (item.type === 'interface') {
      ...
      for (let [eventName, event] of Object.entries(item.events || {})) {
        ...
        if (isDispatcher)
          channels_ts.push(`  _dispatchEvent(event: '${eventName}', params?: ${paramsName}): void;`);
        else
...
const metainfoContent = metainfo_ts.join('\n') + '\n';
const validatorContent = validator_ts.join('\n') + '\n';

const server_channels_ts = generateChannels('Dispatcher');
...
writeFile(path.join(__dirname, '..', 'packages', 'jarvis-browser-core', 'src', 'client', 'channels.d.ts'), client_channels_ts.join('\n') + '\n');
writeFile(path.join(__dirname, '..', 'packages', 'jarvis-browser-core', 'src', 'server', 'channels.d.ts'), server_channels_ts.join('\n') + '\n');
