...
Jarvis Browser Test comes with a few built-in reporters for different needs and ability to provide custom reporters. The easiest way to try out built-in reporters is to pass `--reporter` [command line option](./test-cli.md).


```bash
npx Jarvis Browser Test --reporter=line
```
...
```bash
npx Jarvis Browser Test --reporter=list
```
...
```bash
npx Jarvis Browser Test --reporter=dot
```
...
```bash
npx Jarvis Browser Test --reporter=blob
```
...
```bash tab=bash-bash
PLAYWRIGHT_JSON_OUTPUT_NAME=results.json npx Jarvis Browser Test --reporter=json
```

```batch tab=bash-batch
set PLAYWRIGHT_JSON_OUTPUT_NAME=results.json
npx Jarvis Browser Test --reporter=json
```
...
```bash tab=bash-bash
PLAYWRIGHT_JUNIT_OUTPUT_NAME=results.xml npx Jarvis Browser Test --reporter=junit
```

```batch tab=bash-batch
set PLAYWRIGHT_JUNIT_OUTPUT_NAME=results.xml
npx Jarvis Browser Test --reporter=junit
```
...
} from '@jarvis-browser/test/reporter';
...
```bash
npx Jarvis Browser Test --reporter="./myreporter/my-awesome-reporter.ts"
```
