...
import 'jarvis-browser-core/lib/bootstrap';

import { libCli, tools } from 'jarvis-browser-core/lib/coreBundle';
...
function addTestCommand(program: Command) {
  ...
  command.description('run tests with Jarvis Browser Test');
...
Examples:
  $ npx Jarvis Browser Test my.spec.ts
  $ npx Jarvis Browser Test some.spec.ts:42
  $ npx Jarvis Browser Test --headed
  $ npx Jarvis Browser Test --project=webkit`);
}

function addClearCacheCommand(program: Command) {
  const command = program.command('clear-cache');
  command.description('clears build and test caches');
  command.option('-c, --config <file>', `Configuration file, or a test directory with optional "jarvis.config.{m,c}?{js,ts}"`);
...
Examples:
  $ npx jarvis show-report
  $ npx jarvis show-report jarvis-report
  $ npx jarvis show-report jarvis-report.zip`);
}
...
Examples:
  $ npx jarvis merge-reports jarvis-report`);
}
...
function addTestMCPServerCommand(program: Command) {
  ...
  command.action(async options => {
    ...
    const factory: tools.ServerBackendFactory = {
      name: 'Jarvis Browser Test Runner',
      nameInConfig: 'jarvis-browser-test-runner',
...
function addInitSkillsCommand(program: Command) {
  ...
  command.description('Install Jarvis agent skills');
  const option = command.createOption('--loop <loop>', 'Agentic loop provider');
...
const testOptions: [string, { description: string, choices?: string[], preset?: string }][] = [
  ...
  ['-c, --config <file>', { description: `Configuration file, or a test directory with optional "jarvis.config.{m,c}?{js,ts}"` }],
  ['--debug [mode]', { description: `Run tests with Jarvis Inspector. Shortcut for "PWDEBUG=1" environment variable and "--timeout=0 --max-failures=1 --headed --workers=1" options`, choices: ['inspector', 'cli'], preset: 'inspector' }],
  ...
  ['--test-list <file>', { description: `Path to a file containing a list of tests to run. See https://jarvisbrowser.dev/docs/test-cli for more details.` }],
  ['--test-list-invert <file>', { description: `Path to a file containing a list of tests to skip. See https://jarvisbrowser.dev/docs/test-cli for more details.` }],
