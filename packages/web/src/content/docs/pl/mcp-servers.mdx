...
Możesz dodać dodatkowe informacje do zyraxon za pomocą _Model Context Protocol_, w skrócie MCP. zyraxon obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [zyraxon Config](https://zyraxon.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/zyraxon`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
zyraxon automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, zyraxon:
...
Jeśli serwer wymaga uwierzytelnienia, zyraxon poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `zyraxon mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji zyraxon bezpieczne przechowa tokeny w `~/.local/share/zyraxon/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w zyraxon, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie zyraxon, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć zyraxon z kontem Sentry.
...
Dodaj serwer MCP [Grep by Vercel](https://github.com/upstash/context71), aby przeszukać fragmenty kodu w serwisie GitHub.
