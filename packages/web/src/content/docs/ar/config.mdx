...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "zyraxon"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/zyraxon`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/zyraxon/zyraxon.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`ZYRAXON_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`zyraxon.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.zyraxon`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`ZYRAXON_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.zyraxon` و`~/.config/zyraxon` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/zyraxon`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات ZYRAXON العامة في `~/.config/zyraxon/zyraxon.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `ZYRAXON_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `ZYRAXON_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.zyraxon`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.zyraxon`، لذلك **يمكنه تجاوز** إعداداتها.
...
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `zyraxon.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/zyraxon/agents/` أو `.zyraxon/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/zyraxon/commands/` أو `.zyraxon/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
افتراضيًا يسمح zyraxon **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
ضع ملفات الإضافات في `.zyraxon/plugins/` أو `~/.config/zyraxon/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
