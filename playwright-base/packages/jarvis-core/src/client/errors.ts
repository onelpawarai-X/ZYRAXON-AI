...
export class TargetClosedError extends JarvisError {
...
export class AbortError extends JarvisError {
...
export function parseError(error: SerializedError): JarvisError {
  if (!error.error) {
  ...
  if (error.error.name === 'TimeoutError')
    ...
    e = Object.assign(new JarvisError(error.error.message), { name: error.error.name });
