...
`)
  ...
  const zyraxon = await createZyraxon({ port: 0 })
  ...
  try {
    ...
    const session = await zyraxon.client.session.create()
    const result = await zyraxon.client.session
      .prompt({
