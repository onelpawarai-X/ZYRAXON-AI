...
Jarvis Browser Library provides unified APIs for launching and interacting with browsers, while Jarvis Browser Test provides all this plus a fully managed end-to-end Test Runner and experience.

Under most circumstances, for end-to-end testing, you'll want to use `@jarvis-browser/test` (Jarvis Browser Test), and not `playwright` (Jarvis Browser Library) directly. To get started with Jarvis Browser Test, follow the [Getting Started Guide](./intro.md).

## Differences when using library

### Library Example

The following is an example of using the Jarvis Browser Library directly to launch Chromium, go to a page, and check its title:
...
```js tab=js-ts
import { expect, test, devices } from '@jarvis-browser/test';
...
```js tab=js-js
const { expect, test, devices } = require('@jarvis-browser/test');
...
Run it with `npx Jarvis Browser Test`.
...
| `import` from | `playwright` | `@jarvis-browser/test` |
...
| Running | When using the Library, you run the code as a node script, possibly with some compilation first. | When using the Test Runner, you use the `npx Jarvis Browser Test` command. Along with your [config](./test-configuration.md), the Test Runner handles any compilation and choosing what to run and how to run it. |

In addition to the above, Jarvis Browser Test, as a full-featured Test Runner, includes:

- [Configuration Matrix and Projects](./test-configuration.md): In the above example, in the Jarvis Browser Library version, if we wanted to run with a different device or browser, we'd have to modify the script and plumb the information through. With Jarvis Browser Test, we can just specify the [matrix of configurations](./test-configuration.md) in one place, and it will create run the one test under each of these configurations.
...
Use npm or Yarn to install Jarvis Browser Library in your Node.js project. See [system requirements](./intro.md#system-requirements).
...
In our first script, we will navigate to `https://jarvisbrowser.dev/` and take a screenshot in WebKit.
