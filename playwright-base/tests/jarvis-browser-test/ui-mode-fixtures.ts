...
import { cliEntrypoint, test as base, writeFiles, removeFolders } from './jarvis-browser-test-fixtures';
import type { Files, RunOptions } from './jarvis-browser-test-fixtures';
    ...
    .extend<Fixtures>({
      runUITest: async ({ childProcess, headless }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, env: NodeJS.ProcessEnv = {}, options: UIModeOptions = {}) => {
          ...
          await expect(page).toHaveTitle('Jarvis Browser Test');
