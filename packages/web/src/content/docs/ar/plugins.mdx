...
- `.zyraxon/plugins/` - إضافات على مستوى المشروع
- `~/.config/zyraxon/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/zyraxon/node_modules/`.
...
1. إعدادات عامة (`~/.config/zyraxon/zyraxon.json`)
2. إعدادات المشروع (`zyraxon.json`)
3. دليل الإضافات العام (`~/.config/zyraxon/plugins/`)
4. دليل إضافات المشروع (`.zyraxon/plugins/`)
...
- `client`: عميل SDK لـ zyraxon للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع zyraxon.
...
امنع zyraxon من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى zyraxon:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ zyraxon استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ zyraxon إلى جانب الأدوات المضمنة.
