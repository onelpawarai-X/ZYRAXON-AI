...
Create `packages/jarvis-browser-core/src/tools/backend/<your-tool>.ts`.
...
If your tool doesn't fit an existing capability, add a new one to `packages/jarvis-browser-core/src/tools/mcp/config.d.ts`:
...
In `packages/jarvis-browser-core/src/tools/backend/tools.ts`:
...
In `packages/jarvis-browser-core/src/tools/cli-daemon/commands.ts`, use `declareCommand()`:
...
**Categories** (defined in `packages/jarvis-browser-core/src/tools/cli-daemon/command.ts`):
...
1. Add it to `Category` type in `packages/jarvis-browser-core/src/tools/cli-daemon/command.ts`
2. Add it to the `categories` array in `packages/jarvis-browser-core/src/tools/cli-daemon/helpGenerator.ts`:
   ```typescript
...
### 1. Type definition: `packages/jarvis-browser-core/src/tools/mcp/config.d.ts`
...
### 2. CLI options type: `packages/jarvis-browser-core/src/tools/mcp/config.ts`
...
### 5. MCP server CLI: `packages/jarvis-browser-core/src/tools/mcp/program.ts`
...
```
packages/jarvis-browser-core/src/tools/
