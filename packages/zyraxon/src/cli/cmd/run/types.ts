...
import type { ZyraxonClient, PermissionRequest, QuestionRequest, ToolPart } from "@zyraxon-ai/sdk/v2"
import type { TuiConfig } from "@zyraxon-ai/tui/config"
...
type PromptModel = Parameters<ZyraxonClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ZyraxonClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ZyraxonClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ZyraxonClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ZyraxonClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ZyraxonClient["experimental"]["resource"]["list"]>>["data"]>
...
export type RunEntryBody =
  ...
  | { type: "audio"; url: string; mime: string; filename?: string }
  | { type: "video"; url: string; mime: string; filename?: string }
  | { type: "generating"; category: "image" | "video" | "audio" | "tts"; prompt: string; progress?: number }
...
export type PermissionReply = Parameters<ZyraxonClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ZyraxonClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ZyraxonClient["question"]["reject"]>[0]
