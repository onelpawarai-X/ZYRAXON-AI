...
The `{ page }` argument tells Jarvis Browser Test to set up the `page` fixture and provide it to your test function.
...
import type { Page, Locator } from '@jarvis-browser/test';
...
export class TodoPage {
  ...
  async goto() {
    await this.page.goto('https://demo.jarvisbrowser.dev/todomvc/');
  }
...
const { test } = require('@jarvis-browser/test');
...
- Fixtures are **on-demand** - you can define as many fixtures as you'd like, and Jarvis Browser Test will setup only the ones needed by your test and nothing else.
...
import { test as base } from '@jarvis-browser/test';
...
test.use({ baseURL: 'https://jarvisbrowser.dev' });
```
...
Jarvis Browser Test uses [worker processes](./test-parallel.md) to run test files. Similar to how test fixtures are set up for individual test runs, worker fixtures are set up for each worker process. That's where you can set up services, run servers, etc. Jarvis Browser Test will reuse the worker process for as many test files as it can, provided their worker fixtures match and hence environments are identical.
...
```js
import { test as base, expect } from '@jarvis-browser/test';
...
Jarvis Browser Test supports running multiple test projects that can be configured separately. You can use "option" fixtures to make your configuration options declarative and type safe. Learn more about [parameterizing tests](./test-parameterize.md).
...
export default defineConfig({
  use: {
    baseURL: 'https://jarvisbrowser.dev',
  },
});
```
...
test.use({ baseURL: 'https://jarvisbrowser.dev/docs/intro' });

test('check intro contents', async ({ page }) => {
  // This test will use "https://jarvisbrowser.dev/docs/intro" base url as defined above.
});
...
test.describe(() => {
  ...
  test('can navigate to intro from the home page', async ({ page }) => {
    // This test will use "https://jarvisbrowser.dev" base url as defined in the config.
  });
});
```
...
import { test } from './fixtures';
import { expect } from '@jarvis-browser/test';
