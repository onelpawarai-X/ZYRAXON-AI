...
1. **远程配置**（来自 `.well-known/zyraxon`）- 组织默认值
2. **全局配置**（`~/.config/zyraxon/zyraxon.json`）- 用户偏好
3. **自定义配置**（`ZYRAXON_CONFIG` 环境变量）- 自定义覆盖
4. **项目配置**（项目中的 `zyraxon.json`）- 项目特定设置
5. **`.zyraxon` 目录** - 代理、命令、插件
6. **内联配置**（`ZYRAXON_CONFIG_CONTENT` 环境变量）- 运行时覆盖

这意味着项目配置可以覆盖全局默认值，全局配置可以覆盖远程组织默认值。

:::note
`.zyraxon` 和 `~/.config/zyraxon` 目录的子目录使用**复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，也支持单数名称（例如 `agent/`）。
:::
...
组织可以通过 `.well-known/zyraxon` 端点提供默认配置。当您使用支持该功能的提供商进行身份验证时，会自动获取此配置。
...
将全局 ZYRAXON 配置放在 `~/.config/zyraxon/zyraxon.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
使用 `ZYRAXON_CONFIG` 环境变量指定自定义配置文件路径。
...
使用 `ZYRAXON_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.zyraxon` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
...
自定义目录在全局配置和 `.zyraxon` 目录之后加载，因此**可以覆盖**它们的设置。
...
- `mdnsDomain` - mDNS 服务的自定义域名。默认为 `zyraxon.local`。适用于在同一网络上运行多个实例的场景。
...
您还可以使用 `~/.config/zyraxon/agents/` 或 `.zyraxon/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
您还可以使用 `~/.config/zyraxon/commands/` 或 `.zyraxon/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
将插件文件放置在 `.zyraxon/plugins/` 或 `~/.config/zyraxon/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
