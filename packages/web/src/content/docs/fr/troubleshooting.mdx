...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\zyraxon\log`
...
zyraxon stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/zyraxon/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\zyraxon`
...
ZYRAXON Desktop exécute un serveur ZYRAXON local (le side-car `zyraxon-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
- **Plugins mondiaux**
  ...
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\zyraxon\plugins`
...
- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/zyraxon`
- **Linux** : supprimez `~/.cache/zyraxon` (ou exécutez `rm -rf ~/.cache/zyraxon`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\zyraxon`
...
Si `ZYRAXON_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `ZYRAXON_PORT` (ou choisissez un port libre) et redémarrez.
...
- `zyraxon.settings.dat` (serveur de bureau par défaut URL)
- `zyraxon.global.dat` et `zyraxon.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\zyraxon`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. zyraxon installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\zyraxon`

2. Redémarrez zyraxon pour réinstaller les derniers packages du fournisseur

Cela forcera zyraxon à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
zyraxon détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
