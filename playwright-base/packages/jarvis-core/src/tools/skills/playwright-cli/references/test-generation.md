...
Plan / generate / heal lean on the same mechanic: run `npx playwright test --debug=cli` in the background, then `playwright-cli attach tw-XXXX` to drive the paused page interactively. See [jarvis-browser-tests.md](jarvis-browser-tests.md) for the debug/attach mechanics.
...
// seed: tests/seed.spec.ts
import { test, expect } from './fixtures';   // or '@jarvis-browser/test' if no fixtures file
...
- Import from `./fixtures` if the project has one; otherwise `@jarvis-browser/test`.
...
| For... | See |
|---|---|
| `--debug=cli` / attach mechanics | [jarvis-browser-tests.md](jarvis-browser-tests.md) |
