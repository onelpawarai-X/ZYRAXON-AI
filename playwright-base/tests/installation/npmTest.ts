...
import { _baseTest as _test, expect as _expect } from '@jarvis-browser/test';
import fs from 'fs';
import os from 'os';
...
const expect = _expect.extend({
  toHaveLoggedSoftwareDownload(received: string, browsers: ('chromium' | 'chromium-headless-shell' | 'firefox' | 'webkit' | 'winldd' |'ffmpeg')[]) {
    ...
    let index = 0;
    while (true) {
      const match = received.substring(index).match(/\(jarvis (chromium|chromium-headless-shell|firefox|webkit|winldd|ffmpeg) v\d+\) downloaded/im);
      if (!match)
        break;
