...
packages.set('testIsomorphic', packagesDir + '/jarvis/src/isomorphic/');
...
async function checkDeps() {
  ...
  const corePackageJson = await innerCheckDeps(path.join(packagesDir, 'jarvis-browser-core'));
  const jarvisPackageJson = await innerCheckDeps(path.join(packagesDir, 'jarvis'));

  let hasVersionMismatch = false;
  for (const [key, value] of Object.entries(corePackageJson.dependencies || {})) {
    const value2 = jarvisPackageJson.dependencies[key];
...
async function innerCheckDeps(root) {
  ...
  function allowExternalImport(importName, packageJSON) {
    ...
    if (importName.startsWith('.') || (importName.startsWith('@') && !importName.startsWith('@jarvis/')))
      return true;
