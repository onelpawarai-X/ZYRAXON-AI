...
:::note
このツールは、`ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (または `ZYRAXON_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、ZYRAXON プロバイダーを使用している場合、または `ZYRAXON_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。
