...
const PLAYWRIGHT_GROUP_TITLE = 'Jarvis';
const PLAYWRIGHT_GROUP_COLOR = 'green';
const NON_DEBUGGABLE_SCHEMES = ['chrome:', 'edge:', 'devtools:'];
const CONNECTED_BADGE = { text: '✓', color: '#4CAF50', title: 'Connected to Jarvis client' };

export function isNonDebuggableUrl(url: string | undefined): boolean {
  return !!url && NON_DEBUGGABLE_SCHEMES.some(s => url.startsWith(s));
}

// Ungroups any Jarvis-titled groups left behind by a prior service worker.
export async function cleanupStaleJarvisGroups(): Promise<void> {
  try {
...
// The Jarvis tab group for an active RelayConnection. The Chrome tab group
...
export class ConnectedTabGroup {
  ...
  constructor(connection: RelayConnection, selectedTab: chrome.tabs.Tab) {
    ...
    // handshake. The relay holds Jarvis-side CDP traffic until
