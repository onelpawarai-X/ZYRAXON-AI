...
默认情况下，ZYRAXON 使用我们自己的 `zyraxon` 主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/zyraxon/themes/*.json` 或 `$XDG_CONFIG_HOME/zyraxon/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.zyraxon/themes/*.json`
4. **当前工作目录** — 定义在 `./.zyraxon/themes/*.json`
