  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "zyraxon")
        draft.active = typeof active === "string" ? active : "zyraxon"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "zyraxon")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.zyraxon, store.mode)
    })
