...
description: Crie ferramentas que o LLM pode chamar no zyraxon.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do zyraxon, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.zyraxon/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/zyraxon/tools/`.
