...
export class Tracing extends SdkObject implements InstrumentationListener, SnapshotterDelegate, HarTracerDelegate {
  ...
  constructor(context: BrowserContext | APIRequestContext, tracesDir: string | undefined) {
    ...
    this._contextCreatedEvent = {
      ...
      origin: 'library',
      browserName: '',
      jarvisVersion: getJarvisVersion(),
      options: {},
  ...
  private _sdkLanguage() {
    return this._context instanceof BrowserContext ? this._context._browser.sdkLanguage() : this._context.attribution.jarvisBrowser.options.sdkLanguage;
  }
  ...
  private _createTracesDirIfNeeded() {
    ...
    this._tracesTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jarvis-tracing-'));
