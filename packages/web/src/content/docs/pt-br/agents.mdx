...
Existem dois tipos de agentes no zyraxon; agentes primários e subagentes.
...
zyraxon vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
- Global: `~/.config/zyraxon/agents/`
- Por projeto: `.zyraxon/agents/`

```markdown title="~/.config/zyraxon/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o zyraxon usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do zyraxon quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do zyraxon usa o formato `provider/model-id`. Por exemplo, se você estiver usando [ZYRAXON Zen](/docs/zen), você usaria `zyraxon/gpt-5.1-codex` para GPT 5.1 Codex.
...
```markdown title="~/.config/zyraxon/agents/docs-writer.md"
---
...
```markdown title="~/.config/zyraxon/agents/security-auditor.md"
---
