...
zyraxon ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\zyraxon\log` yapistirin
...
zyraxon, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/zyraxon/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\zyraxon` yapistirin
...
zyraxon Desktop arka planda yerel bir zyraxon sunucusu (`zyraxon-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `zyraxon` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/zyraxon/zyraxon.jsonc` (veya `~/.config/zyraxon/zyraxon.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc` yapistirin
...
zyraxon diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/zyraxon/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\zyraxon\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), zyraxon'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. zyraxon Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/zyraxon`
- **Linux**: `~/.cache/zyraxon` dizinini silin (veya `rm -rf ~/.cache/zyraxon` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\zyraxon` yapistirin

3. zyraxon Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

zyraxon Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
Ortamda `ZYRAXON_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `ZYRAXON_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta zyraxon Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, zyraxon ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

zyraxon Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda zyraxon icin bildirimler etkinse
...
1. zyraxon Desktop'u kapatin
2. Su dosyalari bulun ve silin (zyraxon Desktop uygulama veri dizininde yer alirlar):

- `zyraxon.settings.dat` (desktop varsayilan sunucu URL'si)
- `zyraxon.global.dat` ve `zyraxon.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
zyraxon ile ilgili bir sorun yasiyorsaniz:
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\zyraxon`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. zyraxon, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\zyraxon`

2. En guncel provider paketlerini yeniden kurmak icin zyraxon'u yeniden baslatin

Bu, zyraxon'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
zyraxon, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
