...
Wenn Ihre globale Konfiguration beispielsweise `theme: "zyraxon"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/zyraxon`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/zyraxon/zyraxon.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`ZYRAXON_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`zyraxon.json` im Projekt) – projektspezifische Einstellungen
5. **`.zyraxon` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`ZYRAXON_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.zyraxon` und `~/.config/zyraxon` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/zyraxon` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale ZYRAXON-Konfiguration in `~/.config/zyraxon/zyraxon.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Geben Sie mithilfe der Umgebungsvariablen `ZYRAXON_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `ZYRAXON_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.zyraxon` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.zyraxon` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `zyraxon.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/zyraxon/agents/` oder `.zyraxon/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/zyraxon/commands/` oder `.zyraxon/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.zyraxon/plugins/` oder `~/.config/zyraxon/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
