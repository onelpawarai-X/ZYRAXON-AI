...
import { JarvisServer } from './remote/jarvisServer';
...
export class BrowserServerLauncherImpl implements BrowserServerLauncher {
  ...
  async launchServer(options: LaunchServerOptions & { _sharedBrowser?: boolean, _userDataDir?: string } = {}): Promise<BrowserServer> {
    const jarvis = createJarvis({ sdkLanguage: 'javascript', isServer: true });
    ...
    try {
      ...
      browser = await controller.run(async progress => {
        if (options._userDataDir !== undefined) {
          ...
          const context = await jarvis[this._browserName].launchPersistentContext(progress, options._userDataDir, launchOptions);
          ...
          return await jarvis[this._browserName].launch(progress, launchOptions, toProtocolLogger(options.logger));
        }
      });
    } catch (e) {
    ...
    // 2. Start the server
    const server = new JarvisServer({ mode: options._sharedBrowser ? 'launchServerShared' : 'launchServer', path, maxConnections: Infinity, preLaunchedBrowser: browser });
