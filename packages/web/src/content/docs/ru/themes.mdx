...
С помощью zyraxon вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию zyraxon использует нашу собственную тему `zyraxon`.
...
zyraxon поставляется с несколькими встроенными темами.
...
- Хотите, чтобы zyraxon соответствовал внешнему виду их терминала
...
zyraxon поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/zyraxon/themes/*.json` или `$XDG_CONFIG_HOME/zyraxon/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.zyraxon/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.zyraxon/themes/*.json`.
...
```bash no-frame
mkdir -p ~/.config/zyraxon/themes
vim ~/.config/zyraxon/themes/my-theme.json
```

Для тем проекта:

```bash no-frame
mkdir -p .zyraxon/themes
vim .zyraxon/themes/my-theme.json
```
