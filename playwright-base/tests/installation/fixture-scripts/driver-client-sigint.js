...
const pw = require.resolve('jarvis');
const coreBundle = require.resolve('jarvis-browser-core/lib/coreBundle', { paths: [pw] });
const { oop } = require(coreBundle);

(async () => {
  console.log('launching driver')
  const { jarvis, stop } = await oop.start();
  console.log('launched driver')
  try {
    const browser = await jarvis.chromium.launch({ handleSIGINT: false });
    const context = await browser.newContext();
    const page = await context.newPage();
    // let things settle down
    await page.waitForTimeout(100);
    // send SIGINT to driver
    process.kill(jarvis.driverProcess.pid, 'SIGINT');
