...
import { InstallationLocal, InstallationVersion } from "@zyraxon-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@zyraxon-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@zyraxon-ai/core/v1/config/error"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/zyraxon`
        ...
        if (!Flag.ZYRAXON_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("zyraxon", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
