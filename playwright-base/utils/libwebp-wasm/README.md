...
/ `decodeWebp` are then plain synchronous calls. This matches jarvis-browser-core's
...
The codec ships with jarvis-browser-core:
...
- `webp_codec.wasm` and `webp_codec.LICENSE` are copied into `lib/` by
  ...
  `packages/jarvis-browser-core/.npmignore` (`!lib/**/*.wasm`, `!lib/**/*.LICENSE`).
