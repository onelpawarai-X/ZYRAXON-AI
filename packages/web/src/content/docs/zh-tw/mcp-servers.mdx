...
組織可以透過其 `.well-known/zyraxon` 端點提供預設的 MCP 伺服器。這些伺服器可能預設處於停用狀態，允許使用者按需啟用。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，ZYRAXON 會將 Token 安全地儲存在 `~/.local/share/zyraxon/mcp-auth.json` 中。
...
新增 [Grep by Vercel](https://github.com/upstash/context71) MCP 伺服器以搜尋 GitHub 上的程式碼片段。
