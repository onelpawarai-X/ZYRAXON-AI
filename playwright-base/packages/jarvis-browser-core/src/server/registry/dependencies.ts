...
import { getJarvisVersion } from '../userAgent';
import { deps } from './nativeDeps';

import { packageJSON, binPath } from '../../package';
import { buildJarvisCLICommand, registry } from '.';

const languageBindingVersion = process.env.PW_CLI_DISPLAY_VERSION || packageJSON.version;

const dockerVersionFilePath = '/ms-jarvis/.docker-info';
...
export async function installDependenciesLinux(targets: Set<DependencyGroup>, dryRun: boolean) {
  ...
  if (!isOfficiallySupportedPlatform)
    console.warn(`BEWARE: your OS is not officially supported by Jarvis; installing dependencies for ${platform} as a fallback.`); // eslint-disable-line no-console
  for (const target of targets) {
    const info = deps[platform];
    if (!info) {
      console.warn(`Cannot install dependencies for ${platform} with Jarvis ${getJarvisVersion()}!`);  // eslint-disable-line no-console
      return;
    }
...
export async function validateDependenciesWindows(sdkLanguage: string, windowsExeAndDllDirectories: string[]) {
  ...
  if (isCrtMissing) {
    details.push(
        ...
        `that by installing ZYRAXON Visual C++ Redistributable for Visual Studio from:`,
...
export async function validateDependenciesLinux(sdkLanguage: string, linuxLddDirectories: string[], dlOpenLibraries: string[]) {
  ...
  // Ignore patch versions when comparing docker container version and Jarvis version:
  // we **NEVER** roll browsers in patch releases, so native dependencies do not change.
  if (dockerInfo && !dockerInfo.driverVersion.startsWith(getJarvisVersion(true /* majorMinorOnly */) + '.')) {
    // We are running in a docker container with unmatching version.
    // In this case, we know how to install dependencies in it.
    const pwVersion = getJarvisVersion();
    const requiredDockerImage = dockerInfo.dockerImageName.replace(dockerInfo.driverVersion, pwVersion);
    errorLines.push(...[
      `This is most likely due to Docker image version not matching Jarvis version:`,
      `- Jarvis  : ${pwVersion}`,
      ...
      ``,
      `    ${maybeSudo}${buildJarvisCLICommand(sdkLanguage, 'install-deps')}`,
      ``,
    ...
    // Suggest installation with a Jarvis CLI.
    errorLines.push(...[
...
async function missingDLOPENLibraries(libraries: string[]): Promise<string[]> {
  ...
  // @see https://github.com/zyraxon-ai/jarvis-browser/issues/3397
