...
export class LocalUtilsDispatcher extends Dispatcher<SdkObject, channels.LocalUtilsChannel, RootDispatcher> implements channels.LocalUtilsChannel {
  ...
  constructor(scope: RootDispatcher, jarvis: Jarvis) {
    const localUtils = new SdkObject(jarvis, 'localUtils', 'localUtils');
  ...
  private async _connectOverWebSocket(progress: Progress, params: channels.LocalUtilsConnectParams): Promise<channels.LocalUtilsConnectResult> {
    const wsHeaders = {
      ...
      'x-jarvis-proxy': params.exposeNetwork ?? '',
      ...params.headers,
    };
    const wsEndpoint = await urlToWSEndpoint(progress, params.endpoint);

    const transport = await WebSocketTransport.connect(progress, wsEndpoint, { headers: wsHeaders, followRedirects: true, debugLogHeader: 'x-jarvis-debug-log' });
...
async function urlToWSEndpoint(progress: Progress, endpointURL: string): Promise<string> {
  ...
  const json = await fetchData(progress, {
    ...
    return new Error(`Unexpected status ${response.statusCode} when connecting to ${fetchUrl.toString()}.\n` +
        `This does not look like a Jarvis server, try connecting via ws://.`);
  });
