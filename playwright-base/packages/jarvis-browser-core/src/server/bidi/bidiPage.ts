...
export const kJarvisBindingChannel = 'jarvisChannel';
...
export class BidiPage implements PageDelegate {
  ...
  private async _initialize() {
    ...
    await Promise.all([
      ...
      // If the page is created by the Jarvis client's call, some initialization
  ...
  private async _onScriptMessage(event: bidi.Script.MessageParameters) {
    if (event.channel !== kJarvisBindingChannel)
      return;
