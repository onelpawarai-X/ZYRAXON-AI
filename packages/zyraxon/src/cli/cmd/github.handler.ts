...
const AGENT_USERNAME = "zyraxon-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/zyraxon.yml"
...
jobs:
  zyraxon:
    if: |
      ...
      startsWith(github.event.comment.body, '/zyraxon')
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.zyraxonai.lovable.app" : "https://social-cards.sst.dev/opencode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId1"
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://social-cards.sst.dev/opencode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId2"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/zyraxon,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to zyraxon...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("zyraxon-github-action")
      } catch (error) {
    ...
    async function configureGit(appToken: string) {
      ...
      const config = "http.https://social-cards.sst.dev/opencode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId0"
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `zyraxon/${type}-${hex}-${timestamp}`
      }
      return `zyraxon/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      ...
      const shareUrl = shareId ? `[zyraxon session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the zyraxon infrastructure after your response",
