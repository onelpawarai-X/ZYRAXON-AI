...
1. **Remote config** (from `.well-known/zyraxon`) - organizational defaults
2. **Global config** (`~/.config/zyraxon/zyraxon.json`) - user preferences
3. **Custom config** (`ZYRAXON_CONFIG` env var) - custom overrides
4. **Project config** (`zyraxon.json` in project) - project-specific settings
5. **`.zyraxon` directories** - agents, commands, plugins
6. **Inline config** (`ZYRAXON_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/zyraxon/` on macOS) - admin-controlled
...
:::note
The `.zyraxon` and `~/.config/zyraxon` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/zyraxon` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global ZYRAXON config in `~/.config/zyraxon/zyraxon.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/zyraxon/tui.json`.
...
Specify a custom config file path using the `ZYRAXON_CONFIG` environment variable.
...
Specify a custom config directory using the `ZYRAXON_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.zyraxon` directory, and should
...
The custom directory is loaded after the global config and `.zyraxon` directories, so it **can override** their settings.
...
| Linux    | `/etc/zyraxon/`                         |
| Windows  | `%ProgramData%\zyraxon`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, ZYRAXON reads managed preferences from the `ai.zyraxon.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

ZYRAXON checks these paths:

1. `/Library/Managed Preferences/<user>/ai.zyraxon.managed.plist`
2. `/Library/Managed Preferences/ai.zyraxon.managed.plist`
...
Use the `ai.zyraxon.managed` PayloadType. The ZYRAXON config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.zyraxon.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.zyraxon.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.zyraxon</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `ZYRAXON_TUI_CONFIG` to point to a custom TUI config file.
...
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `zyraxon.local`. Useful for running multiple instances on the same network.
...
You can also define agents using markdown files in `~/.config/zyraxon/agents/` or `.zyraxon/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/zyraxon/commands/` or `.zyraxon/commands/`. [Learn more here](/docs/commands).
...
By default, zyraxon **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
Place plugin files in `.zyraxon/plugins/` or `~/.config/zyraxon/plugins/`. You can also load plugins from npm through the `plugin` option.
