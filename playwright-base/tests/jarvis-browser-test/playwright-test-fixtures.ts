...
import type { JSONReport, JSONReportSpec, JSONReportSuite, JSONReportTest, JSONReportTestResult } from '@jarvis-browser/test/reporter';
import * as fs from 'fs';
import * as os from 'os';
import * as path from 'path';
import { execSync } from 'node:child_process';
import { PNG } from 'jarvis-browser-core/lib/utilsBundle';
import { utils } from '../../packages/jarvis-browser-core/lib/coreBundle';
...
export const cliEntrypoint = path.join(__dirname, '../../packages/jarvis-browser-test/cli.js');

const configFile = (baseDir: string, files: Files): string | undefined => {
  for (const [name, content] of Object.entries(files)) {
    if (name.includes('jarvis.config')) {
...
function startJarvisTest(childProcess: CommonFixtures['childProcess'], baseDir: string, params: any, env: NodeJS.ProcessEnv, options: RunOptions): TestChildProcess {
  ...
  return startJarvisChildProcess(childProcess, baseDir, args, env, options);
}

function startJarvisChildProcess(childProcess: CommonFixtures['childProcess'], baseDir: string, args: string[], env: NodeJS.ProcessEnv, options: RunOptions): TestChildProcess {
  return childProcess({
...
async function runJarvisTest(childProcess: CommonFixtures['childProcess'], baseDir: string, params: any, env: NodeJS.ProcessEnv, options: RunOptions, files: Files, mergeReports: (reportFolder: string, env?: NodeJS.ProcessEnv, options?: RunOptions) => Promise<CliRunResult>, useIntermediateMergeReport: boolean): Promise<RunResult> {
  let reporter;
  ...
  const testProcess = startJarvisTest(childProcess, baseDir, params, useIntermediateMergeReport ? env : envWithJsonReporter, options);
...
async function runJarvisCLI(childProcess: CommonFixtures['childProcess'], args: string[], baseDir: string, env: NodeJS.ProcessEnv): Promise<{ output: string, stdout: string, stderr: string, exitCode: number }> {
...
export const test = base
    ...
    .extend<Fixtures>({
      ...
      runInlineTest: async ({ childProcess, mergeReports, useIntermediateMergeReport }, use, testInfo: TestInfo) => {
        const cacheDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'jarvis-browser-test-cache-'));
        await use(async (files: Files, params: Params = {}, env: NodeJS.ProcessEnv = {}, options: RunOptions = {}) => {
          const baseDir = await writeFiles(testInfo, files, true);
          return await runJarvisTest(childProcess, baseDir, params, { PWTEST_CACHE_DIR: cacheDir, ...env }, options, files, mergeReports, useIntermediateMergeReport);
        });
      ...
      runCLICommand: async ({ childProcess }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, command: string, args?: string[]) => {
          const baseDir = await writeFiles(testInfo, files, true);
          return await runJarvisCLI(childProcess, [command, ...(args || [])], baseDir, { PWTEST_CACHE_DIR: cacheDir });
        });
        await removeFolders([cacheDir]);
      },
      ...
      startCLICommand: async ({ childProcess }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, command: string, args?: string[], options: RunOptions = {}, env: NodeJS.ProcessEnv = {}) => {
          const baseDir = await writeFiles(testInfo, files, true);
          return startJarvisChildProcess(childProcess, baseDir, [command, ...(args || [])], { ...env, PWTEST_CACHE_DIR: cacheDir }, options);
        });
        await removeFolders([cacheDir]);
      },
      ...
      interactWithTestRunner: async ({ childProcess }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, params: Params = {}, env: NodeJS.ProcessEnv = {}, options: RunOptions = {}) => {
          const baseDir = await writeFiles(testInfo, files, true);
          testProcess = startJarvisTest(childProcess, baseDir, params, { PWTEST_CACHE_DIR: cacheDir, ...env }, options);
          return testProcess;
        });
