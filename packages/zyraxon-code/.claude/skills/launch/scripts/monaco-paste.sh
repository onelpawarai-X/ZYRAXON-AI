#!/usr/bin/env bash
# monaco-paste.sh — insert text into the ZYRAXON Code chat input (Monaco) via
# the currently-attached @playwright/cli connection. Synthesizes a
# ClipboardEvent('paste') with a DataTransfer payload — no system
# clipboard involved, so safe to use against multiple parallel ZYRAXON Code
# instances (each subagent's @playwright/cli attaches to its own CDP).
#
# Why this exists: ZYRAXON Code Monaco's `native-edit-context` element doesn't
