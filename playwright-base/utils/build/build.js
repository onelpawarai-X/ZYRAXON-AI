...
updateSteps.push(new ProgramStep({
  ...
  shell: true,
  cwd: path.join(__dirname, '..', '..', 'tests', 'jarvis-test', 'stable-test-runner'),
  concurrent: true,
}));
...
class EsbuildStep extends Step {
  ...
  constructor(options, watchPaths = []) {
    ...
    if (options.bundle) {
      ...
      // Suppress direct-eval warnings — Jarvis intentionally uses eval
...
//    `const { X } = require('jarvis-core/lib/coreBundle').iso`
...
//    `const … = require('jarvis-core/lib/utilsBundle').<key>` so that
...
function _utilsBundleSpecifier(filePath) {
  const coreSrcMarker = `${path.sep}jarvis-core${path.sep}src${path.sep}`;
  const idx = filePath.indexOf(coreSrcMarker);
  if (idx === -1)
    return "'jarvis-core/lib/utilsBundle'";
...
for (const pkg of workspace.packages()) {
  if (!fs.existsSync(path.join(pkg.path, 'src')))
    continue;
  // jarvis-client is built as a bundle.
  if (['@jarvis/client'].includes(pkg.name))
    continue;
  if (pkg.name === 'jarvis-core' || pkg.name === 'jarvis')
    continue;
...
// @jarvis/client — browser-targeted ESM bundle. The client tree is written
// isomorphically; the few genuine node builtins it still imports are swapped for
// browser stubs here (see packages/jarvis-client/src/nodeStubs). esbuild's
// `platform: 'browser'` additionally fails the build if any other builtin leaks.
{
  const clientNodeStub = name => filePath(`packages/jarvis-client/src/nodeStubs/${name}.ts`);
  steps.push(new EsbuildStep({
    bundle: true,
    format: 'esm',
    platform: 'browser',
    target: 'es2020',
    entryPoints: [filePath('packages/jarvis-client/src/index.ts')],
    outfile: filePath('packages/jarvis-client/lib/index.mjs'),
    alias: {
    ...
    filePath('packages/jarvis-client/src'),
    filePath('packages/jarvis-core/src/client'),
...
// Build jarvis-core exported entry points.
steps.push(new EsbuildStep({
  entryPoints: [
    // Performance analysis tool.
    filePath('packages/jarvis-core/src/bootstrap.ts'),

    // Entry points for oop execution.
    filePath('packages/jarvis-core/src/entry/cliDaemon.ts'),
    filePath('packages/jarvis-core/src/entry/dashboardApp.ts'),
    filePath('packages/jarvis-core/src/entry/mcp.ts'),
    filePath('packages/jarvis-core/src/entry/oopBrowserDownload.ts'),

    // CLI client tools, should be a separate bundle.
    filePath('packages/jarvis-core/src/tools/cli-client/*.ts'),
    filePath('packages/jarvis-core/src/package.ts'),
    filePath('packages/jarvis-core/src/tools/utils/socketConnection.ts'),
    filePath('packages/jarvis-core/src/tools/utils/extension.ts'),
  ],
  outdir: filePath('packages/jarvis-core/lib'),
  plugins: [dynamicImportToRequirePlugin],
}));

// jarvis-core/lib/serverRegistry.js
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis-core/src/serverRegistry.js')],
  outfile: filePath('packages/jarvis-core/lib/serverRegistry.js'),
  external: ['fsevents'],
}, [filePath('packages/jarvis-core/src/*')]));

const jarvisCoreSrc = filePath('packages/jarvis-core/src');
const commonUtilsSrc = [filePath('packages/protocol/src'), filePath('packages/utils'), filePath('packages/isomorphic')];

// jarvis-core/lib/utilsBundle.js — bundled npm utilities barrel.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis-core/src/utilsBundle.ts')],
  outfile: filePath('packages/jarvis-core/lib/utilsBundle.js'),
  external: ['fsevents', 'express', '@anthropic-ai/sdk'],
  alias: {
    'raw-body': filePath('utils/build/raw-body.ts'),
  },
}, [filePath('packages/jarvis-core/src/utilsBundle.ts'), filePath('utils/build/raw-body.ts')]));

// Build jarvis-core as a single bundle.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis-core/src/coreBundle.ts')],
  outfile: filePath('packages/jarvis-core/lib/coreBundle.js'),
  external: [
    '../../api.json',
    './help.json',
...
}, [jarvisCoreSrc, ...commonUtilsSrc, filePath('packages/injected')]));

function assertCoreBundleHasNoNodeModules() {
  const bundlePath = filePath('packages/jarvis-core/lib/coreBundle.js');
...
// jarvis/lib/transform/esmLoader.js — bundled ESM loader registered by
...
// parses it).
{
  const jarvisSrc = filePath('packages/jarvis/src');
  steps.push(new EsbuildStep({
    bundle: true,
    entryPoints: [filePath('packages/jarvis/src/transform/esmLoader.ts')],
    outfile: filePath('packages/jarvis/lib/transform/esmLoader.js'),
    external: [
      'jarvis-core',
      'jarvis-core/*',
      '../package',
    ...
    plugins: [],
  }, [jarvisSrc, ...commonUtilsSrc]));
}

// Build jarvis entry points (per-file), excluding matchers/* and
// common/* + transform/* — all of those are produced by bundle steps below.
steps.push(new EsbuildStep({
  entryPoints: [
    filePath('packages/jarvis/src/*.ts'),
    filePath('packages/jarvis/src/agents/**/*.ts'),
    filePath('packages/jarvis/src/cli/**/*.ts'),
    filePath('packages/jarvis/src/mcp/**/*.ts'),
  ],
  outdir: filePath('packages/jarvis/lib'),
  plugins: [dynamicImportToRequirePlugin],
}));

// jarvis/lib/transform/babelBundle.js — bundled babel facade.
...
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/transform/babelBundle.ts')],
  outfile: filePath('packages/jarvis/lib/transform/babelBundle.js'),
  external: [
    '../package',
  ],
  plugins: [dynamicImportToRequirePlugin],
}, [filePath('packages/jarvis/src'), ...commonUtilsSrc]));

// jarvis/lib/matchers/expect.js — bundled jest expect facade.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/matchers/expect.ts')],
  outfile: filePath('packages/jarvis/lib/matchers/expect.js'),
  external: [
    'jarvis-core',
    'jarvis-core/*',
    '../globals',
...
// jarvis/lib/common/index.js — bundled common barrel.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/common/index.ts')],
  outfile: filePath('packages/jarvis/lib/common/index.js'),
  external: [
    'jarvis-core',
    'jarvis-core/*',
    'jarvis',
...
// jarvis/lib/runner/index.js — bundled runner barrel.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/runner/index.ts')],
  outfile: filePath('packages/jarvis/lib/runner/index.js'),
  external: [
    'jarvis-core',
    'jarvis-core/*',
    '../common',
...
// jarvis/lib/isomorphic/index.js — bundled isomorphic barrel.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/isomorphic/index.ts')],
  outfile: filePath('packages/jarvis/lib/isomorphic.js'),
  external: [
  ],
  plugins: [dynamicImportToRequirePlugin],
}, [filePath('packages/jarvis/src')]));

// jarvis/lib/loader/loaderProcessEntry.js — bundled loader process
// entry. Output sits at the same depth as the source so '../X' externals
// resolve to lib/X.js naturally.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/loader/loaderProcessEntry.ts')],
  outfile: filePath('packages/jarvis/lib/loader/loaderProcessEntry.js'),
  external: [
    'jarvis-core',
    'jarvis-core/*',
    '../common',
...
// jarvis/lib/worker/workerProcessEntry.js — bundled worker process
// entry. Output sits at the same depth as the source so '../X' externals
// resolve to lib/X.js naturally.
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis/src/worker/workerProcessEntry.ts')],
  outfile: filePath('packages/jarvis/lib/worker/workerProcessEntry.js'),
  external: [
    'jarvis-core',
    'jarvis-core/*',
    '../common',
...
steps.push(new EsbuildStep({
  bundle: true,
  entryPoints: [filePath('packages/jarvis-core/src/server/electron/loader.ts')],
  outfile: filePath('packages/jarvis-core/lib/server/electron/loader.js'),
  external: ['electron'],
}, [jarvisCoreSrc]));

function copyXdgOpen() {
  const outdir = filePath('packages/jarvis-core/lib');
...
onChanges.push({
  inputs: [
    'packages/jarvis-core/src/tools/cli-daemon/command.ts',
    'packages/jarvis-core/src/tools/cli-daemon/commands.ts',
    'packages/jarvis-core/src/tools/cli-daemon/helpGenerator.ts',
...
onChanges.push({
  inputs: [
    ...
    'packages/jarvis-core/src/third_party/**',
    'packages/isomorphic/**',
...
onChanges.push({
  inputs: [
    ...
    'packages/jarvis-core/src/server/chromium/protocol.d.ts',
  ],
  mustExist: [
...
if (watchMode && !disableInstall) {
  ...
  onChanges.push({
    inputs: ['packages/jarvis-core/browsers.json'],
    command: 'npx',
    args: ['jarvis', 'install'],
  });
}
...
copyFiles.push({
  files: 'packages/jarvis-core/src/server/chromium/*.png',
  from: 'packages/jarvis-core/src',
  to: 'packages/jarvis-core/lib',
});

// esbuild doesn't touch JS files, so copy them manually.
// For example: diff_match_patch.js
copyFiles.push({
  files: 'packages/jarvis-core/src/**/*.js',
  from: 'packages/jarvis-core/src',
  to: 'packages/jarvis-core/lib',
...
copyFiles.push({
  files: 'packages/jarvis-core/src/**/*.json',
  ignored: ['**/injected/**/*'],
  from: 'packages/jarvis-core/src',
  to: 'packages/jarvis-core/lib',
});
...
copyFiles.push({
  files: 'packages/jarvis/src/agents/*.md',
  from: 'packages/jarvis/src',
  to: 'packages/jarvis/lib',
});

copyFiles.push({
  files: 'packages/jarvis/src/agents/*.yml',
  from: 'packages/jarvis/src',
  to: 'packages/jarvis/lib',
});
...
copyFiles.push({
  files: 'packages/jarvis-core/src/tools/skills/**/*',
  from: 'packages/jarvis-core/src',
  to: 'packages/jarvis-core/lib',
});

copyFiles.push({
  files: 'packages/jarvis-core/src/tools/dashboard/*.{png,ico}',
  from: 'packages/jarvis-core/src',
  to: 'packages/jarvis-core/lib',
});
