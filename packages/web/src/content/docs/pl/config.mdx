...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "zyraxon"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/zyraxon`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/zyraxon/zyraxon.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `ZYRAXON_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`zyraxon.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.zyraxon`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `ZYRAXON_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.zyraxon` i `~/.config/zyraxon` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/zyraxon`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację ZYRAXON w `~/.config/zyraxon/zyraxon.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/zyraxon/tui.json`.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `ZYRAXON_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `ZYRAXON_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.zyraxon` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.zyraxon`, więc **może być łączony** z ich zawartością.
...
Użyj `ZYRAXON_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.
...
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `zyraxon.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/zyraxon/agents/` lub `.zyraxon/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/zyraxon/commands/` lub `.zyraxon/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.zyraxon/plugins/` lub `~/.config/zyraxon/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
