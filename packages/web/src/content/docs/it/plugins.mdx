...
- `.zyraxon/plugins/` - plugin a livello progetto
- `~/.config/zyraxon/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/zyraxon/node_modules/`.
...
1. Config globale (`~/.config/zyraxon/zyraxon.json`)
2. Config di progetto (`zyraxon.json`)
3. Directory plugin globale (`~/.config/zyraxon/plugins/`)
4. Directory plugin di progetto (`.zyraxon/plugins/`)
...
- `client`: un client SDK di zyraxon per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere zyraxon.
...
Impedisci a zyraxon di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a zyraxon:
...
L'helper `tool` crea uno strumento personalizzato che zyraxon può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in zyraxon insieme agli strumenti integrati.
