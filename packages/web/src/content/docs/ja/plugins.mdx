...
- `.zyraxon/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/zyraxon/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/zyraxon/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/zyraxon/zyraxon.json`)
2. プロジェクト設定 (`zyraxon.json`)
3. グローバルプラグインディレクトリ (`~/.config/zyraxon/plugins/`)
4. プロジェクトプラグインディレクトリ (`.zyraxon/plugins/`)
