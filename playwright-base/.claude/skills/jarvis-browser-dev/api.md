...
- `packages/jarvis-browser-core/types/types.d.ts` — public API types
- `packages/playwright/types/test.d.ts` — test API types

## Step 2: Implement Client API

Implement the new API in `packages/jarvis-browser-core/src/client/xxx.ts`.
...
- `packages/jarvis-browser-core/src/protocol/validator.ts` — runtime validators
- `packages/jarvis-browser-core/src/utils/isomorphic/protocolMetainfo.ts` — method metadata

## Step 4: Implement Dispatcher

Implement dispatcher handler in `packages/jarvis-browser-core/src/server/dispatchers/xxxDispatcher.ts` as needed.
...
Handler should route the call into the corresponding method in `packages/jarvis-browser-core/src/server/xxx.ts`.

Server methods implement the actual browser interaction:

```typescript
// In packages/jarvis-browser-core/src/server/frames.ts
...
- `packages/jarvis-browser-core/src/server/chromium/crPage.ts` — Chromium (uses CDP: `this._client.send('Page.navigate', { ... })`)
- `packages/jarvis-browser-core/src/server/firefox/ffPage.ts` — Firefox
- `packages/jarvis-browser-core/src/server/webkit/wkPage.ts` — WebKit
