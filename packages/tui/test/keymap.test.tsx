...
import { getZyraxonModeStack, ZYRAXON_BASE_MODE, ZyraxonKeymapProvider, registerZyraxonKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerZyraxonKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when zyraxon mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getZyraxonModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getZyraxonModeStack(keymap).push("autocomplete")
