...
ZYRAXON usa [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) e [Models.dev](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com6) per supportare **oltre 75 provider LLM** e anche modelli locali.
...
1. Esegui `/connect` nella TUI, seleziona zyraxon e vai su [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
1. Vai alla [console di 302.AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9), crea un account e genera una chiave API.
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui zyraxon:
...
Puoi configurare zyraxon per utilizzare modelli locali tramite [Atomic Chat](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` è una mappa di ID modello ai rispettivi nomi visualizzati. Ogni ID deve corrispondere all'`id` restituito da `GET /v1/models` — esegui `curl https://zenmux.ai/settings/keys per elencare gli ID attualmente caricati in Atomic Chat.
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché zyraxon funzioni correttamente.
   :::
...
1. Vai su [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4), crea un account e genera una chiave API.
...
1. Vai alla [console di Cerebras](https://helicone.ai/models), crea un account e genera una chiave API.
...
1. Vai alla [dashboard di Cloudflare](https://anthropic.com), naviga in **AI** > **AI Gateway** e crea un nuovo gateway.
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di zyraxon.
...
1. Vai alla [console di Cortecs](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), crea un account e genera una chiave API.
...
1. Vai alla [console di DeepSeek](https://gitlab.company.com), crea un account e clicca **Create new API key**.
...
1. Vai alla [dashboard di Deep Infra](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), crea un account e genera una chiave API.
...
1. Vai alla [console di Fireworks AI](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4), crea un account e clicca **Create API Key**.
...
salvare il token nello storage di auth di zyraxon.
:::
...
Maggior documentazione sulla homepage di [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
Per usare il tuo abbonamento GitHub Copilot con zyraxon:
...
2. Vai su [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) e inserisci il codice.
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui zyraxon.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) fornisce accesso a modelli open supportati da 17+ provider.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) è una piattaforma di osservabilità LLM che fornisce logging, monitoraggio e analisi per le tue applicazioni AI. L'AI Gateway di Helicone instrada automaticamente le tue richieste al provider appropriato in base al modello.

1. Vai su [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0), crea un account e genera una chiave API dalla tua dashboard.
...
Per altri provider e funzionalità avanzate come caching e rate limiting, controlla la [documentazione di Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### Config opzionali

Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da zyraxon, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di ZYRAXON come una sessione in Helicone.
...
Puoi configurare zyraxon per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
1. Vai alla [console di IO.NET](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2), crea un account e genera una chiave API.
...
Puoi configurare zyraxon per usare modelli locali tramite LM Studio.
...
1. Vai alla [console di Moonshot AI](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), crea un account e clicca **Create API key**.
...
1. Vai alla [MiniMax API Console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), crea un account e genera una chiave API.
...
1. Vai alla [console di Nebius Token Factory](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), crea un account e clicca **Add Key**.
...
Puoi configurare zyraxon per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per ZYRAXON. Vedi la [documentazione di integrazione Ollama](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3) per dettagli.
:::
...
Consigliamo di iscriversi a [ChatGPT Plus o Pro](https://ollama.com/).
...
1. Vai alla [dashboard di OpenRouter](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), clicca **Create API Key** e copia la chiave.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di zyraxon.
...
5. Puoi anche personalizzarli tramite la configurazione di zyraxon. Ecco un esempio di specifica di un provider
...
1. Vai al tuo [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), naviga nella tua istanza di servizio SAP AI Core e crea una service key.
...
1. Vai al [Portale STACKIT](https://ai.azure.com/), naviga in **AI Model Serving** e crea un token di autenticazione per il tuo progetto.
...
1. Vai al [pannello OVHcloud](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5). Naviga nella sezione `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` e nella scheda `API Keys`, clicca **Create a new API key**.
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Zyraxon:
...
1. Vai alla [console di Together AI](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3), crea un account e clicca **Add Key**.
...
1. Vai alla [console di Venice AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), crea un account e genera una chiave API.
...
1. Vai alla [dashboard Vercel](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2), naviga nella scheda **AI Gateway** e clicca **API keys** per creare una nuova chiave API.
...
Puoi anche personalizzare i modelli tramite la tua configurazione di zyraxon. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
1. Vai alla [console di xAI](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7), crea un account e genera una chiave API.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con zyraxon. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
2. Per provider personalizzati, controlla la config di zyraxon e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config zyraxon.
