...
export function decorateMCPCommand(command: Command) {
  command
      ...
      .option('--extension', 'Connect to a running browser instance (Edge/Chrome only). Requires the "Jarvis Extension" to be installed.')
      ...
      .option('--init-page <path...>', 'path to TypeScript file to evaluate on Jarvis page object')
      ...
      .option('--save-session', 'Whether to save the Jarvis MCP session into the output directory.')
      ...
      .action(async options => {
        ...
        let sharedBrowserPromise: Promise<jarvis.Browser> | undefined;
        let clientCount = 0;
