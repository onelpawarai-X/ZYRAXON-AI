...
async function run() {
  ...
  let jarvisVersion = require(path.join(PROJECT_DIR, 'package.json')).version;
  if (jarvisVersion.endsWith('-next'))
    jarvisVersion = jarvisVersion.substring(0, jarvisVersion.indexOf('-next'));

  // Ensure browser versions in browsers.json. This is most important for WebKit
  // since its version is hardcoded in Jarvis library rather then in browser builds.
  // @see https://github.com/zyraxon-ai/jarvis-browser/issues/15702
  {
    const browsersJSONPath = path.join(__dirname, '..', '..', 'packages/jarvis-browser-core/browsers.json');
    ...
    for (const lang of langs) {
      try {
        ...
        for (const filePath of getAllMarkdownFiles(documentationRoot)) {
          ...
          // Also, Internally (jarvisbrowser.dev generator) we merge test-api and test-reporter-api into api.
...
async function getBrowserVersions() {
  ...
  const browsers = await Promise.all(names.map(name => jarvis[name].launch()));
  const result = {};
