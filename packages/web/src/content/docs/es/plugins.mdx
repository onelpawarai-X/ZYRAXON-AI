...
- `.zyraxon/plugins/` - Complementos a nivel de proyecto
- `~/.config/zyraxon/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/zyraxon/node_modules/`.
...
1. Configuración global (`~/.config/zyraxon/zyraxon.json`)
2. Configuración del proyecto (`zyraxon.json`)
3. Directorio global de complementos (`~/.config/zyraxon/plugins/`)
4. Directorio de complementos del proyecto (`.zyraxon/plugins/`)
...
- `client`: Un cliente SDK zyraxon para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar zyraxon.
...
Evite que zyraxon lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a zyraxon:
...
El ayudante `tool` crea una herramienta personalizada a la que zyraxon puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para zyraxon junto con las herramientas integradas.
