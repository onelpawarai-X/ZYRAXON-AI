---
title: Regeln
description: Lege eigene Anweisungen fuer zyraxon fest.
---

Du kannst zyraxon ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in zyraxon den Befehl `/init` aus.
...
So kann zyraxon sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht zyraxon sie zu erweitern.
...
zyraxon kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/zyraxon/AGENTS.md` speichern.
Diese gelten in allen zyraxon-Sitzungen.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/zyraxon/AGENTS.md` existiert)
...
Beim Start sucht zyraxon in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/zyraxon/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `zyraxon.json` oder in `~/.config/zyraxon/zyraxon.json` angeben.
...
zyraxon parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
...
Du kannst zyraxon explizit anweisen, externe Dateien zu lesen.
