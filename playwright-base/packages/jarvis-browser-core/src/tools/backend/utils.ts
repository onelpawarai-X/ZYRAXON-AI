...
export async function waitForCompletion<R>(tab: Tab, callback: () => Promise<R>): Promise<R> {
  ...
  const requests: jarvis.Request[] = [];

  const requestListener = (request: jarvis.Request) => requests.push(request);
...
export function eventWaiter<T>(page: jarvis.Page, event: string, timeout: number): { promise: Promise<T | undefined>, abort: () => void } {
