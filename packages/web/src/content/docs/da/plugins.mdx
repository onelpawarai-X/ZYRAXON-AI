...
- `.zyraxon/plugins/` - Plugins på projektniveau
- `~/.config/zyraxon/plugins/` - Globale plugins
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/zyraxon/node_modules/`.
...
1. Global konfiguration (`~/.config/zyraxon/zyraxon.json`)
2. Projektkonfiguration (`zyraxon.json`)
3. Global plugin-mappe (`~/.config/zyraxon/plugins/`)
4. Projekt plugin bibliotek (`.zyraxon/plugins/`)
...
- `client`: En zyraxon SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide zyraxon.
...
Undgå zyraxon i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til zyraxon:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som zyraxon kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for zyraxon sammen med indbyggede værktøjer.
