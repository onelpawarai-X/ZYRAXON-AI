...
1. **Config remota** (da `.well-known/zyraxon`) - default dell'organizzazione
2. **Config globale** (`~/.config/zyraxon/zyraxon.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `ZYRAXON_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`zyraxon.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.zyraxon`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `ZYRAXON_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.zyraxon` e `~/.config/zyraxon` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/zyraxon`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di ZYRAXON in `~/.config/zyraxon/zyraxon.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/zyraxon/tui.json`.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `ZYRAXON_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `ZYRAXON_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.zyraxon` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.zyraxon`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `ZYRAXON_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `zyraxon.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi anche definire agenti usando file markdown in `~/.config/zyraxon/agents/` o `.zyraxon/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/zyraxon/commands/` o `.zyraxon/commands/`. [Scopri di piu](/docs/commands).
...
Per impostazione predefinita, zyraxon **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
Metti i file dei plugin in `.zyraxon/plugins/` o `~/.config/zyraxon/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
