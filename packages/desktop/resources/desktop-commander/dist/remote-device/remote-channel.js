...
export function observeServerDate(dateHeader) {
    if (!dateHeader)
        return;
...
async function clockAwareFetch(input, init) {
...
export class RemoteChannel {
    constructor() {
        ...
        this.sessionRefreshedHandler = null;
        ...
        this.reconnectAttempt = 0; // recreates since the last success
        this.isRecreatingChannel = false; // re-entrancy guard
        this.joiningSince = null; // start of an unbroken 'joining' run (performance.now())
        ...
        this.lastHeartbeatOkAt = null;
        this.heartbeatListenerRegistered = false;
    ...
    initialize(url, key) {
        ...
        if (!this.heartbeatListenerRegistered) {
            ...
            try {
                this.client.realtime?.onHeartbeat?.((status) => {
                    if (status === 'ok')
                        this.lastHeartbeatOkAt = performance.now();
                });
            }
    ...
    async handleSignedOut() {
        if (this.handlingSignedOut || this.sessionLost || this.shuttingDown)
            return;
     ...
     */
    async trackPresenceWithRetry(recovered, attempts = 3) {
        if (this.isTrackingPresence)
            return; // never stack pushes on a wedged socket
    ...
    async trackPresenceInner(recovered, attempts) {
     ...
     */
    capabilitiesPayload(broadcastCapable) {
        return {
            app_version: VERSION,
     ...
     */
    async setTransportCapable(capable) {
        if (!this.client || !this.deviceId)
            return;
        if (this.transportCapableWritten === capable)
            return; // no redundant writes
        try {
            ...
            if (!capable && this.heartbeatDeviceId) {
                this.updateHeartbeat(this.heartbeatDeviceId).catch(() => { });
            }
        }
        catch (error) {
    ...
    createChannel() {
        return new Promise((resolve, reject) => {
            ...
            this.channel = this.client.channel(channelName, {
                ...
                this.onDoorbell(payload).catch((e) => {
    ...
    dispatchToolCall(payload) {
        try {
            const maybePromise = this.onToolCall?.(payload);
    ...
    checkConnectionHealth() {
        ...
        if (state === 'joined') {
            ...
            if (!this.presenceTracked && this.deviceId && !this.isTrackingPresence) {
                ...
                this.trackPresenceWithRetry(0, 1).catch(() => { });
            }
            return;
        }
    ...
    async recreateChannel() {
        ...
        try {
            ...
            const backoffMs = Math.min(30000, 1000 * 2 ** Math.min(this.reconnectAttempt, 5)) * (0.5 + Math.random());
            ...
            if (this.reconnectAttempt >= TRANSPORT_WITHDRAW_AFTER_ATTEMPTS) {
                ...
                try {
                    await this.withTimeout(() => this.setTransportCapable(false), CAPABILITY_WRITE_TIMEOUT_MS, 'withdrawTransportCapability');
                }
                catch (withdrawErr) {
    ...
    async updateCallResult(callId, status, result = null, errorMessage = null) {
        ...
        if (result !== null)
            updateData.result = stripNullBytes(result);
        ...
        if (errorMessage !== null)
            updateData.error_message = stripNullBytes(errorMessage);
        ...
        if (process.env.DEBUG_MODE === 'true') {
            console.debug(`[DEBUG] Updating call result: ${callId} status=${status}` +
        ...
        if (error) {
            ...
            if (result !== null && status !== 'failed') {
                await this.updateCallResult(callId, 'failed', null, `Result could not be stored (${error.message})`);
            }
        }
        else {
