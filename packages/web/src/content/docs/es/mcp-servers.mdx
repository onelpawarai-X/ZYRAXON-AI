...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/zyraxon`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, ZYRAXON almacenará los tokens de forma segura en `~/.local/share/zyraxon/mcp-auth.json`.
...
Agregue el servidor [Grep by Vercel](https://github.com/upstash/context71) MCP para buscar fragmentos de código en GitHub.
