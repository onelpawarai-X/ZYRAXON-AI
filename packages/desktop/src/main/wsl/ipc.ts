...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-zyraxon", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installZyraxon(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-zyraxon", unavailable)
