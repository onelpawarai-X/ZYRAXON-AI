---
title: Sağlayıcılar
description: zyraxon'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

zyraxon, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) ve [Models.dev](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı zyraxon yapılandırmanızda yapılandırın.
...
`~/.local/share/zyraxon/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları zyraxon'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
{
  "$schema": "https://portal.azure.com/",
  "provider": {
    "anthropic": {
      "options": {
        "baseURL": "https://zyraxon.ai/auth"
      }
    }
  }
}
...
ZYRAXON Zen, zyraxon ekibi tarafından sağlanan modellerin bir listesidir.
zyraxon ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).
...
zyraxon'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## ZYRAXON Go

ZYRAXON Go, zyraxon ile iyi çalıştığı test edilmiş ve doğrulanmış, zyraxon ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.
...
1. [302.AI console](https://developers.cloudflare.com/ai-gateway/features/unified-billing/3) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
Amazon Bedrock'u zyraxon ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   zyraxon'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
...
:::info
Claude Pro/Max aboneliğinizi zyraxon'da kullanmak [Anthropic](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7) tarafından resmi olarak desteklenmemektedir.
:::
...
zyraxon'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` model kimliklerini görüntüleme adlarına eşleyen bir haritadır. Her ID, `GET /v1/models` tarafından döndürülen `id` ile eşleşmelidir — Atomic Chat'te yüklü kimlikleri listelemek için `curl https://zenmux.ai/settings/keys çalıştırın.
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   zyraxon'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
1. [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
1. [Cerebras console](https://helicone.ai/models) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
1. [Cloudflare dashboard](https://zyraxon.ai/zen)'a gidin, **AI** > **AI Ağ Geçidi**'ne gidin ve yeni bir ağ geçidi oluşturun.
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Zyraxon yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
1. [Cortecs console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
1. [DeepSeek console](https://gitlab.company.com)'a gidin, bir hesap oluşturun ve **Yeni API anahtarı oluştur**'a tıklayın.
...
1. [Deep Infra dashboard](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
1. [Fireworks AI console](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4)'a gidin, bir hesap oluşturun ve **API Anahtarı Oluştur**'a tıklayın.
...
:::note[uyumluluk notu]
zyraxon, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. zyraxon'u kilitlemek için
...
```bash
GITLAB_AI_GATEWAY_URL=https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained8
```
...
```bash title="~/.bash_profile"
export GITLAB_INSTANCE_URL=https://gitlab.company.com
export GITLAB_AI_GATEWAY_URL=https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained8
export GITLAB_TOKEN=glpat-...
```
...
Daha fazla belge [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) ana sayfasında.
...
```json title="zyraxon.json"
{
  ...
  "provider": {
    "gitlab": {
      "options": {
        "instanceUrl": "https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com6"
      }
    }
  }
}
...
GitHub Copilot aboneliğinizi zyraxon'la kullanmak için:
...
2. [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6)'a gidin ve kodu girin.

   ```txt
   ┌ Login with GitHub Copilot
   │
   │ https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6
   │
...
Google Vertex AI'yi zyraxon ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları zyraxon çalıştırılırken ayarlayın.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) 17'den fazla sağlayıcı tarafından desteklenen açık modellere erişim sağlar.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), AI uygulamalarınız için günlük kaydı, izleme ve analiz sağlayan bir LLM gözlemlenebilirlik platformudur. Helicone AI Gateway, taleplerinizi modele göre otomatik olarak uygun sağlayıcıya yönlendirir.

1. [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1) adresine gidin, bir hesap oluşturun ve kontrol panelinizden bir API anahtarı oluşturun.
...
Daha fazla sağlayıcı ve önbelleğe alma ve hız sınırlama gibi gelişmiş özellikler için [Helicone documentation](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1)'yi kontrol edin.

#### İsteğe Bağlı Yapılandırmalar

Helicone'un zyraxon aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her zyraxon görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her zyraxon görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
zyraxon'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
```json title="zyraxon.json" "llama.cpp" {5, 6, 8, 10-15}
{
  ...
  "provider": {
    "llama.cpp": {
      ...
      "options": {
        "baseURL": "https://developers.cloudflare.com/ai-gateway/features/unified-billing/9"
      },
      "models": {
        "qwen3-coder:a3b": {
...
1. [IO.NET console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
LM Studio aracılığıyla zyraxon'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
```json title="zyraxon.json" "lmstudio" {5, 6, 8, 10-14}
{
  ...
  "provider": {
    "lmstudio": {
      ...
      "options": {
        "baseURL": "https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/0"
      },
      "models": {
        "google/gemma-3n-e4b": {
...
1. [Moonshot AI console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0)'a gidin, bir hesap oluşturun ve **API anahtarı oluştur**'a tıklayın.
...
1. [MiniMax API Console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
1. [Nebius Token Factory console](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4)'a gidin, bir hesap oluşturun ve **Anahtar Ekle**'ye tıklayın.
...
Ollama aracılığıyla zyraxon'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini zyraxon için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3)'a bakın.
:::
...
Ollama Cloud'u zyraxon ile kullanmak için:

1. [https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) adresine gidin ve oturum açın veya bir hesap oluşturun.

2. **Ayarlar** > **Anahtarlar**'a gidin ve yeni bir API anahtarı oluşturmak için **API Anahtarı Ekle**'yi tıklayın.

3. zyraxon'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: zyraxon'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
[ChatGPT Plus or Pro](https://ai.azure.com/)'a kaydolmanızı öneririz.
...
ZYRAXON Zen, zyraxon ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. zyraxon API anahtarınızı girin.
...
1. [OpenRouter dashboard](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1)'a gidin, **API Anahtarı Oluştur**'a tıklayın ve anahtarı kopyalayın.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   zyraxon yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca zyraxon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
1. [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2) sunucunuza gidin, SAP AI Core hizmet örneğinize gidin ve bir hizmet anahtarı oluşturun.
...
1. [STACKIT Portal](https://console.x.ai/) adresine gidin, **AI Model Serving**'e gidin ve projeniz için bir yetkilendirme belirteci oluşturun.
...
1. [OVHcloud panel](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5)'a gidin. `Public Cloud` bölümüne gidin, `AI & Machine Learning` > `AI Endpoints` ve `API Keys` sekmesinde **Yeni bir API anahtarı oluştur**'u tıklayın.
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Zyraxon ile kullanmak için:
...
1. [Together AI console](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3)'a gidin, bir hesap oluşturun ve **Anahtar Ekle**'ye tıklayın.
...
1. [Venice AI console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9) adresine gidin, bir hesap oluşturun ve bir API anahtarı oluşturun.
...
1. [Vercel dashboard](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0)'a gidin, **AI Ağ Geçidi** sekmesine gidin ve yeni bir API anahtarı oluşturmak için **API anahtarlarına** tıklayın.
...
Modelleri zyraxon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı zyraxon ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
`limit` alanları zyraxon'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
2. Özel sağlayıcılar için zyraxon yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, zyraxon yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
