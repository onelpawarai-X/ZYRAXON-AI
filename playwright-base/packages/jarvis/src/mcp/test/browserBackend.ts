...
import type { tools } from 'jarvis-browser-core/lib/coreBundle';
import type * as jarvis from '../../../index';
...
export function createCustomMessageHandler(testInfo: TestInfoImpl, context: jarvis.BrowserContext) {
  let backend: tools.BrowserBackend | undefined;
  const config: tools.ContextConfig = { capabilities: ['testing'] };
  let tools: typeof import('jarvis-browser-core/lib/coreBundle').tools | undefined;
  ...
  return async (data: BrowserMCPRequest): Promise<BrowserMCPResponse> => {
    if (!tools)
      ({ tools } = require('jarvis-browser-core/lib/coreBundle') as typeof import('jarvis-browser-core/lib/coreBundle'));
...
async function generatePausedMessage(tools: typeof import('jarvis-browser-core/lib/coreBundle').tools, testInfo: TestInfoImpl, context: jarvis.BrowserContext) {
  const lines: string[] = [];
...
export async function runDaemonForContext(testInfo: TestInfoImpl, context: jarvis.BrowserContext) {
  ...
  console.log([
    ...
    `- Run "jarvis-cli attach ${sessionName}" to attach to this test`,
  ].join('\n'));
