...
export class Browser extends ChannelOwner<channels.BrowserChannel> implements api.Browser {
  ...
  async _innerNewContext(userOptions: BrowserContextOptions = {}, forReuse: boolean): Promise<BrowserContext> {
    const options = this._browserType._jarvis.selectors._withSelectorOptions(userOptions);
  ...
  private _setupBrowserContext(context: BrowserContext) {
    ...
    this._browserType._jarvis.selectors._contextsForSelectors.add(context);
    context.setDefaultTimeout(this._browserType._jarvis._defaultContextTimeout);
    context.setDefaultNavigationTimeout(this._browserType._jarvis._defaultContextNavigationTimeout);
  }
