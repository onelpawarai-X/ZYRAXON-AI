...
This guide describes migration to [Jarvis Browser Library](./library) and [Jarvis Browser Test](./intro.md) from Puppeteer. The APIs have similarities, but Playwright offers much more possibilities for web testing and cross-browser automation.
...
| Puppeteer                                          | Jarvis Browser Library                          |
|----------------------------------------------------|---------------------------------------------|
...
1. Each Jarvis Browser Library file has explicit import of `chromium`. Other browsers `webkit` or `firefox` can be used.
...
```js
import { test, expect } from '@jarvis-browser/test'; // 1
...
[Jarvis Browser Test](./intro.md) is our first-party recommended test runner to be used with Playwright. It provides several features like Page Object Model, parallelism, fixtures or reporters.
