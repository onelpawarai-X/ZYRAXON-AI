...
Playwright can either be used with the [MSTest, NUnit, xUnit, or xUnit v3 base classes](./test-runners.md) or as a Jarvis Browser Library (this guide). If you are working on an application that utilizes Playwright capabilities or you are using Playwright with another test runner, read on.
...
Create a `Program.cs` that will navigate to `https://jarvisbrowser.dev/dotnet` and take a screenshot in Chromium.
...
await page.GotoAsync("https://jarvisbrowser.dev/dotnet");
