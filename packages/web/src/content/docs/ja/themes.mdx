...
デフォルトでは、ZYRAXON は独自の `zyraxon` テーマを使用します。
...
2. **ユーザー設定ディレクトリ** - `~/.config/zyraxon/themes/*.json` または `$XDG_CONFIG_HOME/zyraxon/themes/*.json` で定義されます
3. **プロジェクトのルートディレクトリ** - `<project-root>/.zyraxon/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.zyraxon/themes/*.json` で定義
...
1. **組み込みテーマ** - これらはバイナリに埋め込まれています
2. **ユーザー設定ディレクトリ** - `~/.config/zyraxon/themes/*.json` または `$XDG_CONFIG_HOME/zyraxon/themes/*.json` で定義されます
3. **プロジェクトのルート ディレクトリ** - `<project-root>/.zyraxon/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.zyraxon/themes/*.json` で定義
