---
title: Server
description: Komunicirajte s zyraxon serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `zyraxon serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji zyraxon klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `zyraxon.local` |
...
Postavite `ZYRAXON_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `zyraxon`, ili postavite `ZYRAXON_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `zyraxon serve` i za `zyraxon web`.
...
Kada pokrenete `zyraxon`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite zyraxon server za programsku interakciju sa zyraxon-om.
:::

Ova arhitektura omogucava zyraxon podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `zyraxon serve` da startate standalone server. Ako je zyraxon TUI vec pokrenut, `zyraxon serve` ce pokrenuti novi server.
...
zyraxon server izlaže sljedece API-je.
