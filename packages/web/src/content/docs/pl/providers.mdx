---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w zyraxon.
---

import config from "../../../../config.mjs"
export const console = config.console

zyraxon używa [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0) i [Models.dev](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com6) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji zyraxon.
...
w `~/.local/share/zyraxon/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim zyraxon
config.
...
ZYRAXON Zen to lista modeli dostarczonych przez zespół zyraxon, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z zyraxon. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od ZYRAXON Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `ZYRAXON Zen` i przejdź do [zyraxon.ai/auth](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5).
...
Działa jak każdy inny dostawca w zyraxon i jest całkowicie opcjonalny w użyciu.

---

## ZYRAXON Go

ZYRAXON Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół zyraxon, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z zyraxon.

1. Uruchom polecenie `/connect` w TUI, wybierz `ZYRAXON Go` i przejdź do [zyraxon.ai/auth](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5).
...
1. Przejdź do [konsoli 302.AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9), utwórz konto i wygeneruj klucz API.
...
Aby używać Amazon Bedrock z zyraxon:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania zyraxon:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w zyraxon nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować zyraxon do korzystania z modeli lokalnych przez [Atomic Chat](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` to mapa identyfikatorów modeli na ich nazwy wyświetlane. Każdy identyfikator musi odpowiadać `id` zwracanemu przez `GET /v1/models` — uruchom `curl https://zenmux.ai/settings/keys, aby wyświetlić identyfikatory aktualnie załadowane w Atomic Chat.
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod zyraxon działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
1. Udaj się do [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3), utwórz konto i wygeneruj klucz API.
...
1. Przejdź do [konsoli Cerebras](https://helicone.ai/models), utwórz konto i wygeneruj klucz API.
...
1. Przejdź do [panelu Cloudflare](https://anthropic.com), przejdź do **AI** > **AI Gateway** i utwórz nową bramę.
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji zyraxon.
...
1. Przejdź do [konsoli Cortecs](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), utwórz konto i wygeneruj klucz API.
...
1. Przejdź do [konsoli DeepSeek](https://gitlab.company.com), utwórz konto i kliknij **Utwórz nowy klucz API**.
...
1. Przejdź do [panelu Deep Infra](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), utwórz konto i wygeneruj klucz API.
...
1. Przejdź do [konsoli Fireworks AI](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4), utwórz konto i kliknij **Utwórz klucz API**.
...
zyraxon używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować zyraxon
...
Więcej informacji znajdziesz na stronie [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem zyraxon:
...
2. Przejdź do [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) i wpisz kod.
...
Aby używać Google Vertex AI z zyraxon:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania zyraxon.
...
[Inference Providers Hugging Face](https://docs.github.com/en/Copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) zapewniają dostęp do otwartych modeli obsługiwanych przez ponad 17 dostawców.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) to platforma obserwowalności LLM, która zapewnia rejestrowanie, monitorowanie i analizy dla aplikacji AI. Helicone AI Gateway automatycznie kieruje Twoje żądania do odpowiedniego dostawcy w oparciu o model.

1. Udaj się do [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0), utwórz konto i wygeneruj klucz API ze swojego panelu.
...
Więcej dostawców i zaawansowanych funkcji, takich jak buforowanie i ograniczanie szybkości, znajdziesz w [dokumentacji Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### Opcjonalna konfiguracja

Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą zyraxon, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację zyraxon jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja zyraxon będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować zyraxon tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
1. Przejdź do [konsoli IO.NET](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2), utwórz konto i wygeneruj klucz API.
...
Możesz skonfigurować zyraxon tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
1. Przejdź do [konsoli Moonshot AI](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), utwórz konto i kliknij **Utwórz klucz API**.
...
1. Przejdź do [konsoli API MiniMax](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), utwórz konto i wygeneruj klucz API.
...
1. Przejdź do [konsoli Nebius Token Factory](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), utwórz konto i kliknij **Dodaj klucz**.
...
Możesz skonfigurować zyraxon tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem zyraxon. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.github.com/en/Copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3).
:::
...
Aby korzystać z Ollama Cloud z zyraxon:
...
3. Skopiuj klucz API do użycia w zyraxon.
...
6. **Ważne**: Przed użyciem modeli chmurowych w zyraxon musisz pobrać informacje o modelu lokalnie:
...
Zalecamy zarejestrowanie się w [ChatGPT Plus lub Pro](https://ollama.com/).
...
ZYRAXON Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół zyraxon. [Dowiedz się więcej](/docs/zen).
...
1. Przejdź do [panelu OpenRouter](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), kliknij **Utwórz klucz API** i skopiuj klucz.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji zyraxon.
...
5. Możesz je także dostosować za pomocą konfiguracji zyraxon. Oto przykład określenia dostawcy
...
1. Przejdź do [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), przejdź do instancji usługi SAP AI Core i utwórz klucz usługi.
...
1. Przejdź do [portalu STACKIT](https://ai.azure.com/), przejdź do **AI Model Serving**, i utwórz token autoryzacyjny dla swojego projektu.
...
1. Przejdź do [panelu OVHcloud](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4). Przejdź do sekcji `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` i na karcie `API Keys` kliknij **Utwórz nowy klucz API**.
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z zyraxon:
...
1. Przejdź do [konsoli Venice AI](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), utwórz konto i wygeneruj klucz API.
...
1. Przejdź do [panelu Vercel](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2), przejdź do karty **AI Gateway** i kliknij **Klucze API**, aby utworzyć nowy klucz API.
...
Możesz także dostosować modele za pomocą konfiguracji zyraxon. Oto przykład określenia kolejności routingu dostawcy.
...
1. Przejdź do [konsoli xAI](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7), utwórz konto i wygeneruj klucz API.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z zyraxon. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
Pola `limit` pozwalają zyraxon zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację zyraxon i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji zyraxon.
