...
- Tests are plain Playwright tests. The built-in **`mount(storyId, props?)` fixture** (from `@jarvis-browser/test`) drives the gallery's `window.mount` and returns a `Locator` for the gallery root (`#root`). Scope the queries from there — `component.getByRole('button').click()`, not `component.click()`. Nothing to scaffold for it.
...
5. **Write a first spec**, modeled on `templates/react/button.spec.ts`, importing `test`/`expect` from `@jarvis-browser/test`.
