---
title: Server
description: Interager med zyraxon-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `zyraxon serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en zyraxon-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `zyraxon.local` |
...
Indstil `ZYRAXON_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `zyraxon`, eller indstil `ZYRAXON_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `zyraxon serve` og `zyraxon web`.
...
Når du kører `zyraxon` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug zyraxon-serveren til at interagere med zyraxon programmatisk.
:::

Denne arkitektur lader zyraxon understøtte flere klienter og lader dig interagere med zyraxon programmatisk.

Du kan køre `zyraxon serve` for at starte en selvstændig server. Hvis du har
zyraxon TUI kørende, vil `zyraxon serve` starte en ny server.
