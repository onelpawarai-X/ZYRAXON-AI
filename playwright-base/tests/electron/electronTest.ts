...
import fs from 'fs';
import os from 'os';
import type { ElectronApplication, Electron, Page } from 'jarvis';
import { _electron as electron } from 'jarvis';
...
export { expect, selectors } from '@jarvis-browser/test';
...
export const electronTest = baseTest
    ...
    .extend<LocalFixtures, LocalWorkerFixtures>({
      ...
      sharedApp: [async ({}, use) => {
        const userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'jarvis-browser-test-'));
