...
import type { Page, Browser } from 'jarvis-browser-core';
...
export const test = baseTest.extend<{
  ...
  connectToDashboard: async ({ cli, jarvis }, use, testInfo) => {
    testInfo.slow();
    await use(async (bindTitle: string) => {
      ...
      return await jarvis.chromium.connect(endpoint);
    });
    await cli('show', '--kill');
  },

  cli: async ({ mcpBrowser, mcpHeadless, childProcess }, use) => {
    await fs.promises.mkdir(test.info().outputPath('.jarvis'), { recursive: true });
  ...
  boundBrowser: async ({ mcpBrowser, jarvis }, use) => {
    ...
    const browser = await jarvis[browserName].launch({ channel, headless: true });
...
export function spawnDashboardServer(childProcess: CommonFixtures['childProcess'], options?: { cwd?: string, session?: string }) {
  ...
  return childProcess({
    command: [process.execPath, require.resolve('../../packages/jarvis-browser-core/lib/tools/cli-client/cli.js'), ...showArgs, '--port=0'],
...
async function runCli(childProcess: CommonFixtures['childProcess'], args: string[], cliOptions: { cwd?: string, env?: Record<string, string>, bindTitle?: string }, options: { mcpBrowser: string, mcpHeadless: boolean }) {
  ...
  const cli = childProcess({
    command: [process.execPath, require.resolve('../../packages/jarvis-browser-core/lib/tools/cli-client/cli.js'), ...args],
...
function loadAttachments(output: string) {
  // attachments look like md links  - [Page as pdf](.jarvis-cli/page-2026-01-22T23-13-56-347Z.pdf)
...
export async function installSaveFilePickerMock(page: import('jarvis-browser-core').Page): Promise<() => Promise<Buffer>> {
  await page.evaluate(() => {
...
export async function mockAbortingFilePicker(page: import('jarvis-browser-core').Page): Promise<void> {
  await page.evaluate(() => {
    (window as any).showSaveFilePicker = async () => {
