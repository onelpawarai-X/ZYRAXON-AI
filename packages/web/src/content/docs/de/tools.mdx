...
:::note
Dieses Tool ist nur verfuegbar, wenn `ZYRAXON_EXPERIMENTAL_LSP_TOOL=true` (oder `ZYRAXON_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der ZYRAXON-Provider genutzt wird oder die Umgebungsvariable `ZYRAXON_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.
