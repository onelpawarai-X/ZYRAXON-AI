---
title: Configuração
description: Usando a configuração JSON do zyraxon.
---

Você pode configurar o zyraxon usando um arquivo de configuração JSON.

---

## Formato

O zyraxon suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/zyraxon`) - padrões organizacionais
2. **Configuração global** (`~/.config/zyraxon/zyraxon.json`) - preferências do usuário
3. **Configuração personalizada** (`ZYRAXON_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`zyraxon.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.zyraxon`** - agentes, comandos, plugins
6. **Configuração inline** (`ZYRAXON_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.zyraxon` e `~/.config/zyraxon` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/zyraxon`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do zyraxon em `~/.config/zyraxon/zyraxon.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/zyraxon/tui.json`.
...
Quando o zyraxon é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `ZYRAXON_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `ZYRAXON_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.zyraxon`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.zyraxon`, então ele **pode substituir** suas configurações.
...
Use `ZYRAXON_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor zyraxon.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `zyraxon.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do zyraxon através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o zyraxon tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/zyraxon/agents/` ou `.zyraxon/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o zyraxon recuará para `"build"` com um aviso.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/zyraxon/commands/` ou `.zyraxon/commands/`. [Saiba mais aqui](/docs/commands).
...
O zyraxon fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o zyraxon **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o zyraxon com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.zyraxon/plugins/` ou `~/.config/zyraxon/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o zyraxon para usar apenas provedores específicos, em vez de desabilitá-los um a um.
