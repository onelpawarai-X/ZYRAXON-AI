...
//
// Configuration flows through zyraxon's built-in test affordances:
...
//
// Today only `zyraxon.run` is fully wired. The shape supports adding more
// builders (`zyraxon.serve(opts)`, `zyraxon.acp(opts)`, `zyraxon.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ZyraxonCli.
...
import { AppProcess } from "@zyraxon-ai/core/process"
...
const zyraxonRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(zyraxonRoot, "src/index.ts")
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("zyraxon.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("zyraxon.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("zyraxon.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
    ...
    const acp = Effect.fn("zyraxon.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const zyraxon: ZyraxonCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, zyraxon })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `zyraxon.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "zyraxon") {
...
// every call site. Use this for any test that needs the zyraxon CLI fixture.
//
...
// resources (e.g. `zyraxon.serve`) without an extra `Effect.scoped` wrapper —
