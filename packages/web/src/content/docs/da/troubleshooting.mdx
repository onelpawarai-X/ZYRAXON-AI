...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\zyraxon\log`
...
zyraxon gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/zyraxon/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\zyraxon`
...
ZYRAXON Desktop kører en lokal ZYRAXON-server (`zyraxon-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
- **macOS/Linux**: `~/.config/zyraxon/zyraxon.jsonc` (eller `~/.config/zyraxon/zyraxon.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/zyraxon/zyraxon.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\zyraxon\zyraxon.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\zyraxon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/zyraxon`
- **Linux**: slet `~/.cache/zyraxon` (eller kør `rm -rf ~/.cache/zyraxon`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\zyraxon`
...
Hvis du har `ZYRAXON_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `ZYRAXON_PORT` (eller vælg en ledig port) og start på ny.
...
- `zyraxon.settings.dat` (skrivebordsstandardserver URL)
- `zyraxon.global.dat` og `zyraxon.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\zyraxon`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. zyraxon installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\zyraxon`

2. Start zyraxon på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge zyraxon til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
zyraxon vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
