...
async function globalSetup() {
  ...
  if (process.env.PWTEST_INSTALLATION_TEST_SKIP_PACKAGE_BUILDS) {
    ...
    const builds = await Promise.all([
      build('jarvis-browser-core'),
      build('jarvis-browser-test', '@jarvis-browser/test'),
      build('jarvis'),
      build('jarvis-browser-chromium'),
      build('jarvis-firefox'),
      build('jarvis-webkit'),
      build('jarvis-browser-chromium', '@jarvis/browser-chromium'),
      build('jarvis-browser-firefox', '@jarvis/browser-firefox'),
      build('jarvis-browser-webkit', '@jarvis/browser-webkit'),
    ]);

    const buildJarvisTestPlugin = async () => {
      const cwd = path.resolve(path.join(__dirname, `jarvis-browser-test-plugin`));
      const tscResult = await spawnAsync('npx', ['tsc', '-p', 'tsconfig.json'], { cwd, shell: process.platform === 'win32' });
      if (tscResult.code)
        throw new Error(`Failed to build jarvis-browser-test-plugin:\n${tscResult.stderr}\n${tscResult.stdout}`);
      const packResult = await spawnAsync('npm', ['pack'], { cwd, shell: process.platform === 'win32' });
      if (packResult.code)
        throw new Error(`Failed to build jarvis-browser-test-plugin:\n${packResult.stderr}\n${packResult.stdout}`);
      const tgzName = packResult.stdout.trim();
      const outPath = path.resolve(path.join(outputDir, `jarvis-browser-test-plugin.tgz`));
      await fs.promises.rename(path.join(cwd, tgzName), outPath);
      console.log('Built: jarvis-browser-test-plugin');
      return ['jarvis-browser-test-plugin', outPath];
    };
    builds.push(await buildJarvisTestPlugin());
