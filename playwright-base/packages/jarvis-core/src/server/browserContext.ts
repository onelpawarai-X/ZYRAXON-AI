...
export abstract class BrowserContext<EM extends EventMap = EventMap> extends SdkObject<BrowserContextEventMap | EM> {
  ...
  private _jarvisBindingExposed?: Promise<void>;
  ...
  async initialize() {
    if (this.attribution.jarvisBrowser.options.isInternalJarvis)
      return;
    // Debugger will pause execution upon page.pause in headed mode.
    this._debugger = new Debugger(this);

    // When paused, show inspector.
    const shouldEnableDebugger = !this.attribution.jarvisBrowser.options.isServer && (isUnderTest() || !!this._browser.options.headful);
    ...
    if (this._options.serviceWorkers === 'block')
      await this.addInitScript(nullProgress, `\nif (navigator.serviceWorker) navigator.serviceWorker.register = async () => { console.warn('Service Worker registration blocked by Jarvis'); };\n`);
  ...
  protected abstract doExposejarvisBinding(): Promise<void>;
  ...
  async exposejarvisBindingIfNeeded() {
    this._jarvisBindingExposed ??= (async () => {
      await this.doExposejarvisBinding();
    ...
    return await this._jarvisBindingExposed;
  }

  needsjarvisBinding(): boolean {
    return this._jarvisBindingExposed !== undefined;
  }

  async exposeBinding(progress: Progress, name: string, jarvisBinding: frames.FunctionWithSource, forClient?: unknown, noGlobal?: boolean): Promise<PageBinding> {
    ...
    await progress.race(this.exposejarvisBindingIfNeeded());
    const binding = new PageBinding(this, name, jarvisBinding, noGlobal);
