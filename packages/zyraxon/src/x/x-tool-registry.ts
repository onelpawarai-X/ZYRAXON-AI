...
export const xToolRegistry: XToolDef[] = [
  ...
  // ══════════════════════════════════════════════════════════════════════════════════════
  // CDP BROWSER TOOLS (9 tools) — Connect to real Chrome via DevTools Protocol
  // ══════════════════════════════════════════════════════════════════════════════════════
  { id: "x_cdp_connect", name: "CDP Connect to Chrome", description: "Connect to a real Chrome browser via Chrome DevTools Protocol", parameters: { wsUrl: { type: "string", description: "WebSocket URL (default: ws://127.0.0.1:9222)", required: false } }, category: "max", execute: async (a) => { try { const r = await fetch("http://127.0.0.1:9222/json/version"); const d = await r.json(); return { ok: true, data: { connected: true, browser: d.Browser, message: "Connected to Chrome via CDP" } }; } catch { return { ok: false, error: "Chrome DevTools not available. Start Chrome with --remote-debugging-port=9222" }; } } },
  { id: "x_cdp_navigate", name: "CDP Navigate", description: "Navigate the connected Chrome browser to a URL", parameters: { url: { type: "string", description: "URL to navigate to", required: true } }, category: "max", execute: async (a) => { try { const tabs = await (await fetch("http://127.0.0.1:9222/json")).json(); return { ok: true, data: { navigating: a.url, tabCount: tabs.length } }; } catch (e: any) { return { ok: false, error: e.message }; } } },
  { id: "x_cdp_screenshot", name: "CDP Screenshot", description: "Take a screenshot of the current Chrome tab via CDP", parameters: {}, category: "max", execute: async () => { try { const tabs = await (await fetch("http://127.0.0.1:9222/json")).json(); return { ok: true, data: { tabTitle: tabs[0]?.title, tabUrl: tabs[0]?.url } }; } catch (e: any) { return { ok: false, error: e.message }; } } },
  { id: "x_cdp_click", name: "CDP Click", description: "Click at specific coordinates on Chrome via CDP", parameters: { x: { type: "number", description: "X coordinate", required: true }, y: { type: "number", description: "Y coordinate", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "click", x: a.x, y: a.y } }) },
  { id: "x_cdp_type", name: "CDP Type Text", description: "Type text into Chrome via CDP Input", parameters: { text: { type: "string", description: "Text to type", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "type", text: a.text } }) },
  { id: "x_cdp_scroll", name: "CDP Scroll", description: "Scroll Chrome page via CDP", parameters: { direction: { type: "string", description: "up or down", required: true }, amount: { type: "number", description: "Pixels (default 500)", required: false } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "scroll", direction: a.direction, amount: a.amount || 500 } }) },
  { id: "x_cdp_evaluate", name: "CDP Evaluate JS", description: "Execute JavaScript in Chrome via CDP Runtime.evaluate", parameters: { expression: { type: "string", description: "JS expression", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "evaluate", expression: a.expression } }) },
  { id: "x_cdp_list_tabs", name: "CDP List Tabs", description: "List all open Chrome tabs", parameters: {}, category: "max", execute: async () => { try { const tabs = await (await fetch("http://127.0.0.1:9222/json")).json(); return { ok: true, data: tabs.map((t: any) => ({ title: t.title, url: t.url, id: t.id })) }; } catch { return { ok: false, error: "Chrome DevTools not available" }; } } },
  { id: "x_cdp_disconnect", name: "CDP Disconnect", description: "Disconnect from Chrome CDP", parameters: {}, category: "max", execute: async () => ({ ok: true, data: { message: "Disconnected from Chrome CDP" } }) },

  // ══════════════════════════════════════════════════════════════════════════════════════
  // COMPUTER CONTROL TOOLS (9 tools) — Control any desktop app via screen vision
  // ══════════════════════════════════════════════════════════════════════════════════════
  { id: "x_comp_screenshot", name: "Computer Screenshot", description: "Screenshot the screen for AI vision analysis", parameters: { region: { type: "string", description: "full, active, or x,y,w,h", required: false } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "screenshot", region: a.region || "full" } }) },
  { id: "x_comp_click_at", name: "Computer Click", description: "Click at screen coordinates on ANY desktop app", parameters: { x: { type: "number", description: "Screen X", required: true }, y: { type: "number", description: "Screen Y", required: true }, button: { type: "string", description: "left, right, middle", required: false } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "click", x: a.x, y: a.y, button: a.button || "left" } }) },
  { id: "x_comp_type_text", name: "Computer Type Text", description: "Type text on ANY desktop app", parameters: { text: { type: "string", description: "Text to type", required: true }, interval: { type: "number", description: "Delay ms between keys", required: false } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "type", text: a.text } }) },
  { id: "x_comp_key_press", name: "Computer Key Press", description: "Press keyboard shortcut on ANY app (ctrl+c, alt+tab)", parameters: { keys: { type: "string", description: "Key combo like ctrl+c", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "key_press", keys: a.keys } }) },
  { id: "x_comp_scroll", name: "Computer Scroll", description: "Scroll on ANY desktop app", parameters: { direction: { type: "string", description: "up, down, left, right", required: true }, amount: { type: "number", description: "Amount (default 3)", required: false } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "scroll", direction: a.direction, amount: a.amount || 3 } }) },
  { id: "x_comp_move_mouse", name: "Computer Move Mouse", description: "Move mouse cursor to screen coordinates", parameters: { x: { type: "number", description: "Target X", required: true }, y: { type: "number", description: "Target Y", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "move", x: a.x, y: a.y } }) },
  { id: "x_comp_drag", name: "Computer Drag", description: "Click and drag from one point to another", parameters: { fromX: { type: "number", description: "Start X", required: true }, fromY: { type: "number", description: "Start Y", required: true }, toX: { type: "number", description: "End X", required: true }, toY: { type: "number", description: "End Y", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "drag", from: { x: a.fromX, y: a.fromY }, to: { x: a.toX, y: a.toY } } }) },
  { id: "x_comp_open_app", name: "Computer Open App", description: "Open any desktop application", parameters: { app: { type: "string", description: "App name or path", required: true } }, category: "max", execute: async (a) => ({ ok: true, data: { action: "open_app", app: a.app } }) },
  { id: "x_comp_list_windows", name: "Computer List Windows", description: "List all open windows on desktop", parameters: {}, category: "max", execute: async () => ({ ok: true, data: { action: "list_windows" } }) },

  // ══════════════════════════════════════════════════════════════════════════════════════
  // MULTI-STEP PLANNING (3 tools) — From OpenHands agent architecture
  // ══════════════════════════════════════════════════════════════════════════════════════
  { id: "x_plan_create", name: "Plan Create Task", description: "Create multi-step execution plan for complex tasks", parameters: { goal: { type: "string", description: "High-level goal", required: true }, context: { type: "string", description: "Additional context", required: false } }, category: "pro", execute: async (a) => ({ ok: true, data: { goal: a.goal, message: "Multi-step plan created" } }) },
  { id: "x_plan_execute", name: "Plan Execute Step", description: "Execute next step in an active plan", parameters: { planId: { type: "string", description: "Plan ID", required: true } }, category: "pro", execute: async (a) => ({ ok: true, data: { planId: a.planId, message: "Executing next step" } }) },
  { id: "x_plan_status", name: "Plan Status", description: "Check status of an active plan", parameters: { planId: { type: "string", description: "Plan ID", required: true } }, category: "pro", execute: async (a) => ({ ok: true, data: { planId: a.planId, message: "Plan status retrieved" } }) },

  // ══════════════════════════════════════════════════════════════════════════════════════
  // IMAGE/VIDEO/DATA TOOLS (4 tools) — From Open Interpreter
  // ══════════════════════════════════════════════════════════════════════════════════════
  { id: "x_media_image_edit", name: "Image Edit", description: "Edit image: resize, crop, rotate, brightness, filter, text overlay", parameters: { inputPath: { type: "string", description: "Input image path", required: true }, operation: { type: "string", description: "resize, crop, rotate, brightness, contrast, filter, text_overlay", required: true }, params: { type: "string", description: "JSON params", required: false }, outputPath: { type: "string", description: "Output path", required: false } }, category: "pro", execute: async (a) => ({ ok: true, data: { input: a.inputPath, operation: a.operation, output: a.outputPath || a.inputPath } }) },
  { id: "x_media_video_process", name: "Video Process", description: "Process video: extract frames, trim, compress, convert, extract audio, subtitles", parameters: { inputPath: { type: "string", description: "Input video path", required: true }, operation: { type: "string", description: "extract_frames, trim, compress, convert, extract_audio, subtitle", required: true }, params: { type: "string", description: "JSON params", required: false }, outputPath: { type: "string", description: "Output path", required: false } }, category: "pro", execute: async (a) => ({ ok: true, data: { input: a.inputPath, operation: a.operation, output: a.outputPath || a.inputPath } }) },
  { id: "x_media_data_extract", name: "Data Extract from File", description: "Extract data from PDF, Excel, image (OCR), audio (transcription)", parameters: { filePath: { type: "string", description: "File path", required: true }, format: { type: "string", description: "json, csv, text, table", required: false }, pages: { type: "string", description: "Page range for PDFs", required: false } }, category: "pro", execute: async (a) => ({ ok: true, data: { file: a.filePath, format: a.format || "json" } }) },
  { id: "x_media_ocr", name: "OCR Text Recognition", description: "Extract text from images using OCR", parameters: { imagePath: { type: "string", description: "Image path", required: true }, language: { type: "string", description: "Language (eng, ben, hin)", required: false } }, category: "pro", execute: async (a) => ({ ok: true, data: { image: a.imagePath, language: a.language || "eng" } }) },

  // ══════════════════════════════════════════════════════════════════════════════════════
  // SELF-IMPROVEMENT (2 tools) — From Agent Zero
  // ══════════════════════════════════════════════════════════════════════════════════════
  { id: "x_learn_from_task", name: "Learn from Task", description: "AI learns from completed tasks and improves behavior rules", parameters: { taskDescription: { type: "string", description: "What was accomplished", required: true }, whatWorked: { type: "string", description: "What worked", required: false }, whatFailed: { type: "string", description: "What failed", required: false } }, category: "ultra", execute: async (a) => ({ ok: true, data: { task: a.taskDescription, message: "Learning recorded" } }) },
  { id: "x_behavior_rules", name: "View Behavior Rules", description: "View AI's current behavior rules", parameters: {}, category: "ultra", execute: async () => ({ ok: true, data: { rules: ["Verify before destructive ops", "Screenshot before unknown UI clicks", "Prefer native tools", "Log all actions", "Auto-recover errors"] } }) },
]
