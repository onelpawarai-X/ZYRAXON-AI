...
import { DispatcherConnection, JarvisDispatcher, RootDispatcher } from '../server';
import { AndroidDevice } from '../server/android/android';
import { Browser } from '../server/browser';
import { DebugControllerDispatcher } from '../server/dispatchers/debugControllerDispatcher';
import { JarvisDispatcherOptions } from '../server/dispatchers/jarvisDispatcher';

import type { DispatcherScope, Jarvis } from '../server';
import type { ServerTransport } from './serverTransport';

export interface JarvisInitializeResult extends JarvisDispatcherOptions {
  dispose?(): Promise<void>;
}

export class JarvisConnection {
  ...
  constructor(semaphore: Semaphore, transport: ServerTransport, controller: boolean, jarvis: Jarvis, initialize: () => Promise<JarvisInitializeResult>, id: string) {
    ...
    if (controller) {
      ...
      this._root = new DebugControllerDispatcher(this._dispatcherConnection, jarvis.debugController);
      return;
    }
    ...
    this._root = new RootDispatcher(this._dispatcherConnection, async (scope, params) => {
      ...
      const dispatcher = new JarvisDispatcher(scope, jarvis, options);
