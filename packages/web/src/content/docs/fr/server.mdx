---
title: Serveur
description: Interagissez avec le serveur zyraxon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `zyraxon serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client zyraxon peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `zyraxon.local` |
...
Définissez `ZYRAXON_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `zyraxon`, ou définissez `ZYRAXON_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `zyraxon serve` et à `zyraxon web`.
...
Lorsque vous exécutez `zyraxon`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur zyraxon pour interagir avec zyraxon par programmation.
:::

Cette architecture permet à zyraxon de prendre en charge plusieurs clients et vous permet d'interagir avec zyraxon par programme.

Vous pouvez exécuter `zyraxon serve` pour démarrer un serveur autonome. Si vous avez le
zyraxon TUI en cours d'exécution, `zyraxon serve` démarrera un nouveau serveur.
...
Le serveur zyraxon expose les API suivantes.
