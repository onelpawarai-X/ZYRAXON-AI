...
export class AndroidServerLauncherImpl {
  async launchServer(options: LaunchAndroidServerOptions = {}): Promise<BrowserServer> {
    ...
    let devices = await controller.run(progress => jarvis.android.devices(progress, {
    ...
    // 2. Start the server
    const server = new JarvisServer({ mode: 'launchServer', path, maxConnections: 1, preLaunchedAndroidDevice: device });
