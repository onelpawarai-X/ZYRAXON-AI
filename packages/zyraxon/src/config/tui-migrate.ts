...
export async function migrateTuiConfig(input: MigrateInput) {
  const zyraxon = await zyraxonFiles(input)
  for (const file of zyraxon) {
...
async function zyraxonFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "zyraxon"),
    ...(await Filesystem.findUp(["zyraxon.json", "zyraxon.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "zyraxon"))
  }
