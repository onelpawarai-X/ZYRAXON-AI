...
|---------|----------|---------|
| `jarvis-browser-core` | `jarvis-browser-core` | Browser automation engine: client, server, dispatchers, protocol |
| `playwright` | `playwright` | Test runner + browser automation (public package) |
| `jarvis-browser-test` | `@jarvis-browser/test` | Test runner entry point |
...
`jarvis-browser-chromium`, `playwright-firefox`, `playwright-webkit` — per-browser distributions.
...
| Directory | Purpose |
|-----------|---------|
| `tests/` | All test suites (page, library, jarvis-browser-test, mcp, etc.) |
...
| `npm run ttest <filter>` | Test runner (`tests/jarvis-browser-test/`) |
...
| `tests/jarvis-browser-test/` | `import { test, expect } from './jarvis-browser-test-fixtures'` | test runner fixtures | Test runner: reporters, config, annotations, retries |
...
Fixes: https://github.com/zyraxon-ai/jarvis-browser/issues/39562
EOF
)"
