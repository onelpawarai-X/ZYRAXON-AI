...
export const xToolRegistry: XToolDef[] = [
  ...
  { id: "x_scenario_create", name: "Scenario Create", description: "Create simulation scenario", parameters: { id: { type: "string", required: true }, initialState: { type: "object", required: true }, events: { type: "array", required: true } }, category: "digital-twin", execute: async (a) => new ScenarioRunner().createScenario(a.id, a.initialState, a.events) },
  ...
  { id: "x_sim_add_task", name: "Sim Scheduler Add Task", description: "Add simulation task", parameters: { id: { type: "string", required: true }, interval: { type: "number", required: true } }, category: "digital-twin", execute: async (a) => { const c = new SimulationScheduler(); c.addTask(a.id, a.interval, () => {}); return { ok: true } } },
