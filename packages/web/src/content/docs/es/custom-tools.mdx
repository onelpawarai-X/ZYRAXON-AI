...
description: Cree herramientas que LLM pueda llamar en zyraxon.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de zyraxon como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.zyraxon/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/zyraxon/tools/`.
...
```ts title=".zyraxon/tools/database.ts" {1}
...
```ts title=".zyraxon/tools/math.ts"
import { tool } from "@zyraxon-ai/plugin"
...
```ts title=".zyraxon/tools/bash.ts"
import { tool } from "@zyraxon-ai/plugin"
...
```ts title=".zyraxon/tools/project.ts" {8}
import { tool } from "@zyraxon-ai/plugin"
...
```python title=".zyraxon/tools/add.py"
import sys
...
```ts title=".zyraxon/tools/python-add.ts" {10}
import { tool } from "@zyraxon-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".zyraxon/tools/add.py")
