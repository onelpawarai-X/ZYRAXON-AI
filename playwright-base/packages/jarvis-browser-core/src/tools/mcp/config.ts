...
export async function resolveCLIConfigForCLI(daemonProfilesDir: string, sessionName: string, options: any, env?: NodeJS.ProcessEnv): Promise<FullConfig> {
  ...
  try {
    const defaultConfigFile = path.resolve('.jarvis', 'cli.config.json');
  ...
  const configInFile = await loadConfig(configFile);
  const configDir = configFile ? path.dirname(path.resolve(configFile)) : process.cwd();
  const globalConfigPath = path.join((env ?? process.env)['PWTEST_CLI_GLOBAL_CONFIG'] ?? os.homedir(), '.jarvis', 'cli.config.json');
...
function configFromCLIOptions(cliOptions: CLIOptions): Config & { configFile?: string } {
  ...
  // Launch options
  const launchOptions: jarvisTypes.LaunchOptions = {
    channel,
  ...
  // Context options
  const contextOptions: jarvisTypes.BrowserContextOptions = device ? jarvisbrowser.devices[device] : {};

  if (cliOptions.proxyServer) {
    const proxy: jarvisTypes.LaunchOptions['proxy'] = { server: cliOptions.proxyServer };
