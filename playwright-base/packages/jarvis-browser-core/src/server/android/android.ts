/**
 * Copyright ZYRAXON AI. All rights reserved.
 *
...
const ARTIFACTS_FOLDER = path.join(os.tmpdir(), 'jarvis-artifacts-');
...
export class AndroidDevice extends SdkObject {
  ...
  private async _installDriver(progress: Progress): Promise<PipeTransport> {
    ...
    await progress.race(this._shell(`am force-stop com.microsoft.jarvis.androiddriver`));

    // uninstall and install driver on every execution
    if (!this._options.omitDriverInstall) {
      debug('pw:android')('Uninstalling the old driver');
      await this.shell(progress, `cmd package uninstall com.microsoft.jarvis.androiddriver`);
      await this.shell(progress, `cmd package uninstall com.microsoft.jarvis.androiddriver.test`);
      ...
      for (const file of ['android-driver.apk', 'android-driver-target.apk']) {
        ...
        if (!fs.existsSync(fullName))
          throw new Error(`Please install Android driver apk using '${packageManagerCommand} jarvis install android'`);
    ...
    this._shell('am instrument -w com.microsoft.jarvis.androiddriver.test/androidx.test.runner.AndroidJUnitRunner').catch(e => debug('pw:android')(e));
    const socket = await this._waitForLocalAbstract(progress, 'jarvis_android_driver_socket');
  ...
  async launchBrowser(progress: Progress, pkg: string = 'com.android.chrome', options: channels.AndroidDeviceLaunchBrowserParams): Promise<BrowserContext> {
    ...
    const socketName = isUnderTest() ? 'webview_devtools_remote_jarvis_test' : ('jarvis_' + createGuid() + '_devtools_remote');
  ...
  private async _connectToBrowser(progress: Progress, socketName: string, options: types.BrowserContextOptions = {}): Promise<BrowserContext> {
    ...
    try {
      ...
      const browser = await progress.race(CRBrowser.connect(this.attribution.jarvisBrowser, androidBrowser, browserOptions));
