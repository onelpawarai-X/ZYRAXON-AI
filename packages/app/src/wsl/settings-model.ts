...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-zyraxon" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslZyraxonAction(check?: WslZyraxonCheck) {
  if (!check) return
...
export function addServerViewModel(input: {
  ...
  const zyraxonCheck = selectedDistro ? (state?.zyraxonChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.zyraxonChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateZyraxon" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.zyraxonMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installZyraxon" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  zyraxonCheck: WslZyraxonCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingZyraxon =
    input.state?.job?.kind === "install-zyraxon" && input.state.job.distro === input.selectedDistro
  if (!ready || probingZyraxon) {
    return {
      variant: "contrast",
  ...
  if (!addServerZyraxonReady(input.zyraxonCheck)) {
    const update = !!input.zyraxonCheck?.resolvedPath && input.zyraxonCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingZyraxon
        ? { key: "wsl.onboarding.updatingZyraxon" }
        : update
          ? { key: "wsl.onboarding.updateZyraxon" }
          : { key: "wsl.onboarding.installZyraxon" },
      disabled: !!input.state?.job || input.adding,
      action: "install-zyraxon",
      loading: installingZyraxon,
...
function addServerZyraxonReady(check: WslZyraxonCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.zyraxonChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.zyraxonChecks[item.name]) return [`zyraxon:${item.name}`]
    return []
  })
