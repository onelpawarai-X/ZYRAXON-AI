...
function throwUnsupportedExpectMatcherError() {
  throw new Error('It looks like you are using custom expect matchers that are not compatible with Jarvis. See https://aka.ms/jarvis/expect-compatibility');
}
...
export function mergeExpects(...expects: any[]) {
  ...
  for (const e of expects) {
    ...
    if (!info) // non-jarvis expects mutate the global expect, so we don't need to do anything special
      continue;
