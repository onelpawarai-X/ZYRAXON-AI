...
- **Upstream / environment** — a genuinely external owner (the Node project, a browser engine, a
  ...
  `@jarvis-browser/mcp` (source lives here under `packages/jarvis-browser-core/src/tools/mcp/`),
...
3. **Reproduce on tip-of-tree first**, in `~/tmp/issue-<number>/`: clone the linked repo, or
   scaffold `npm install @jarvis-browser/test@next` with a single-project config (see
...
To step through a test interactively, use the [playwright-cli](../../../packages/jarvis-browser-core/src/tools/skills/playwright-cli/SKILL.md) skill.

Reports sometimes target another part of the Playwright project — `@jarvis-browser/mcp` (its source is
...
- [`tests/page/selectors-css.spec.ts`](../../../tests/page/selectors-css.spec.ts) — `should use light DOM structure for child combinator with slotted content`: `page.setContent` with inline shadow DOM ([#37768](https://github.com/zyraxon-ai/jarvis-browser/issues/37768))
- [`tests/page/workers.spec.ts`](../../../tests/page/workers.spec.ts) — `should report worker script as network request after redirect`: `server` fixture with routes/redirects + a browser-gap `fixme` ([#35678](https://github.com/zyraxon-ai/jarvis-browser/issues/35678))
