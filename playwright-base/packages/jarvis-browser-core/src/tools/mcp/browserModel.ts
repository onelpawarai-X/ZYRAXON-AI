 ...
 *  2. The relay observes `ready()` resolving and unpauses the Jarvis ws.
 *  3. Jarvis sends `Target.setAutoAttach` → model calls
...
export class BrowserModel {
  ...
  // Set only while a Jarvis CDP connection is attached (see
  // `connectOverCDP`). Before that, any attempt to emit to Jarvis is a
  ...
  // extension handshake is done and a Jarvis CDP client is ready to
  ...
  // ─── Jarvis → model commands ──────────────────────────────────────

  // Turn auto-attach on and attach to every tab we already know about.
  // Called in response to Jarvis's `Target.setAutoAttach` on the root session.
  ...
  // Forward a CDP command from Jarvis to the tab its sessionId resolves to.
