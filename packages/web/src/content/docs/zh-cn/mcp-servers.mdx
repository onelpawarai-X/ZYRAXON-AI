...
组织可以通过其 `.well-known/zyraxon` 端点提供默认的 MCP 服务器。这些服务器可能默认处于禁用状态，允许用户按需启用。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，ZYRAXON 会将 Token 安全地存储在 `~/.local/share/zyraxon/mcp-auth.json` 中。
...
添加 [Grep by Vercel](https://github.com/upstash/context71) MCP 服务器以搜索 GitHub 上的代码片段。
