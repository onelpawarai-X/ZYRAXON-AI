...
( cd ~/tmp/<good> && npm init -y && npm install @jarvis-browser/test@<good-ver> && npx playwright install chromium)
( cd ~/tmp/<bad>  && npm init -y && npm install @jarvis-browser/test@<bad-ver> && npx playwright install chromium )
```
...
The compiled JS in `node_modules/jarvis-browser-core/lib/` and `node_modules/playwright/lib/` is the source of truth for what shipped.
...
- **Don't run `npm init playwright@latest`** — it's interactive and `--quiet` does not skip the prompts. `npm init -y` + `npm install @jarvis-browser/test@<ver>` is faster and deterministic.
