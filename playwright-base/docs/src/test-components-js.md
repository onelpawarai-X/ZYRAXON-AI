...
Tests run in Node.js while components run in a real browser: real clicks are triggered, real layout is executed, visual regression is possible. At the same time, tests get everything Jarvis Browser Test offers: parallelism, parametrization, retries and post-mortem tracing.
...
- **It is stable.** Tests import `test` and `expect` from plain `@jarvis-browser/test`, and [`method: Fixtures.mount`] is a documented built-in fixture. There is no experimental package to depend on and no separate config dialect.
...
```sh
npx Jarvis Browser Test --project=components
```
...
| `defineConfig` from the ct package | Plain `defineConfig` from `@jarvis-browser/test` |
