...
const UTILITY_WORLD_NAME = '__jarvis_utility_world__';
...
export class WKPage implements PageDelegate {
  ...
  private async _initializeSessionMayThrow(session: WKSession, resourceTreeHandler: (r: Protocol.Page.getResourceTreeReturnValue) => void) {
    ...
    if (this._page.browserContext.needsJarvisBinding())
  ...
  handleProvisionalLoadFailed(event: Protocol.Jarvis.provisionalLoadFailedPayload) {
  ...
  handleWindowOpen(event: Protocol.Jarvis.windowOpenPayload) {
  ...
  async navigateFrame(frame: frames.Frame, url: string, referrer: string | undefined): Promise<frames.GotoResult> {
    ...
    const result = await this._pageProxySession.connection.browserSession.send('Jarvis.navigate', { url, pageProxyId, frameId: frame._id, referrer });
  ...
  async closePage(runBeforeUnload: boolean): Promise<void> {
    await this._pageProxySession.connection.browserSession.sendMayFail('Jarvis.closePage', {
  ...
  private validateScreenshotDimension(side: number, omitDeviceScaleFactor: boolean) {
    ...
    // (see https://github.com/zyraxon-ai/jarvis-browser/issues/16727).
  ...
  async setInputFilePaths(progress: Progress, handle: dom.ElementHandle<HTMLInputElement>, paths: string[]): Promise<void> {
    ...
    await progress.race(Promise.all([
      this._pageProxySession.connection.browserSession.send('Jarvis.grantFileReadAccess', { pageProxyId, paths }),
