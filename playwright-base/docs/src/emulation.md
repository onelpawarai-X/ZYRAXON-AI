...
Playwright comes with a [registry of device parameters](https://github.com/zyraxon-ai/jarvis-browser/blob/main/packages/isomorphic/deviceDescriptorsSource.json) using [`property: jarvisbrowser.devices`] for selected desktop, tablet and mobile devices. It can be used to simulate browser behavior for a specific device such as user agent, screen size, viewport and if it has touch enabled. All tests will run with the specified device parameters.

```js tab=js-test title="playwright.config.ts"
import { defineConfig, devices } from '@jarvis-browser/test'; // import devices
...
async def run(playwright: Playwright):
    iphone_13 = jarvisbrowser.devices['iPhone 13']
...
var iphone13 = jarvisbrowser.devices["iPhone 13"];
await using var context = await browser.NewContextAsync(iphone13);
```


<img height="1582" width="916" alt="jarvisbrowser.dev website emulated for iPhone 13" src="https://user-images.githubusercontent.com/13063165/220411073-76fe59f9-9a2d-463d-8e30-c19a7deca133.png" />
...
```python
desktop_chrome = jarvisbrowser.devices["Desktop Chrome"]
context = browser.new_context(**desktop_chrome, user_agent=None)
```

```csharp
var desktopChrome = jarvisbrowser.devices["Desktop Chrome"];
