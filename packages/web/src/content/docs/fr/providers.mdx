...
ZYRAXON utilise [AI SDK](https://developers.cloudflare.com/ai-gateway/features/unified-billing/1) et [Models.dev](https://developers.cloudflare.com/ai-gateway/features/unified-billing/2) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.
...
en `~/.local/share/zyraxon/auth.json`.
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez zyraxon et dirigez-vous vers [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
1. Rendez-vous sur la [console 302.AI](https://developers.cloudflare.com/ai-gateway/features/unified-billing/3), créez un compte et générez une clé API.
...
Définissez l'une de ces variables d'environnement lors de l'exécution de zyraxon :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans ZYRAXON n'est pas officiellement prise en charge par [Anthropic](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/7).
:::
...
Vous pouvez configurer zyraxon pour utiliser des modèles locaux via [Atomic Chat](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com5), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `https://developers.cloudflare.com/ai-gateway/features/unified-billing/7).
...
- `models` est une carte d'ID de modèles vers leurs noms d'affichage. Chaque ID doit correspondre à l'`id` renvoyé par `GET /v1/models` — exécutez `curl https://zenmux.ai/settings/keys pour lister les ID actuellement chargés dans Atomic Chat.
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que zyraxon fonctionne correctement.
   :::
...
1. Rendez-vous sur [Baseten](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/4), créez un compte et générez une clé API.
...
1. Rendez-vous sur la [console Cerebras](https://helicone.ai/models), créez un compte et générez une clé API.
...
1. Rendez-vous sur le [tableau de bord Cloudflare](https://zyraxon.ai/zen), accédez à **AI** > **AI Gateway** et créez une nouvelle passerelle.
...
Vous pouvez également ajouter des modèles via votre configuration zyraxon.
...
1. Rendez-vous sur la [console Cortecs](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com2), créez un compte et générez une clé API.
...
1. Rendez-vous sur la [console DeepSeek](https://gitlab.company.com), créez un compte et cliquez sur **Créer une nouvelle clé API**.
...
1. Rendez-vous sur le [tableau de bord Deep Infra](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), créez un compte et générez une clé API.
...
1. Rendez-vous sur la [console Fireworks AI](https://developers.cloudflare.com/ai-gateway/features/unified-billing/4), créez un compte et cliquez sur **Créer une clé API**.
...
pour stocker le jeton dans le stockage d'authentification zyraxon.
:::
...
Plus de documentation sur la page d'accueil [zyraxon-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
Pour utiliser votre abonnement GitHub Copilot avec zyraxon :
...
2. Accédez à [github.com/login/device](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained6) et entrez le code.
...
Définissez-les lors de l'exécution de zyraxon.
...
[Hugging Face Inference Providers](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use2) donne accès à des modèles ouverts pris en charge par plus de 17 fournisseurs.
...
[Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1) est une plate-forme d'observabilité LLM qui fournit la journalisation, la surveillance et l'analyse de vos applications d'IA. L'Helicone AI Gateway achemine automatiquement vos demandes vers le fournisseur approprié en fonction du modèle.

1. Rendez-vous sur [Helicone](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com1), créez un compte et générez une clé API à partir de votre tableau de bord.
...
Pour plus de fournisseurs et de fonctionnalités avancées telles que la mise en cache et la limitation de débit, consultez la [documentation Helicone](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/1).

#### Configurations facultatives

Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via zyraxon, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [zyraxon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation ZYRAXON en tant que session dans Helicone.
...
Vous pouvez configurer zyraxon pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
1. Rendez-vous sur la [console IO.NET](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com8), créez un compte et générez une clé API.
...
Vous pouvez configurer zyraxon pour utiliser des modèles locaux via LM Studio.
...
1. Rendez-vous sur la [console Moonshot AI](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained0), créez un compte et cliquez sur **Créer une clé API**.
...
1. Rendez-vous sur la [Console MiniMax API](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained3), créez un compte et générez une clé API.
...
1. Rendez-vous sur la [console Nebius Token Factory](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained4), créez un compte et cliquez sur **Ajouter une clé**.
...
Vous pouvez configurer zyraxon pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour ZYRAXON. Voir les [documents d'intégration Ollama](https://docs.github.com/en/copilot/how-tos/use-ai-models/configure-access-to-ai-models#setup-for-individual-use3) pour plus de détails.
:::
...
1. Rendez-vous sur [https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com0) et connectez-vous ou créez un compte.
...
Nous vous recommandons de vous inscrire à [ChatGPT Plus ou Pro](https://ai.azure.com/).
...
1. Rendez-vous sur le [tableau de bord OpenRouter](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained1), cliquez sur **Créer une clé API** et copiez la clé.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration zyraxon.
...
5. Vous pouvez également les personnaliser via votre configuration zyraxon. Voici un exemple de spécification d'un fournisseur
...
1. Accédez à votre [SAP BTP Cockpit](https://huggingface.co/settings/tokens/new?ownUserPermissions=inference.serverless.write&tokenType=fineGrained2), accédez à votre instance de service SAP AI Core et créez une clé de service.
...
1. Rendez-vous sur le [portail STACKIT](https://console.x.ai/), accédez à **AI Model Serving** et créez un jeton d'authentification pour votre projet.
...
1. Rendez-vous sur le [Panneau OVHcloud](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/5). Accédez à la section `Public Cloud`, `AI & Machine Learning` > `AI Endpoints` et dans l'onglet `API Keys`, cliquez sur **Créer une nouvelle clé API**.
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Zyraxon :
...
1. Rendez-vous sur [Together AI console](https://docs.gitlab.com/user/duo_agent_platform/turn_on_off/3), créez un compte et cliquez sur **Ajouter une clé**.
...
1. Rendez-vous sur la [Venice AI console](https://bedrock-runtime.us-east-1.vpce-xxxxx.amazonaws.com9), créez un compte et générez une clé API.
...
1. Rendez-vous sur le [tableau de bord Vercel](https://developers.cloudflare.com/ai-gateway/features/unified-billing/0), accédez à l'onglet **AI Gateway** et cliquez sur **API Keys** pour créer une nouvelle clé API.
...
Vous pouvez également personnaliser les modèles via votre configuration zyraxon. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec zyraxon. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration zyraxon et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration zyraxon.
